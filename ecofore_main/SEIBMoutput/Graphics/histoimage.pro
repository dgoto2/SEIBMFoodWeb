...
IF N_Elements(image) EQ 0 THEN image = LoadData(4)
...
IF N_Elements(binsize) EQ 0 THEN BEGIN
   ...
   binsize = 2.0 > (range / 128.0); an integer cannot be divided by an integer
ENDIF
...
histoPos =    [0.15, 0.675, 0.95, 0.950]; Normalized coordinates
