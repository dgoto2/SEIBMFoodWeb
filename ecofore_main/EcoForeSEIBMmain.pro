PRO EcoForeSEIBMmain; *Lake Erie Spatially Explicit Indiviudal-Based Models for walleye (WAE), 
...
nYP = 30000L ; number of YEP superindividuals(SIs)
nWAE = 30000L; number of WAE SIs
nRAS = 30000L; number of RAS SIs
nEMS = 30000L; number of EMS SIs
nROG = 30000L; number of ROG SIs

; INITIAL TOTAL NUMBER OF FISH POPUALTION IN CENTRAL LAKE ERIE; 50,000,000
NpopYP = (103540000D + 78843333D + 122247000D + 3257000D + 23600000D + 1645000D + 8300000D)*2.; number of YEP individuals
NpopWAE = (10710000D + 6960000D + 30129000D + 150000D + 8138000D + 553000D + 2430000D + 1028000D)*2. ; number of WAE individuals
NpopRAS = (36683333D + 63653333D)*3.; number of RAS individuals
NpopEMS = (446761667D + 432036667D)*3.; number of EMS individuals
NpopROG = (201138333D + 260038333D)*3.; number of ROG individuals
;PRINT, NpopYP 
;PRINT, NpopWAE 
;PRINT, NpopRAS
;PRINT, NpopEMS 
...
FOR iDay = 182L - 1L, 304L - 1L DO BEGIN;(Change iday values in other sub routines)*************DAYLY LOOP***************************************************************
  ...
  IF iDAY EQ 181L THEN BEGIN; iday = DOY -1, ONLY DONE ONCE AT THE BEGINNING OF SIMULATIONS--> DOY NEEDS TO BE CHANGED WHEN TESTING
  ...
  TotBenBio = TotBenBio + Envir3d[8, *]
  
  FOR iHour = 0L, 23L DO BEGIN;************************************HOURLY LOOP***************************************************************
    PRINT,  'DAY', iday + 1L, '     HOUR', ihour
    ...
    ; Call only an hourly input from a daily input read from the file
     ...
     IF ((iDay EQ 181L) AND (iHour EQ 0L)) THEN BEGIN; iday = DOY-1
        YP = YEPinitial(NpopYP, nYP, TotBenBio, envir3d2, nGridcell)  
       WAE = WAEinitial(NpopWAE, nWAE, TotBenBio, envir3d2, nGridcell)  
       RAS = RASinitial(NpopRAS, nRAS, TotBenBio, envir3d2, nGridcell)  
       EMS = EMSinitial(NpopEMS, nEMS, TotBenBio, envir3d2, nGridcell)  
       ROG = ROGinitial(NpopROG, nROG, TotBenBio, envir3d2, nGridcell) 
     ENDIF
...
;        
       ...
       ;IF(iHour EQ 0L) AND (iTime EQ 0L)THEN BEGIN; only evaluate once a day
          YPmoveHV = YEPMoveHV(ts, iHour, YP, nYP, envir3d2, TotBenBio, FISHPREY, Grid2D2, YP[39, *], YP[40, *], YP[41, *], YP[63, *])
          WEmoveHV = WAEMoveHV(ts, iHour, WAE, nWAE, envir3d2, TotBenBio, FISHPREY, Grid2D2, WAE[39, *], WAE[40, *], WAE[41, *], WAE[63, *])
          RSmoveHV = RASMoveHV(ts, iHour, RAS, nRAS, envir3d2, TotBenBio, Grid2D2, RAS[39, *], RAS[40, *], RAS[41, *], RAS[63, *])
          ESmoveHV = EMSMoveHV(ts, iHour, EMS, nEMS, envir3d2, TotBenBio, Grid2D2, EMS[39, *], EMS[40, *], EMS[41, *], EMS[63, *])
          RGmoveHV = ROGMoveHV(ts, iHour, ROG, nROG, envir3d2, TotBenBio, Grid2D2, ROG[39, *], ROG[40, *], ROG[41, *], ROG[63, *])
...
;        WAEFISHPREY[3, WAE[14, *]] = WAE[2, *] * WAE[0, *]; ARRAY FOR FISH PREY TOTAL BIOMASS)
     ...
     ENDIF
       ...
       ;**********************************************************************************************************
        ...
        ;PRINT,'EMSenc[29:34, *]', EMSenc[29:34, *]
       ...
       ;*********************************************************************************************************
        ...
        YPres = YEPresp(YP[19, *], YP[27, *], YP[2, *], YP[1, *], ts, YP[20, *], YP[29, *], YacclDO[4, *], nYP, YP[63, *]) ; determine respiration for yellow perch 
        ...
        ; Growth
        YPgro = YEPgrowth(YP[1, *], YP[2, *], YP[3, *], YP[4, *], YPcmx, YPeaten, YPres[0, *], YP[26, *], nYP, ts, iday, ihour, iTime)
        WAEgro = WAEgrowth(WAE[1, *], WAE[2, *], WAE[3, *], WAE[4, *], WAEcmx, WAEeaten, WAEres[0, *], WAE[26, *], nWAE, ts, iday, ihour, iTime)
        RASgro = RASgrowth(RAS[1, *], RAS[2, *], RAS[3, *], RAS[4, *], RAScmx, RASeaten, RASres[0, *], RAS[26, *], nRAS, ts, iday, ihour, iTime)
        EMSgro = EMSgrowth(EMS[1, *], EMS[2, *], EMS[3, *], EMS[4, *], EMScmx, EMSeaten, EMSres[0, *], EMS[26, *], nEMS, ts, iday, ihour, iTime)
        ROGgro = ROGgrowth(ROG[1, *], ROG[2, *], ROG[3, *], ROG[4, *], ROGcmx, ROGeaten, ROGres[0, *], ROG[26, *], nROG, ts, iday, ihour, iTime)
  ...
  ;ENDIF 
    ...
    ; Create output files for the daytime
    IF iHOUR EQ 11L THEN BEGIN
      ;***Creat an output file for YEP*********************************************************
      ;counter =  iDay - 213L + 1L; Same as the initial day of a daily loop 
      ;PRINT, 'Counter', counter
      ;PRINT, 'DAY', day
      ; Number of rows in Excel = 1048576L
      PRINT, nYP; rowS
      pointer1 = nYP * counter; 1st line to read in 
      pointer2 = nYP * (counter - 20L)
      pointer3 = nYP * (counter - 40L)
      pointer4 = nYP * (counter - 60L)
      pointer5 = nYP * (counter - 80L)
      pointer6 = nYP * (counter - 100L)
      pointer7 = nYP * (counter - 120L)
      pointer8 = nYP * (counter - 140L)
      ;iDay = STRING(iDay)
      ;iHour = STRING(iHour)
      data = YP
      Time='_Day_'
