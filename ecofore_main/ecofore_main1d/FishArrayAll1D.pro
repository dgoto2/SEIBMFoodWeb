FUNCTION FishArrayAll1D, YP, EMS, RAS, ROG, WAE, nYP, nEMS, nRAS, nROG, nWAE, nGridcell
;
;***********TEST ONLY****************************************************************************************
;PRO FishArray
;ihour = 15L; 
...
;  ENDIF
  MultiSI=where(EMS[14, ID] EQ EMS[14, *], MultiSIcount, complement=SingleSI, ncomplement=SingleSIcount)
  ;fishprey2=where(YP[1, ID] gt YP[14, MultiSI], fishprey2count, complement=nonfishprey2, ncomplement=nonfishprey2count)  
  IF MultiSIcount GT 1. THEN BEGIN
    ;print, MultiSI
  ;  if fishprey2count gt 0. then begin
      FISHALL[1, EMS[14, ID]] = MIN(EMS[1, MultiSI]); LENGTH
      FISHALL[2, EMS[14, ID]] = MIN(EMS[2, MultiSI]); WEIGHT
      ;PRINT, TRANSPOSE(YP[14, MultiSI])
      ;PRINT, TRANSPOSE(YP[1, MultiSI]); fish length
      ;PRINT, TRANSPOSE(YP[2, MultiSI]); fish weight
   ; endif
...
;  ENDIF
  MultiSI=where(RAS[14, ID] EQ RAS[14, *], MultiSIcount, complement=SingleSI, ncomplement=SingleSIcount)
  ;fishprey2=where(YP[1, ID] gt YP[14, MultiSI], fishprey2count, complement=nonfishprey2, ncomplement=nonfishprey2count)  
  IF MultiSIcount GT 1. THEN BEGIN
    ;print, MultiSI
  ;  if fishprey2count gt 0. then begin
      FISHALL[1, RAS[14, ID]] = MIN(RAS[1, MultiSI]); LENGTH
      FISHALL[2, RAS[14, ID]] = MIN(RAS[2, MultiSI]); WEIGHT
      ;PRINT, TRANSPOSE(YP[14, MultiSI])
      ;PRINT, TRANSPOSE(YP[1, MultiSI]); fish length
      ;PRINT, TRANSPOSE(YP[2, MultiSI]); fish weight
   ; endif
...
;  ENDIF
  MultiSI=where(ROG[14, ID] EQ ROG[14, *], MultiSIcount, complement=SingleSI, ncomplement=SingleSIcount)
  ;fishprey2=where(YP[1, ID] gt YP[14, MultiSI], fishprey2count, complement=nonfishprey2, ncomplement=nonfishprey2count)  
  IF MultiSIcount GT 1. THEN BEGIN
    ;print, MultiSI
  ;  if fishprey2count gt 0. then begin
      FISHALL[1, ROG[14, ID]] = MIN(ROG[1, MultiSI]); LENGTH
      FISHALL[2, ROG[14, ID]] = MIN(ROG[2, MultiSI]); WEIGHT
      ;PRINT, TRANSPOSE(YP[14, MultiSI])
      ;PRINT, TRANSPOSE(YP[1, MultiSI]); fish length
      ;PRINT, TRANSPOSE(YP[2, MultiSI]); fish weight
   ; endif
...
;  ENDIF
  MultiSI=where(WAE[14, ID] EQ WAE[14, *], MultiSIcount, complement=SingleSI, ncomplement=SingleSIcount)
  ;fishprey2=where(YP[1, ID] gt YP[14, MultiSI], fishprey2count, complement=nonfishprey2, ncomplement=nonfishprey2count)  
  IF MultiSIcount GT 1. THEN BEGIN
    ;print, MultiSI
  ;  if fishprey2count gt 0. then begin
      FISHALL[1, WAE[14, ID]] = MIN(WAE[1, MultiSI]); LENGTH
      FISHALL[2, WAE[14, ID]] = MIN(WAE[2, MultiSI]); WEIGHT
      ;PRINT, TRANSPOSE(YP[14, MultiSI])
      ;PRINT, TRANSPOSE(YP[1, MultiSI]); fish length
      ;PRINT, TRANSPOSE(YP[2, MultiSI]); fish weight
   ; endif
