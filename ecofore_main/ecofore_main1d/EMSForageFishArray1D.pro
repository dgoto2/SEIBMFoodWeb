FUNCTION EMSForageFishArray1D, YP, EMS, RAS, ROG, WAE, nEMS, nGridcell
; Creat a fish array for potential COMPETITORS

PRINT, 'EMSFISHCOMARRAY BEGINS HERE'
...
FOR ID = 0L, nGridcell-1L DO BEGIN
  YEPmultiSI = WHERE((EMS[14, *] EQ ID) AND (YP[14, *] EQ ID), YEPmultiSIcount); 
  ...
  ;      
      ; DETERMINE TOTAL PREY FISH ABUNDANCE AND BIOMASS IN EACH CELL
        ...
        FISHCOMARRAY[4, ID] = TOTAL(YP[0, YEPmultiSI[YEPmultiSIprey]]); ABUNDANCE
        FISHCOMARRAY[5, ID] = TOTAL(YP[2, YEPmultiSI[YEPmultiSIprey]] * YP[0, YEPmultiSI[YEPmultiSIprey]]); BIOMASS
      ENDIF
    ENDIF 
ENDFOR
...
; RAINBOW SMELT
FOR ID = 0L, nGridcell-1L DO BEGIN
  RASmultiSI = WHERE((EMS[14, *] EQ ID) AND (RAS[14, *] EQ ID), RASmultiSIcount); 
...
; walleye
FOR ID = 0L, nGridcell-1L DO BEGIN
  WAEmultiSI = WHERE((EMS[14, *] EQ ID) AND (WAE[14, *] EQ ID), WAEmultiSIcount); 
  ...
  IF WAEmultiSIcount GT 0. THEN BEGIN
    ...
    IF WAEmultiSIpreycount GT 0 THEN BEGIN
      ; RANDOMLY CHOOSE 1 SI FOR FORAGING
      m = 1
      n = WAEmultiSIpreycount; multiSIpreycount
      ;im = findgen(n)+1 ; input array
...
PRINT, 'EMSFISHCOMARRAY ENDS HERE'
RETURN, FISHCOMARRAY
END