FUNCTION YEPFishPreyArray1DMove, YP, EMS, RAS, ROG, WAE, nGridcell
; Creat a fish array for potential prey

PRINT, 'YEPFishPreyArray1DMove BEGINS HERE'
...
FOR ID = 0L, nGridcell-1L DO BEGIN
  ...
  IF YEPmultiSIcount GT 0. THEN BEGIN
    ...
    YEPmultiSIprey = WHERE(YP[6, YEPmultiSI] LT 1, YEPmultiSIpreycount)
...
;    multiSIprey = WHERE(ROG[6, multiSI] LE 1., multiSIpreycount);
;    ;IF multiSIprey2count gt 0 THEN print, multiSI[multiSIprey2]
;    
...
;      FISHCOMARRAY[19, ID] = ROG[1, multiSI[multiSIprey]]; LENGTH
;      FISHCOMARRAY[20, ID] = ROG[2, multiSI[multiSIprey]]; WEIGHT
;      FISHCOMARRAY[21, multiSI[ID2]] = ROG[2, PreyFishID] * ROG[0, PreyFishID]; BIOMASS
;      
;    ; DETERMINE TOTAL PREY FISH ABUNDANCE AND BIOMASS IN EACH CELL
;      FISHCellIDcount[3, ID] = n_elements(multiSI[multiSIprey]); NUMBER OF SIs
;      FISHCOMARRAY[22, ID] = TOTAL(ROG[0, multiSI[multiSIprey]]); ABUNDANCE
;      FISHCOMARRAY[23, ID] = TOTAL(ROG[2, multiSI[multiSIprey]] * ROG[0, multiSI[multiSIprey]]); BIOMASS
;    ENDIF
;  ENDFOR
; ENDIF
;ENDFOR
...
PRINT, 'YEPFishPreyArray1DMove ENDS HERE'
RETURN, FISHPREY
END