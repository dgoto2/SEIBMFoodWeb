FUNCTION YEPFishDeadPreyArray1D, YEPenc, YP, nYEP, nGridcell
; Creat a fish prey array eaten by yellow perch

PRINT, 'YEPFishDeadPreyArray1D BEGINS HERE'
...
; PRINT, WAEmultiSI
  ...
  IF YEPmultiSIcount GT 0. THEN BEGIN
    YEPmultiSIprey = WHERE(YEPenc[45, YEPmultiSI] GT 0., YEPmultiSIpreycount);
...
; EMERALD SHINER
FOR ID = 0L, nGridcell-1L DO BEGIN
    YEPmultiSI = WHERE((YP[14, *] EQ ID), YEPultiSIcount); 

    IF YEPmultiSIcount GT 0. THEN BEGIN
    EMSmultiSIprey = WHERE(YEPenc[46, YEPmultiSI] GT 0., EMSmultiSIpreycount);
...
;        FISHPREY[9, ID] = EMS[2, PreyFishID] * EMS[0, PreyFishID]; BIOMASS
      ...
      ; DETERMINE TOTAL PREY FISH ABUNDANCE AND BIOMASS IN EACH CELL
        FISHCellIDcount[1, ID] = n_elements(YEPmultiSI[EMSmultiSIprey]); NUMBER OF SIs
        FISHPREY[10, ID] = ROUND(TOTAL(YEPenc[46, YEPmultiSI[EMSmultiSIprey]])/n_elements(YEPmultiSI[EMSmultiSIprey])); ABUNDANCE
...
FOR ID = 0L, nGridcell-1L DO BEGIN
  ...
  IF YEPmultiSIcount GT 0. THEN BEGIN
    RASmultiSIprey = WHERE(YEPenc[47, YEPmultiSI] GT 0., RASmultiSIpreycount);
...
;      FISHPREY[15, ID] = RAS[2, PreyFishID] * RAS[0, PreyFishID]; BIOMASS
    ...
    ; DETERMINE TOTAL PREY FISH ABUNDANCE AND BIOMASS IN EACH CELL
      FISHCellIDcount[2, ID] = n_elements(YEPmultiSI[RASmultiSIprey]); NUMBER OF SIs
      FISHPREY[16, ID] = ROUND(TOTAL(YEPenc[47, YEPmultiSI[RASmultiSIprey]])/n_elements(YEPmultiSI[RASmultiSIprey])); ABUNDANCE
...
;    
;    IF YEPmultiSIcount GT 0. THEN BEGIN
;    WAEmultiSIprey = WHERE((YEPenc[49, YEPmultiSI] GT 0.), WAEmultiSIpreycount);
;    
...
;        
;      ; DETERMINE TOTAL PREY FISH ABUNDANCE AND BIOMASS IN EACH CELL
;        FISHCellIDcount[4, ID] = n_elements(YEPmultiSI[WAEmultiSIprey]); NUMBER OF SIs
;        FISHPREY[28, ID] = ROUND(TOTAL(YEPenc[49, YEPmultiSI[WAEmultiSIprey]])/n_elements(YEPmultiSI[WAEmultiSIprey])); ABUNDANCE
...
PRINT, 'YEPFishDeadPreyArray1D ENDS HERE'
RETURN, FISHPREY
END