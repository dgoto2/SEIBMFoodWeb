;FUNCTION WAEmoveV, WAE, nWAE, newinput, Grid3D, Grid2D, DOacclim, NewGridID2
;function determines movement in X,Y,Z direction for all WALLEYE

;----------------------TEST ONLY---------------------------------------------------------------------------
PRO WAEmoveV, WA, nWAE, newinput, Grid3D, Grid2D, DOacclim, NewGridID2
; NEED to change NewInputFiles, YPinitial, YEPacclT, YEPacclDO for testing
nWAE = 50L; number of SI WAE
ihour = 2L
WAE = WAEinitial()
...
;--Vertical movement---------------------------------------------------------------------------------------------------------------------------------
NewCell = FLTARR(15, nWAE);
NewLoc = FLTARR(nWAE)
Ncell = FLTARR(15, nWAE)
d = FLTARR(nWAE)
...
FOR evvv = 0L, nWAE - 1L DO BEGIN
  FOR ev = 0L, nWAE - 1L DO BEGIN;evv + 1L 
...
LengthV = FLTARR(nWAE*20L)
;depth = indgen(20); 1 = top layer; 20 = bottom layer
j = 19L
FOR ii = 0L, nWAE*20L - 1L DO BEGIN
FOR iii = 0L, nWAE - 1L DO BEGIN
LengthV[ii : j] = WAE[1, iii]
ii = j
j = j + 20L
ENDFOR
ENDFOR
...
;calculate attack probability using chesson's alpha = capture efficiency
  TOT = FLTARR(nWAE*20L); total number of all prey atacked and captured
  t = FLTARR(m, nWAE*20L); total number of each prey atacked and captured
      t[0,*] = (Calpha[0,*] * dens[0,*])
      t[1,*] = (Calpha[1,*] * dens[1,*])
      t[2,*] = (Calpha[2,*] * dens[2,*])
      t[3,*] = (Calpha[3,*] * dens[3,*])
...
preyTOT = FLTARR(nWAE*20L)      
jv = 19L
FOR ivvv = 0L, nWAE*20L - 1L DO BEGIN
...
preyTOT2 = FLTARR(nWAE*20L)
j = 19L
FOR iii = 0L, nWAE*20L - 19L DO BEGIN
FOR iiii = 0L, nWAE*20L - 1L DO BEGIN
...
FOR dovvv = 0L, nWAE - 1L DO BEGIN
  FOR dov = 0L, nWAE - 1L DO BEGIN
...
EnvironVMax = FLTARR(nWAE*20L)
EnvironVMax2 = FLTARR(nWAE*20L)
EnvironVMax3 = FLTARR(nWAE*20L)
jvvv = 19L
; Create array with highest habitat quality-based probability for fish
FOR iv = 0L, nWAE*20L - 1L DO BEGIN
...
DepthLoc = FLTARR(nWAE)
DepthLoc2 = FLTARR(nWAE)
jvvvv = 19L
FOR iv = 0L, nWAE*20L - 1L DO BEGIN
  FOR ivv = 0L, nWAE-1L DO BEGIN 
