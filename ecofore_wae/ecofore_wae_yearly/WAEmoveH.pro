;FUNCTION WAEMoveH, WAE, nWAE, newinput, Grid2D, DOacclim
...
;----------------------TEST ONLY---------------------------------------------------------------------------
PRO WAEMoveH, WAE, nWAE, newinput, Grid2D, DOacclim
; NEED to change NewInputFiles, WAEinitial, WAEacclT, WAEacclDO for testing

ts = 360L; 6 minutes
nWAE = 100L; number of SI WAE
ihour = 15L
WAE = WAEinitial()
newinput = newinputfiles()
newinput = newinput[*, 77500L * ihour : 77500L * ihour + 77499L]
length = FLTARR(nWAE); fish length
length = WAE[1, *]; in mm
temp = WAE[19, *]; C
Grid2D = GridCells2D()
DOacclim = WAEacclDO()
xOldLocWithinCell = WAE[39, *]; TEST ONLY 
yOldLocWithinCell = WAE[40, *]; TEST ONLY
...
;****IF THE CELL DOES NOT EXIST WITHIN NEIBORING CELLS, FISH WILL STAY IN THE CURRENT CELL
  IF (LocH[18, ihh] GE 0.0) THEN NewGridXY[0, ihh] = LocH[18, ihh] ELSE NewGridXY[0, ihh] = WAE[13, ihh] 
  IF (LocH[19, ihh] GE 0.0) THEN NewGridXY[1, ihh] = LocH[19, ihh] ELSE NewGridXY[1, ihh] = WAE[13, ihh] 
  IF (LocH[20, ihh] GE 0.0) THEN NewGridXY[2, ihh] = LocH[20, ihh] ELSE NewGridXY[2, ihh] = WAE[13, ihh] 
  IF (LocH[21, ihh] GE 0.0) THEN NewGridXY[3, ihh] = LocH[21, ihh] ELSE NewGridXY[3, ihh] = WAE[13, ihh] 
  IF (LocH[22, ihh] GE 0.0) THEN NewGridXY[4, ihh] = LocH[22, ihh] ELSE NewGridXY[4, ihh] = WAE[13, ihh] 
  IF (LocH[23, ihh] GE 0.0) THEN NewGridXY[5, ihh] = LocH[23, ihh] ELSE NewGridXY[5, ihh] = WAE[13, ihh] 
  IF (LocH[24, ihh] GE 0.0) THEN NewGridXY[6, ihh] = LocH[24, ihh] ELSE NewGridXY[6, ihh] = WAE[13, ihh] 
  IF (LocH[25, ihh] GE 0.0) THEN NewGridXY[7, ihh] = LocH[25, ihh] ELSE NewGridXY[7, ihh] = WAE[13, ihh] 
  IF (LocH[26, ihh] GE 0.0) THEN NewGridXY[8, ihh] = LocH[26, ihh] ELSE NewGridXY[8, ihh] = WAE[13, ihh] 
...
; FOR ihhh = 0L, nYP - 1L DO BEGIN
  LocHV[0, ihh] = WHERE((WAE[12, ihh] EQ NewInput[2, *]) AND (NewGridXY[0, ihh] EQ NewInput[3, *]), NewXYZloc1)
  LocHV[1, ihh] = WHERE((WAE[12, ihh] EQ NewInput[2, *]) AND (NewGridXY[1, ihh] EQ NewInput[3, *]), NewXYZloc2)
  LocHV[2, ihh] = WHERE((WAE[12, ihh] EQ NewInput[2, *]) AND (NewGridXY[2, ihh] EQ NewInput[3, *]), NewXYZloc3)
  LocHV[3, ihh] = WHERE((WAE[12, ihh] EQ NewInput[2, *]) AND (NewGridXY[3, ihh] EQ NewInput[3, *]), NewXYZloc4)
  LocHV[4, ihh] = WHERE((WAE[12, ihh] EQ NewInput[2, *]) AND (NewGridXY[4, ihh] EQ NewInput[3, *]), NewXYZloc5)
  LocHV[5, ihh] = WHERE((WAE[12, ihh] EQ NewInput[2, *]) AND (NewGridXY[5, ihh] EQ NewInput[3, *]), NewXYZloc6)
  LocHV[6, ihh] = WHERE((WAE[12, ihh] EQ NewInput[2, *]) AND (NewGridXY[6, ihh] EQ NewInput[3, *]), NewXYZloc7)
  LocHV[7, ihh] = WHERE((WAE[12, ihh] EQ NewInput[2, *]) AND (NewGridXY[7, ihh] EQ NewInput[3, *]), NewXYZloc8)
  LocHV[8, ihh] = WHERE((WAE[12, ihh] EQ NewInput[2, *]) AND (WAE[13, ihh] EQ NewInput[3, *]), NewXYZloc9)
...
ENDFOR
  ...
  CTO = 25L; Optimal temperture for consumption
...
EnvironHVDO = FLTARR(9, nWAE)
EnvironHVT = FLTARR(9, nWAE)
...
;*************MAY NEED TO CHANGE FOR WALLEYE**********************************
  ...
  IF (pl3count GT 0.0) THEN Calpha[3,*] = ABS(0.50 - 1.75 * (PL[3,*] / WAE[1, *])) $
  ELSE Calpha[3,*] = 0.00 ;for benthic prey for flounder by Rose et al. 1996 
  Length4 = WHERE(WAE[1, *] GT 60.0, L4count, complement = L4c, ncomplement = L4ccount)
  IF (L4count GT 0.0) THEN Calpha[4,*] = 0.001 $; for bythotrephes CHANGE!!! with Rainbow smelt from Barnhisel and Harvey
  ELSE Calpha[4,*] = 0.00
  PL5a = WHERE((PL[5] / WAE[1, *]) LT 0.20, pl5acount, complement = pl5ac, ncomplement = pl5account)
...
; Calculate attack probability using chesson's alpha = capture efficiency
  TOT = FLTARR(9, nWAE); total number of all prey atacked and captured
  t = FLTARR(m*9, nWAE); total number of each prey atacked and captured
  ;t[0,*] = (Calpha[0,*] * dens[0,*])
  t[1,*] = (Calpha[0,*] * dens[1,*])
  ;t[2,*] = (Calpha[0,*] * dens[2,*])
  t[3,*] = (Calpha[0,*] * dens[3,*])
...
;  ;PRINT, 'Swimming speed in Z-dimension (mm/s) =', SS*COS(VerOriAng)
  ...
  MoveSpeed[0, *] = SS * COS(HorOriAng) * RANDOMU(seed, nWAE, /DOUBLE) + newinput[12, WAE[14, *]]; with a random comopnent
  MoveSpeed[1, *] = SS * SIN(HorOriAng) * RANDOMU(seed, nWAE, /DOUBLE) + newinput[13, WAE[14, *]];
  ...
  ;***********************************************************************************
  xOldLocWithinCell = FLTARR(nWAE); TEST ONLY 
  yOldLocWithinCell = FLTARR(nWAE); TEST ONLY
...
IF xyMoveOut1count GT 0.0 THEN WAE[13, xyMoveOut1] = NewGridXY[0, xyMoveOut1]
...
IF xyMoveOut2count GT 0.0 THEN YP[13, xyMoveOut2] = NewGridXY[1, xyMoveOut2]
...
IF xyMoveOut3count GT 0.0 THEN WAE[13, xyMoveOut3] = NewGridXY[2, xyMoveOut3]
...
IF xyMoveOut4count GT 0.0 THEN WAE[13, xyMoveOut4] = NewGridXY[3, xyMoveOut4]
...
IF xyMoveOut5count GT 0.0 THEN WAE[13, xyMoveOut5] = NewGridXY[4, xyMoveOut5]
...
IF xyMoveOut6count GT 0.0 THEN WAE[13, xyMoveOut6] = NewGridXY[5, xyMoveOut6]
...
IF xyMoveOut7count GT 0.0 THEN WAE[13, xyMoveOut7] = NewGridXY[6, xyMoveOut7]
...
IF xyMoveOut8count GT 0.0 THEN WAE[13, xyMoveOut8] = NewGridXY[7, xyMoveOut8]
...
IF xyMoveOut9count GT 0.0 THEN WAE[13, xyMoveOut9] = NewGridXY[8, xyMoveOut9]
...
FishHorMove = FLTARR(4, NWAE)
FishHorMove[0, *] = WAE[13, *]; New horizontal cell ID
