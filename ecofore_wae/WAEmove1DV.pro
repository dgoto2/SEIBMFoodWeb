FUNCTION WAEMove1DV, ts, iHour, WAE, nWAE, NewInput, FISHPREY, FishPred, zOldLocWithinCell, Oxydebt

;function determines movement in Z-direction for walleye

PRINT, 'Walleye 1D Movement Begins Here'
...
; DOWNWARD MOVEMNT
; current layer -3
Lower3Cells0 = WHERE((WAE[14, *] LE 44L), Lower3Cells0count, complement = NONLower3Cells0, ncomplement = NonLower3Cells0count)
...
; current layer -2
Lower2Cells0 = WHERE((WAE[14, *] LE 45L), Lower2Cells0count, complement = NONLower2Cells0, ncomplement = NonLower2Cells0count)
...
; current layer -1
Lower1Cells0 = WHERE((WAE[14, *] LE 46L), Lower1Cells0count, complement = NONLower1Cells0, ncomplement = NonLower1Cells0count)
...
; UPWARD MOVEMENT
; current layer +1
Upper1Cells0 = WHERE((WAE[14, *] GE 1L), Upper1Cells0count, complement = NONUpper1Cells0, ncomplement = NonUpper1Cells0count)
...
; current layer +2
Upper2Cells0 = WHERE((WAE[14, *] GE 2L), Upper2Cells0count, complement = NONUpper2Cells0, ncomplement = NonUpper2Cells0count)
...
; current layer +3
Upper3Cells0 = WHERE((WAE[14, *] GE 3L), Upper3Cells0count, complement = NONUpper3Cells0, ncomplement = NonUpper3Cells0count)
...
Tf = FLTARR(7, nWAE)
; DO***************************************************************************************************************
DOacclim = WAEacclDO(WAE[29, *], WAE[28, *], WAE[20, *], WAE[27, *], WAE[26, *], WAE[19, *], ts, WAE[1, *], WAE[2, *], nWAE, WAE[63, *])
;PRINT, 'DOacclim[7, *]'
...
DOf0b = EnvironV[7, *] - 3.*DOacclim[5, *]
DOf1b = EnvironV[22, *] - 3.*DOacclim[5, *]
DOf2b = EnvironV[37, *] - 3.*DOacclim[5, *]
DOf3b = EnvironV[52, *] - 3.*DOacclim[5, *]
DOf4b = EnvironV[67, *] - 3.*DOacclim[5, *]
DOf5b = EnvironV[82, *] - 3.*DOacclim[5, *]
DOf6b = EnvironV[97, *] - 3.*DOacclim[5, *]
;PRINT, 'DOacclim[7, *]'
;PRINT, TRANSPOSE(5*DOacclim[7, 0:100])
;PRINT, 'DOacclim[5, *]'
;PRINT, TRANSPOSE(3*DOacclim[5, 0:100])
...
IF DOf2count0 GT 0.0 THEN DOf[0, DOf20] = ((EnvironV[7, DOf20] - 5*DOacclim[7, DOf20])/(3.*DOacclim[5, DOf20] - 5*DOacclim[7, DOf20]))
IF DOf2count1 GT 0.0 THEN DOf[1, DOf21] = ((EnvironV[22, DOf21] - 5*DOacclim[7, DOf21])/(3.*DOacclim[5, DOf21] - 5*DOacclim[7, DOf21]))
IF DOf2count2 GT 0.0 THEN DOf[2, DOf22] = ((EnvironV[37, DOf22] - 5*DOacclim[7, DOf22])/(3.*DOacclim[5, DOf22] - 5*DOacclim[7, DOf22]))
IF DOf2count3 GT 0.0 THEN DOf[3, DOf23] = ((EnvironV[52, DOf23] - 5*DOacclim[7, DOf23])/(3.*DOacclim[5, DOf23] - 5*DOacclim[7, DOf23]))
IF DOf2count4 GT 0.0 THEN DOf[4, DOf24] = ((EnvironV[67, DOf24] - 5*DOacclim[7, DOf24])/(3.*DOacclim[5, DOf24] - 5*DOacclim[7, DOf24]))
IF DOf2count5 GT 0.0 THEN DOf[5, DOf25] = ((EnvironV[82, DOf25] - 5*DOacclim[7, DOf25])/(3.*DOacclim[5, DOf25] - 5*DOacclim[7, DOf25]))
IF DOf2count6 GT 0.0 THEN DOf[6, DOf26] = ((EnvironV[97, DOf26] - 5*DOacclim[7, DOf26])/(3.*DOacclim[5, DOf26] - 5*DOacclim[7, DOf26]))
...
; Light*****************************************************************************************************************
litfac = FLTARR(7, nWAE); a multiplication factor to include the effect of light intensity
t1 = FLTARR(7, nWAE)
t2 = FLTARR(7, nWAE)
...
EnvironVDO = FLTARR(7, nWAE)
EnvironVT = FLTARR(7, nWAE)
EnvironVL = FLTARR(7, nWAE)
EnvironVDO[0, *] = DOUBLE(DOf[0, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVDO[1, *] = DOUBLE(DOf[1, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVDO[2, *] = DOUBLE(DOf[2, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVDO[3, *] = DOUBLE(DOf[3, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVDO[4, *] = DOUBLE(DOf[4, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVDO[5, *] = DOUBLE(DOf[5, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVDO[6, *] = DOUBLE(DOf[6, *] * RANDOMU(seed, nWAE, /DOUBLE))
;PRINT, 'Environv[8, *]', EnvironV[8, *]; DO-based habitat index with a random component

EnvironVT[0, *] = DOUBLE(Tf[0, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVT[1, *] = DOUBLE(Tf[1, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVT[2, *] = DOUBLE(Tf[2, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVT[3, *] = DOUBLE(Tf[3, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVT[4, *] = DOUBLE(Tf[4, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVT[5, *] = DOUBLE(Tf[5, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVT[6, *] = DOUBLE(Tf[6, *] * RANDOMU(seed, nWAE, /DOUBLE))
;PRINT, 'Environv[9, *]', EnvironV[9, *]; Temp-based habitat index with a random component

EnvironVL[0, *] = DOUBLE(litfac[0, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVL[1, *] = DOUBLE(litfac[1, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVL[2, *] = DOUBLE(litfac[2, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVL[3, *] = DOUBLE(litfac[3, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVL[4, *] = DOUBLE(litfac[4, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVL[5, *] = DOUBLE(litfac[5, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironVL[6, *] = DOUBLE(litfac[6, *] * RANDOMU(seed, nWAE, /DOUBLE))
;PRINT, 'EnvironVDO'
;PRINT, EnvironVDO[*, 0:100]
;PRINT, 'EnvironVT'
;PRINT, EnvironVT[*, 0:100]
;PRINT, 'EnvironVL'
;PRINT, EnvironVL[*, 0:100]


; Determine predation risk*****************************************************************************************************
; Fish prey density
; Walleye
Preddens = FLTARR(14, nWAE)
...
PredRisk = FLTARR(14, nWAE)
; walleye as a predator
IF (LengthLT200Acount GT 0.0) THEN BEGIN
  PredL1A = WHERE(((WAE[1, LengthLT200A] / FishPred[5, LocV[0, LengthLT200A]]) LT 0.6), PredL1Acount, complement = PredL1Ac, ncomplement = PredL1Account)
  IF (PredL1Acount GT 0.0) THEN PredRisk[0, LengthLT200A[PredL1A]] = 0.9
  IF (PredL1Account GT 0.0) THEN PredRisk[0, LengthLT200A[PredL1Ac]] = 0.00 ; for walleye
ENDIF  
IF (LengthGE200Acount GT 0.0) THEN BEGIN
  PredL1A = WHERE(((WAE[1, LengthGE200A] / FishPred[5, LocV[0, LengthGE200A]]) LT 0.4), PredL1Acount, complement = PredL1Ac, ncomplement = PredL1Account)
...
IF (LengthLT200Bcount GT 0.0) THEN BEGIN
  PredL1B = WHERE(((WAE[1, LengthLT200B] / FishPred[5, LocV[1, LengthLT200B]]) LT 0.6), PredL1Bcount, complement = PredL1Bc, ncomplement = PredL1Bccount)
...
IF (LengthGE200Bcount GT 0.0) THEN BEGIN
  PredL1B = WHERE(((WAE[1, LengthGE200B] / FishPred[5, LocV[1, LengthGE200B]]) LT 0.4), PredL1Bcount, complement = PredL1Bc, ncomplement = PredL1Bccount)
...
IF (LengthLT200Ccount GT 0.0) THEN BEGIN
  PredL1C = WHERE(((WAE[1, LengthLT200C] / FishPred[5, LocV[2, LengthLT200C]]) LT 0.6), PredL1Ccount, complement = PredL1Cc, ncomplement = PredL1Cccount)
...
IF (LengthGE200Ccount GT 0.0) THEN BEGIN
  PredL1C = WHERE(((WAE[1, LengthGE200C] / FishPred[5, LocV[2, LengthGE200C]]) LT 0.4), PredL1Ccount, complement = PredL1Cc, ncomplement = PredL1Cccount)
...
IF (LengthLT200Dcount GT 0.0) THEN BEGIN
  PredL1D = WHERE(((WAE[1, LengthLT200D] / FishPred[5, LocV[3, LengthLT200D]]) LT 0.6), PredL1Dcount, complement = PredL1Dc, ncomplement = PredL1Dccount)
...
IF (LengthGE200Dcount GT 0.0) THEN BEGIN
  PredL1D = WHERE(((WAE[1, LengthGE200D] / FishPred[5, LocV[3, LengthGE200D]]) LT 0.4), PredL1Dcount, complement = PredL1Dc, ncomplement = PredL1Dccount)
...
IF (LengthLT200Ecount GT 0.0) THEN BEGIN
  PredL1E = WHERE(((WAE[1, LengthLT200E] / FishPred[5, LocV[4, LengthLT200E]]) LT 0.6), PredL1Ecount, complement = PredL1Ec, ncomplement = PredL1Eccount)
  IF (PredL1Ecount GT 0.0) THEN PredRisk[4, LengthLT200E[PredL1E]] = 0.9
  IF (PredL1Eccount GT 0.0) THEN PredRisk[4, LengthLT200E[PredL1Ec]] = 0.00 ; 
ENDIF  
IF (LengthGE200Ecount GT 0.0) THEN BEGIN
  PredL1E = WHERE(((WAE[1, LengthGE200E] / FishPred[5, LocV[4, LengthGE200E]]) LT 0.4), PredL1Ecount, complement = PredL1Ec, ncomplement = PredL1Eccount)
  IF (PredL1Ecount GT 0.0) THEN PredRisk[4, LengthGE200E[PredL1E]] = 0.9
  IF (PredL1Eccount GT 0.0) THEN PredRisk[4, LengthGE200E[PredL1Ec]] = 0.00 ; 
ENDIF

IF (LengthLT200Fcount GT 0.0) THEN BEGIN
  PredL1F = WHERE(((WAE[1, LengthLT200F] / FishPred[5, LocV[5, LengthLT200F]]) LT 0.6), PredL1Fcount, complement = PredL1Fc, ncomplement = PredL1Fccount)
  IF (PredL1Fcount GT 0.0) THEN PredRisk[5, LengthLT200F[PredL1F]] = 0.9
  IF (PredL1Fccount GT 0.0) THEN PredRisk[5, LengthLT200F[PredL1Fc]] = 0.00 ; 
ENDIF  
IF (LengthGE200Fcount GT 0.0) THEN BEGIN
  PredL1F = WHERE(((WAE[1, LengthGE200F] / FishPred[5, LocV[5, LengthGE200F]]) LT 0.4), PredL1Fcount, complement = PredL1Fc, ncomplement = PredL1Fccount)
  IF (PredL1Fcount GT 0.0) THEN PredRisk[5, LengthGE200F[PredL1F]] = 0.9
  IF (PredL1Fccount GT 0.0) THEN PredRisk[5, LengthGE200F[PredL1Fc]] = 0.00 ; 
ENDIF

IF (LengthLT200Gcount GT 0.0) THEN BEGIN
  PredL1G = WHERE(((WAE[1, LengthLT200G] / FishPred[5, LocV[6, LengthLT200G]]) LT 0.6), PredL1Gcount, complement = PredL1Gc, ncomplement = PredL1Gccount)
  IF (PredL1Gcount GT 0.0) THEN PredRisk[6, LengthLT200G[PredL1G]] = 0.9
  IF (PredL1Gccount GT 0.0) THEN PredRisk[6, LengthLT200G[PredL1Gc]] = 0.00 ; 
ENDIF  
IF (LengthGE200Gcount GT 0.0) THEN BEGIN
  PredL1G = WHERE(((WAE[1, LengthGE200G] / FishPred[5, LocV[6, LengthGE200G]]) LT 0.4), PredL1Gcount, complement = PredL1Gc, ncomplement = PredL1Gccount)
  IF (PredL1Gcount GT 0.0) THEN PredRisk[6, LengthGE200G[PredL1G]] = 0.9
  IF (PredL1Gccount GT 0.0) THEN PredRisk[6, LengthGE200G[PredL1Gc]] = 0.00 ; 
ENDIF

; Calculate attack probability using chesson's alpha = capture efficiency
TOTpred = FLTARR(7, nWAE); total number of all prey atacked and captured
Predt = FLTARR(2*7, nWAE); total number of each prey atacked and captured
; walleye
...
;; yellow perch
;Predt[7,*] = (PredRisk[7,*] * Preddens[7,*]) * FishPred[26, LocV[0, *]]
;Predt[8,*] = (PredRisk[8,*] * Preddens[8,*]) * FishPred[26, LocV[1, *]]
;Predt[9,*] = (PredRisk[9,*] * Preddens[9,*]) * FishPred[26, LocV[2, *]]
;Predt[10,*] = (PredRisk[10,*] * Preddens[10,*]) * FishPred[26, LocV[3, *]]
;Predt[11,*] = (PredRisk[11,*] * Preddens[11,*]) * FishPred[26, LocV[4, *]]
;Predt[12,*] = (PredRisk[12,*] * Preddens[12,*]) * FishPred[26, LocV[5, *]]
;Predt[13,*] = (PredRisk[13,*] * Preddens[13,*]) * FishPred[26, LocV[6, *]]
...
;PL[6, *] = FISHPREY[7, *]; length for emerald shiner in mm 
;PL[7, *] = FISHPREY[13, *]; length for rainbow smelt in mm 
;PL[8, *] = FISHPREY[19, *]; length for round goby in mm 
;PL[9, *] = FISHPREY[26, *]; length for round goby in mm 
...
Calpha = FLTARR(40, nWAE); from Letcher et al. for zooplankton
Calpha[0, *] = 193499 * WAE[1, *]^(-7.64); for microzooplankton - rotifers

LengthLT100 = WHERE((WAE[1, *] LT 43.0), LengthLT100count, complement = LengthGE100, ncomplement = LengthGE100count)
IF (LengthLT100count GT 0.0) THEN BEGIN
  Calpha[1, LengthLT100] = 0.2 / (1.0 + ((0.4 / 0.09) - 1.0) * EXP(-13.0 * 0.031 * WAE[1, LengthLT100]))^(1.0 / 0.031) ; for mesozooplankton1 - calanoids  
  Calpha[2, LengthLT100] = 0.2 / (1.0 + ((0.4 / 0.09) - 1.0) * EXP(-13.0 * 0.031 * WAE[1, LengthLT100]))^(1.0 / 0.031) ; for mesozooplankton2 - cladocerans
ENDIF
IF (LengthGE100count GT 0.0) THEN BEGIN
  Calpha[1, LengthGE100] = 0.; 1/m / (1.0 + ((0.4 / 0.09) - 1.0) * EXP(-13.0 * 0.031 * WAE[1, LengthGE30]))^(1.0 / 0.031); 193499 * WAE[1, LengthGE43]^(-7.64); for mesozooplankton1 - calanoids  
  Calpha[2, LengthGE100] = 0.; 1/m / (1.0 + ((0.4 / 0.09) - 1.0) * EXP(-13.0 * 0.031 * WAE[1, LengthGE30]))^(1.0 / 0.031); 193499 * WAE[1, LengthGE43]^(-7.64) ; for mesozooplankton2 - cladocerans
ENDIF

PL3 = WHERE((PL[3, *] / WAE[1, *]) LE 0.20, pl3count, complement = pl3c, ncomplement = pl3ccount)
IF (pl3count GT 0.0) THEN Calpha[3, PL3] = ABS(1. - 1.75 * (PL[3, PL3] / WAE[1, PL3]))
...
LengthLT200 = WHERE((WAE[1, *] LT 200.0), LengthLT200count, complement = LengthGE200, ncomplement = LengthGE200count)
; yellow perch  
IF (LengthLT200count GT 0.0) THEN BEGIN
  PL5A = WHERE(((FISHPREY[1, LocV[0, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL5Acount, complement = PL5Ac, ncomplement = PL5Account)
  IF (PL5Acount GT 0.0) THEN Calpha[5, LengthLT200[PL5A]] = 0.9
  IF (PL5Account GT 0.0) THEN Calpha[5, LengthLT200[PL5Ac]] = 0.00 ; for yellow perch
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL5A = WHERE(((FISHPREY[1, LocV[0, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL5Acount, complement = PL5Ac, ncomplement = PL5Account)
  IF (PL5Acount GT 0.0) THEN Calpha[5, LengthGE200[PL5A]] = 0.9
  IF (PL5Account GT 0.0) THEN Calpha[5, LengthGE200[PL5Ac]] = 0.00 ; for yellow perch
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL5B = WHERE(((FISHPREY[1, LocV[1, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL5Bcount, complement = PL5Bc, ncomplement = PL5Bccount)
  IF (PL5Bcount GT 0.0) THEN Calpha[6, LengthLT200[PL5B]] = 0.9
  IF (PL5Bccount GT 0.0) THEN Calpha[6, LengthLT200[PL5Bc]] = 0.00 ; for yellow perch
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL5B = WHERE(((FISHPREY[1, LocV[1, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL5Bcount, complement = PL5Bc, ncomplement = PL5Bccount)
  IF (PL5Bcount GT 0.0) THEN Calpha[6, LengthGE200[PL5B]] = 0.9
  IF (PL5Bccount GT 0.0) THEN Calpha[6, LengthGE200[PL5Bc]] = 0.00 ; for yellow perch
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL5C = WHERE(((FISHPREY[1, LocV[2, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL5Ccount, complement = PL5Cc, ncomplement = PL5Cccount)
  IF (PL5Ccount GT 0.0) THEN Calpha[7, LengthLT200[PL5C]] = 0.9
  IF (PL5Cccount GT 0.0) THEN Calpha[7, LengthLT200[PL5Cc]] = 0.00 ; for yellow perch
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL5C = WHERE(((FISHPREY[1, LocV[2, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL5Ccount, complement = PL5Cc, ncomplement = PL5Cccount)
  IF (PL5Ccount GT 0.0) THEN Calpha[7, LengthGE200[PL5C]] = 0.9
  IF (PL5Cccount GT 0.0) THEN Calpha[7, LengthGE200[PL5Cc]] = 0.00 ; for yellow perch
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL5D = WHERE(((FISHPREY[1, LocV[3, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL5Dcount, complement = PL5Dc, ncomplement = PL5Dccount)
  IF (PL5Dcount GT 0.0) THEN Calpha[8, LengthLT200[PL5D]] = 0.9
  IF (PL5Dccount GT 0.0) THEN Calpha[8, LengthLT200[PL5Dc]] = 0.00 ; for yellow perch
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL5D = WHERE(((FISHPREY[1, LocV[3, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL5Dcount, complement = PL5Dc, ncomplement = PL5Dccount)
  IF (PL5Dcount GT 0.0) THEN Calpha[8, LengthGE200[PL5D]] = 0.9
  IF (PL5Dccount GT 0.0) THEN Calpha[8, LengthGE200[PL5Dc]] = 0.00 ; for yellow perch
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL5E = WHERE(((FISHPREY[1, LocV[4, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL5Ecount, complement = PL5Ec, ncomplement = PL5Eccount)
  IF (PL5Ecount GT 0.0) THEN Calpha[9, LengthLT200[PL5E]] = 0.9
  IF (PL5Eccount GT 0.0) THEN Calpha[9, LengthLT200[PL5Ec]] = 0.00 ; for yellow perch
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL5E = WHERE(((FISHPREY[1, LocV[4, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL5Ecount, complement = PL5Ec, ncomplement = PL5Eccount)
  IF (PL5Ecount GT 0.0) THEN Calpha[9, LengthGE200[PL5E]] = 0.9
  IF (PL5Eccount GT 0.0) THEN Calpha[9, LengthGE200[PL5Ec]] = 0.00 ; for yellow perch
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL5F = WHERE(((FISHPREY[1, LocV[5, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL5Fcount, complement = PL5Fc, ncomplement = PL5Fccount)
  IF (PL5Fcount GT 0.0) THEN Calpha[10, LengthLT200[PL5F]] = 0.9
  IF (PL5Fccount GT 0.0) THEN Calpha[10, LengthLT200[PL5Fc]] = 0.00 ; for yellow perch
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL5F = WHERE(((FISHPREY[1, LocV[5, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL5Fcount, complement = PL5Fc, ncomplement = PL5Fccount)
  IF (PL5Fcount GT 0.0) THEN Calpha[10, LengthGE200[PL5F]] = 0.9
  IF (PL5Fccount GT 0.0) THEN Calpha[10, LengthGE200[PL5Fc]] = 0.00 ; for yellow perch
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL5G = WHERE(((FISHPREY[1, LocV[6, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL5Gcount, complement = PL5Gc, ncomplement = PL5Gccount)
  IF (PL5Gcount GT 0.0) THEN Calpha[11, LengthLT200[PL5G]] = 0.9
  IF (PL5Gccount GT 0.0) THEN Calpha[11, LengthLT200[PL5Gc]] = 0.00 ; for yellow perch
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL5G = WHERE(((FISHPREY[1, LocV[6, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL5Gcount, complement = PL5Gc, ncomplement = PL5Gccount)
  IF (PL5Gcount GT 0.0) THEN Calpha[11, LengthGE200[PL5G]] = 0.9
  IF (PL5Gccount GT 0.0) THEN Calpha[11, LengthGE200[PL5Gc]] = 0.00 ; for yellow perch
ENDIF

; emerald shiner
IF (LengthLT200count GT 0.0) THEN BEGIN
  PL6A = WHERE(((FISHPREY[7, LocV[0, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL6Acount, complement = PL6Ac, ncomplement = PL6Account)
  IF (PL6Acount GT 0.0) THEN Calpha[12, LengthLT200[PL6A]] = 0.9
  IF (PL6Account GT 0.0) THEN Calpha[12, LengthLT200[PL6Ac]] = 0.00 ; for emerald shiner
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL6A = WHERE(((FISHPREY[7, LocV[0, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL6Acount, complement = PL6Ac, ncomplement = PL6Account)
  IF (PL6Acount GT 0.0) THEN Calpha[12, LengthGE200[PL6A]] = 0.9
  IF (PL6Account GT 0.0) THEN Calpha[12, LengthGE200[PL6Ac]] = 0.00 ; for emerald shiner
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL6B = WHERE(((FISHPREY[7, LocV[1, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL6Bcount, complement = PL6Bc, ncomplement = PL6Bccount)
  IF (PL6Bcount GT 0.0) THEN Calpha[13, LengthLT200[PL6B]] = 0.9
  IF (PL6Bccount GT 0.0) THEN Calpha[13, LengthLT200[PL6Bc]] = 0.00 ; for emerald shiner
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL6B = WHERE(((FISHPREY[7, LocV[1, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL6Bcount, complement = PL6Bc, ncomplement = PL6Bccount)
  IF (PL6Bcount GT 0.0) THEN Calpha[13, LengthGE200[PL6B]] = 0.9
  IF (PL6Bccount GT 0.0) THEN Calpha[13, LengthGE200[PL6Bc]] = 0.00 ; for emerald shiner
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL6C = WHERE(((FISHPREY[7, LocV[2, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL6Ccount, complement = PL6Cc, ncomplement = PL6Cccount)
  IF (PL6Ccount GT 0.0) THEN Calpha[14, LengthLT200[PL6C]] = 0.9
  IF (PL6Cccount GT 0.0) THEN Calpha[14, LengthLT200[PL6Cc]] = 0.00 ; for emerald shiner
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL6C = WHERE(((FISHPREY[7, LocV[2, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL6Ccount, complement = PL6Cc, ncomplement = PL6Cccount)
  IF (PL6Ccount GT 0.0) THEN Calpha[14, LengthGE200[PL6C]] = 0.9
  IF (PL6Cccount GT 0.0) THEN Calpha[14, LengthGE200[PL6Cc]] = 0.00 ; for emerald shiner
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL6D = WHERE(((FISHPREY[7, LocV[3, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL6Dcount, complement = PL6Dc, ncomplement = PL6Dccount)
  IF (PL6Dcount GT 0.0) THEN Calpha[15, LengthLT200[PL6D]] = 0.9
  IF (PL6Dccount GT 0.0) THEN Calpha[15, LengthLT200[PL6Dc]] = 0.00 ; for emerald shiner
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL6D = WHERE(((FISHPREY[7, LocV[3, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL6Dcount, complement = PL6Dc, ncomplement = PL6Dccount)
  IF (PL6Dcount GT 0.0) THEN Calpha[15, LengthGE200[PL6D]] = 0.9
  IF (PL6Dccount GT 0.0) THEN Calpha[15, LengthGE200[PL6Dc]] = 0.00 ; for emerald shiner
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL6E = WHERE(((FISHPREY[7, LocV[4, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL6Ecount, complement = PL6Ec, ncomplement = PL6Eccount)
  IF (PL6Ecount GT 0.0) THEN Calpha[16, LengthLT200[PL6E]] = 0.9
  IF (PL6Eccount GT 0.0) THEN Calpha[16, LengthLT200[PL6Ec]] = 0.00 ; for emerald shiner
ENDIF
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL6E = WHERE(((FISHPREY[7, LocV[4, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL6Ecount, complement = PL6Ec, ncomplement = PL6Eccount)
  IF (PL6Ecount GT 0.0) THEN Calpha[16, LengthGE200[PL6E]] = 0.9
  IF (PL6Eccount GT 0.0) THEN Calpha[16, LengthGE200[PL6Ec]] = 0.00 ; for emerald shiner
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL6F = WHERE(((FISHPREY[7, LocV[5, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL6Fcount, complement = PL6Fc, ncomplement = PL6Fccount)
  IF (PL6Fcount GT 0.0) THEN Calpha[17, LengthLT200[PL6F]] = 0.9
  IF (PL6Fccount GT 0.0) THEN Calpha[17, LengthLT200[PL6Fc]] = 0.00 ; for emerald shiner
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL6F = WHERE(((FISHPREY[7, LocV[5, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL6Fcount, complement = PL6Fc, ncomplement = PL6Fccount)
  IF (PL6Fcount GT 0.0) THEN Calpha[17, LengthGE200[PL6F]] = 0.9
  IF (PL6Fccount GT 0.0) THEN Calpha[17, LengthGE200[PL6Fc]] = 0.00 ; for emerald shiner
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL6G = WHERE(((FISHPREY[7, LocV[6, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL6Gcount, complement = PL6Gc, ncomplement = PL6Gccount)
  IF (PL6Gcount GT 0.0) THEN Calpha[18, LengthLT200[PL6G]] = 0.9
  IF (PL6Gccount GT 0.0) THEN Calpha[18, LengthLT200[PL6Gc]] = 0.00 ; for emerald shiner
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL6G = WHERE(((FISHPREY[7, LocV[6, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL6Gcount, complement = PL6Gc, ncomplement = PL6Gccount)
  IF (PL6Gcount GT 0.0) THEN Calpha[18, LengthGE200[PL6G]] = 0.9
  IF (PL6Gccount GT 0.0) THEN Calpha[18, LengthGE200[PL6Gc]] = 0.00 ; for emerald shiner
ENDIF

; rainbow smelt
IF (LengthLT200count GT 0.0) THEN BEGIN
  PL7A = WHERE(((FISHPREY[13, LocV[0, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL7Acount, complement = PL7Ac, ncomplement = PL7Account)
  IF (PL7Acount GT 0.0) THEN Calpha[19, LengthLT200[PL7A]] = 0.9
  IF (PL7Account GT 0.0) THEN Calpha[19, LengthLT200[PL7Ac]] = 0.00 ; for rainbow smelt
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL7A = WHERE(((FISHPREY[13, LocV[0, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL7Acount, complement = PL7Ac, ncomplement = PL7Account)
  IF (PL7Acount GT 0.0) THEN Calpha[19, LengthGE200[PL7A]] = 0.9
  IF (PL7Account GT 0.0) THEN Calpha[19, LengthGE200[PL7Ac]] = 0.00 ; for rainbow smelt
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL7B = WHERE(((FISHPREY[13, LocV[1, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL7Bcount, complement = PL7Bc, ncomplement = PL7Bccount)
  IF (PL7Bcount GT 0.0) THEN Calpha[20, LengthLT200[PL7B]] = 0.9
  IF (PL7Bccount GT 0.0) THEN Calpha[20, LengthLT200[PL7Bc]] = 0.00 ; for rainbow smelt
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL7B = WHERE(((FISHPREY[13, LocV[1, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL7Bcount, complement = PL7Bc, ncomplement = PL7Bccount)
  IF (PL7Bcount GT 0.0) THEN Calpha[20, LengthGE200[PL7B]] = 0.9
  IF (PL7Bccount GT 0.0) THEN Calpha[20, LengthGE200[PL7Bc]] = 0.00 ; for rainbow smelt
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL7C = WHERE(((FISHPREY[13, LocV[2, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL7Ccount, complement = PL7Cc, ncomplement = PL7Cccount)
  IF (PL7Ccount GT 0.0) THEN Calpha[21, LengthLT200[PL7C]] = 0.9
  IF (PL7Cccount GT 0.0) THEN Calpha[21, LengthLT200[PL7Cc]] = 0.00 ; for rainbow smelt
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL7C = WHERE(((FISHPREY[13, LocV[2, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL7Ccount, complement = PL7Cc, ncomplement = PL7Cccount)
  IF (PL7Ccount GT 0.0) THEN Calpha[21, LengthGE200[PL7C]] = 0.9
  IF (PL7Cccount GT 0.0) THEN Calpha[21, LengthGE200[PL7Cc]] = 0.00 ; for rainbow smelt
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL7D = WHERE(((FISHPREY[13, LocV[3, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL7Dcount, complement = PL7Dc, ncomplement = PL7Dccount)
  IF (PL7Dcount GT 0.0) THEN Calpha[22, LengthLT200[PL7D]] = 0.9
  IF (PL7Dccount GT 0.0) THEN Calpha[22, LengthLT200[PL7Dc]] = 0.00 ; for rainbow smelt
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL7D = WHERE(((FISHPREY[13, LocV[3, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL7Dcount, complement = PL7Dc, ncomplement = PL7Dccount)
  IF (PL7Dcount GT 0.0) THEN Calpha[22, LengthGE200[PL7D]] = 0.9
  IF (PL7Dccount GT 0.0) THEN Calpha[22, LengthGE200[PL7Dc]] = 0.00 ; for rainbow smelt
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL7E = WHERE(((FISHPREY[13, LocV[4, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL7Ecount, complement = PL7Ec, ncomplement = PL7Eccount)
  IF (PL7Ecount GT 0.0) THEN Calpha[23, LengthLT200[PL7E]] = 0.9
  IF (PL7Eccount GT 0.0) THEN Calpha[23, LengthLT200[PL7Ec]] = 0.00 ; for rainbow smelt
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL7E = WHERE(((FISHPREY[13, LocV[4, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL7Ecount, complement = PL7Ec, ncomplement = PL7Eccount)
  IF (PL7Ecount GT 0.0) THEN Calpha[23, LengthGE200[PL7E]] = 0.9
  IF (PL7Eccount GT 0.0) THEN Calpha[23, LengthGE200[PL7Ec]] = 0.00 ; for rainbow smelt
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL7F = WHERE(((FISHPREY[13, LocV[5, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL7Fcount, complement = PL7Fc, ncomplement = PL7Fccount)
  IF (PL7Fcount GT 0.0) THEN Calpha[24, LengthLT200[PL7F]] = 0.9
  IF (PL7Fccount GT 0.0) THEN Calpha[24, LengthLT200[PL7Fc]] = 0.00 ; for rainbow smelt
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL7F = WHERE(((FISHPREY[13, LocV[5, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL7Fcount, complement = PL7Fc, ncomplement = PL7Fccount)
  IF (PL7Fcount GT 0.0) THEN Calpha[24, LengthGE200[PL7F]] = 0.9
  IF (PL7Fccount GT 0.0) THEN Calpha[24, LengthGE200[PL7Fc]] = 0.00 ; for rainbow smelt
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL7G = WHERE(((FISHPREY[13, LocV[6, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL7Gcount, complement = PL7Gc, ncomplement = PL7Gccount)
  IF (PL7Gcount GT 0.0) THEN Calpha[25, LengthLT200[PL7A]] = 0.9
  IF (PL7Gccount GT 0.0) THEN Calpha[25, LengthLT200[PL7Ac]] = 0.00 ; for rainbow smelt
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL7G = WHERE(((FISHPREY[13, LocV[6, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL7Gcount, complement = PL7Gc, ncomplement = PL7Gccount)
  IF (PL7Gcount GT 0.0) THEN Calpha[25, LengthGE200[PL7G]] = 0.9
  IF (PL7Gccount GT 0.0) THEN Calpha[25, LengthGE200[PL7Gc]] = 0.00 ; for rainbow smelt
ENDIF


;IF (LengthLT200count GT 0.0) THEN BEGIN
;  PL8A = WHERE(((FISHPREY[19, LocV[0, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL8Acount, complement = PL8Ac, ncomplement = PL8Account)
;  IF (PL8Acount GT 0.0) THEN Calpha[26, LengthLT200[PL8A]] = 0.9
;  IF (PL8Account GT 0.0) THEN Calpha[26, LengthLT200[PL8Ac]] = 0.00 ; for round goby
;ENDIF  
;IF (LengthGE200count GT 0.0) THEN BEGIN
;  PL8A = WHERE(((FISHPREY[19, LocV[0, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL8Acount, complement = PL8Ac, ncomplement = PL8Account)
;  IF (PL8Acount GT 0.0) THEN Calpha[26, LengthGE200[PL8A]] = 0.9
;  IF (PL8Account GT 0.0) THEN Calpha[26, LengthGE200[PL8Ac]] = 0.00 ; for round goby
;ENDIF
;
;IF (LengthLT200count GT 0.0) THEN BEGIN
;  PL8B = WHERE(((FISHPREY[19, LocV[1, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL8Bcount, complement = PL8Bc, ncomplement = PL8Bccount)
;  IF (PL8Bcount GT 0.0) THEN Calpha[27, LengthLT200[PL8B]] = 0.9
;  IF (PL8Bccount GT 0.0) THEN Calpha[27, LengthLT200[PL8Bc]] = 0.00 ; for round goby
;ENDIF  
;IF (LengthGE200count GT 0.0) THEN BEGIN
;  PL8B = WHERE(((FISHPREY[19, LocV[1, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL8Bcount, complement = PL8Bc, ncomplement = PL8Bccount)
;  IF (PL8Bcount GT 0.0) THEN Calpha[27, LengthGE200[PL8B]] = 0.9
;  IF (PL8Bccount GT 0.0) THEN Calpha[27, LengthGE200[PL8Bc]] = 0.00 ; for round goby
;ENDIF
;
;IF (LengthLT200count GT 0.0) THEN BEGIN
;  PL8C = WHERE(((FISHPREY[19, LocV[2, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL8Ccount, complement = PL8Cc, ncomplement = PL8Cccount)
;  IF (PL8Ccount GT 0.0) THEN Calpha[28, LengthLT200[PL8C]] = 0.9
;  IF (PL8Cccount GT 0.0) THEN Calpha[28, LengthLT200[PL8Cc]] = 0.00 ; for round goby
;ENDIF  
;IF (LengthGE200count GT 0.0) THEN BEGIN
;  PL8C = WHERE(((FISHPREY[19, LocV[2, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL8Ccount, complement = PL8Cc, ncomplement = PL8Cccount)
;  IF (PL8Ccount GT 0.0) THEN Calpha[28, LengthGE200[PL8C]] = 0.9
;  IF (PL8Cccount GT 0.0) THEN Calpha[28, LengthGE200[PL8Cc]] = 0.00 ; for round goby
;ENDIF
;
;IF (LengthLT200count GT 0.0) THEN BEGIN
;  PL8D = WHERE(((FISHPREY[19, LocV[3, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL8Dcount, complement = PL8Dc, ncomplement = PL8Dccount)
;  IF (PL8Dcount GT 0.0) THEN Calpha[29, LengthLT200[PL8D]] = 0.9
;  IF (PL8Dccount GT 0.0) THEN Calpha[29, LengthLT200[PL8Dc]] = 0.00 ; for round goby
;ENDIF  
;IF (LengthGE200count GT 0.0) THEN BEGIN
;  PL8D = WHERE(((FISHPREY[19, LocV[3, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL8Dcount, complement = PL8Dc, ncomplement = PL8Dccount)
;  IF (PL8Dcount GT 0.0) THEN Calpha[29, LengthGE200[PL8D]] = 0.9
;  IF (PL8Dccount GT 0.0) THEN Calpha[29, LengthGE200[PL8Dc]] = 0.00 ; for round goby
;ENDIF
;
;IF (LengthLT200count GT 0.0) THEN BEGIN
;  PL8E = WHERE(((FISHPREY[19, LocV[4, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL8Ecount, complement = PL8Ec, ncomplement = PL8Eccount)
;  IF (PL8Ecount GT 0.0) THEN Calpha[30, LengthLT200[PL8E]] = 0.9
;  IF (PL8Eccount GT 0.0) THEN Calpha[30, LengthLT200[PL8Ec]] = 0.00 ; for round goby
;ENDIF  
;IF (LengthGE200count GT 0.0) THEN BEGIN
;  PL8E = WHERE(((FISHPREY[19, LocV[4, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL8Ecount, complement = PL8Ec, ncomplement = PL8Eccount)
;  IF (PL8Ecount GT 0.0) THEN Calpha[30, LengthGE200[PL8E]] = 0.9
;  IF (PL8Eccount GT 0.0) THEN Calpha[30, LengthGE200[PL8Ec]] = 0.00 ; for round goby
;ENDIF

;IF (LengthLT200count GT 0.0) THEN BEGIN
;  PL8F = WHERE(((FISHPREY[19, LocV[5, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL8Fcount, complement = PL8Fc, ncomplement = PL8Fccount)
;  IF (PL8Fcount GT 0.0) THEN Calpha[31, LengthLT200[PL8F]] = 0.9
;  IF (PL8Fccount GT 0.0) THEN Calpha[31, LengthLT200[PL8Fc]] = 0.00 ; for round goby
;ENDIF  
;IF (LengthGE200count GT 0.0) THEN BEGIN
;  PL8F = WHERE(((FISHPREY[19, LocV[5, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL8Fcount, complement = PL8Fc, ncomplement = PL8Fccount)
;  IF (PL8Fcount GT 0.0) THEN Calpha[31, LengthGE200[PL8F]] = 0.9
;  IF (PL8Fccount GT 0.0) THEN Calpha[31, LengthGE200[PL8Fc]] = 0.00 ; for round goby
;ENDIF
;
;IF (LengthLT200count GT 0.0) THEN BEGIN
;  PL8G = WHERE(((FISHPREY[19, LocV[6, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL8Gcount, complement = PL8Gc, ncomplement = PL8Gccount)
;  IF (PL8Gcount GT 0.0) THEN Calpha[32, LengthLT200[PL8G]] = 0.9
;  IF (PL8Gccount GT 0.0) THEN Calpha[32, LengthLT200[PL8Gc]] = 0.00 ; for round goby
;ENDIF  
;IF (LengthGE200count GT 0.0) THEN BEGIN
;  PL8G = WHERE(((FISHPREY[19, LocV[6, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL8Gcount, complement = PL8Gc, ncomplement = PL8Gccount)
;  IF (PL8Gcount GT 0.0) THEN Calpha[32, LengthGE200[PL8G]] = 0.9
;  IF (PL8Gccount GT 0.0) THEN Calpha[32, LengthGE200[PL8Gc]] = 0.00 ; for round goby
;ENDIF


IF (LengthLT200count GT 0.0) THEN BEGIN
  PL9A = WHERE(((FISHPREY[25, LocV[0, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL9Acount, complement = PL9Ac, ncomplement = PL9Account)
  IF (PL9Acount GT 0.0) THEN Calpha[33, LengthLT200[PL9A]] = 0.9
  IF (PL9Account GT 0.0) THEN Calpha[33, LengthLT200[PL9Ac]] = 0.00 ; for walleye
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL9A = WHERE(((FISHPREY[25, LocV[0, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL9Acount, complement = PL9Ac, ncomplement = PL9Account)
  IF (PL9Acount GT 0.0) THEN Calpha[33, LengthGE200[PL9A]] = 0.9
  IF (PL9Account GT 0.0) THEN Calpha[33, LengthGE200[PL9Ac]] = 0.00 ; for walleye
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL9B = WHERE(((FISHPREY[25, LocV[1, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL9Bcount, complement = PL9Bc, ncomplement = PL9Bccount)
  IF (PL9Bcount GT 0.0) THEN Calpha[34, LengthLT200[PL9B]] = 0.9
  IF (PL9Bccount GT 0.0) THEN Calpha[34, LengthLT200[PL9Bc]] = 0.00 ; for walleye
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL9B = WHERE(((FISHPREY[25, LocV[1, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL9Bcount, complement = PL9Bc, ncomplement = PL9Bccount)
  IF (PL9Bcount GT 0.0) THEN Calpha[34, LengthGE200[PL9B]] = 0.9
  IF (PL9Bccount GT 0.0) THEN Calpha[34, LengthGE200[PL9Bc]] = 0.00 ; for walleye
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL9C = WHERE(((FISHPREY[25, LocV[2, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL9Ccount, complement = PL9Cc, ncomplement = PL9Cccount)
  IF (PL9Ccount GT 0.0) THEN Calpha[35, LengthLT200[PL9C]] = 0.9
  IF (PL9Cccount GT 0.0) THEN Calpha[35, LengthLT200[PL9Cc]] = 0.00 ; for walleye
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL9C = WHERE(((FISHPREY[25, LocV[2, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL9Ccount, complement = PL9Cc, ncomplement = PL9Cccount)
  IF (PL9Ccount GT 0.0) THEN Calpha[35, LengthGE200[PL9C]] = 0.9
  IF (PL9Cccount GT 0.0) THEN Calpha[35, LengthGE200[PL9Cc]] = 0.00 ; for walleye
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL9D = WHERE(((FISHPREY[25, LocV[3, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL9Dcount, complement = PL9Dc, ncomplement = PL9Dccount)
  IF (PL9Dcount GT 0.0) THEN Calpha[36, LengthLT200[PL9D]] = 0.9
  IF (PL9Dccount GT 0.0) THEN Calpha[36, LengthLT200[PL9Dc]] = 0.00 ; for walleye
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL9D = WHERE(((FISHPREY[25, LocV[3, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL9Dcount, complement = PL9Dc, ncomplement = PL9Dccount)
  IF (PL9Dcount GT 0.0) THEN Calpha[36, LengthGE200[PL9D]] = 0.9
  IF (PL9Dccount GT 0.0) THEN Calpha[36, LengthGE200[PL9Dc]] = 0.00 ; for walleye
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL9E = WHERE(((FISHPREY[25, LocV[4, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL9Ecount, complement = PL9Ec, ncomplement = PL9Eccount)
  IF (PL9Ecount GT 0.0) THEN Calpha[37, LengthLT200[PL9E]] = 0.9
  IF (PL9Eccount GT 0.0) THEN Calpha[37, LengthLT200[PL9Ec]] = 0.00 ; for walleye
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL9E = WHERE(((FISHPREY[25, LocV[4, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL9Ecount, complement = PL9Ec, ncomplement = PL9Eccount)
  IF (PL9Ecount GT 0.0) THEN Calpha[37, LengthGE200[PL9E]] = 0.9
  IF (PL9Eccount GT 0.0) THEN Calpha[37, LengthGE200[PL9Ec]] = 0.00 ; for walleye
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL9F = WHERE(((FISHPREY[25, LocV[5, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL9Fcount, complement = PL9Fc, ncomplement = PL9Fccount)
  IF (PL9Fcount GT 0.0) THEN Calpha[38, LengthLT200[PL9F]] = 0.9
  IF (PL9Fccount GT 0.0) THEN Calpha[38, LengthLT200[PL9Fc]] = 0.00 ; for walleye
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL9F = WHERE(((FISHPREY[25, LocV[5, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL9Fcount, complement = PL9Fc, ncomplement = PL9Fccount)
  IF (PL9Fcount GT 0.0) THEN Calpha[38, LengthGE200[PL9F]] = 0.9
  IF (PL9Fccount GT 0.0) THEN Calpha[38, LengthGE200[PL9Fc]] = 0.00 ; for walleye
ENDIF

IF (LengthLT200count GT 0.0) THEN BEGIN
  PL9G = WHERE(((FISHPREY[25, LocV[6, LengthLT200]] / WAE[1, LengthLT200]) LT 0.6), PL9Gcount, complement = PL9Gc, ncomplement = PL9Gccount)
  IF (PL9Gcount GT 0.0) THEN Calpha[39, LengthLT200[PL9G]] = 0.9
  IF (PL9Gccount GT 0.0) THEN Calpha[39, LengthLT200[PL9Gc]] = 0.00 ; for walleye
ENDIF  
IF (LengthGE200count GT 0.0) THEN BEGIN
  PL9G = WHERE(((FISHPREY[25, LocV[6, LengthGE200]] / WAE[1, LengthGE200]) LT 0.4), PL9Gcount, complement = PL9Gc, ncomplement = PL9Gccount)
  IF (PL9Gcount GT 0.0) THEN Calpha[39, LengthGE200[PL9G]] = 0.9
  IF (PL9Gccount GT 0.0) THEN Calpha[39, LengthGE200[PL9Gc]] = 0.00 ; for walleye
ENDIF
;PRINT, 'Calpha'
;PRINT, Calpha
...
; Calculate attack probability using chesson's alpha = capture efficiency
TOT = FLTARR(7, nWAE); total number of all prey atacked and captured
t = FLTARR(m*7, nWAE); total number of each prey atacked and captured
; microzooplankton
...
;; Add small values for zero prey cells to avoid floating errors
...
EnvironVprey = FLTARR(7, nWAE)
EnvironVprey[0, *] = (TOT[0, *] / preyTOT2[0, *]) * RANDOMU(seed, nWAE, /DOUBLE)
EnvironVprey[1, *] = (TOT[1, *] / preyTOT2[1, *]) * RANDOMU(seed, nWAE, /DOUBLE)
EnvironVprey[2, *] = (TOT[2, *] / preyTOT2[2, *]) * RANDOMU(seed, nWAE, /DOUBLE)
EnvironVprey[3, *] = (TOT[3, *] / preyTOT2[3, *]) * RANDOMU(seed, nWAE, /DOUBLE)
EnvironVprey[4, *] = (TOT[4, *] / preyTOT2[4, *]) * RANDOMU(seed, nWAE, /DOUBLE)
EnvironVprey[5, *] = (TOT[5, *] / preyTOT2[5, *]) * RANDOMU(seed, nWAE, /DOUBLE)
EnvironVprey[6, *] = (TOT[6, *] / preyTOT2[6, *]) * RANDOMU(seed, nWAE, /DOUBLE)
;PRINT, 'EnvironVprey'
...
Gutfull2 = (100.- (WAE[60, *] < 100.0))/100.; < (1./3.); * RANDOMU(seed, nYP, /DOUBLE)
...
; Calculate fish swimming speed, S, in body /sec
SS = FLTARR(nWAE)
S = FLTARR(nWAE)
...
;PRINT, TRANSPOSE(MoveSpeed[2, 0:99])
...
VerSize = FLTARR(nWAE)
VerSize[*] = .5 * 1000.; in mm, ALL VERTICAL LAYERS = .5 m
...
IF zMoveOut0count GT 0.0 THEN WAE[14, zMoveOut0] = LocV[6, zMoveOut0];
IF zMoveOut1count GT 0.0 THEN WAE[14, zMoveOut1] = LocV[6, zMoveOut1];
IF zMoveOut2count GT 0.0 THEN WAE[14, zMoveOut2] = LocV[5, zMoveOut2];
IF zMoveOut3count GT 0.0 THEN WAE[14, zMoveOut3] = LocV[4, zMoveOut3];
IF zMoveOut4count GT 0.0 THEN WAE[14, zMoveOut4] = LocV[0, zMoveOut4];
IF zMoveOut5count GT 0.0 THEN WAE[14, zMoveOut5] = LocV[3, zMoveOut5];
IF zMoveOut6count GT 0.0 THEN WAE[14, zMoveOut6] = LocV[2, zMoveOut6];
IF zMoveOut7count GT 0.0 THEN WAE[14, zMoveOut7] = LocV[1, zMoveOut7];
IF zMoveOut8count GT 0.0 THEN WAE[14, zMoveOut8] = LocV[1, zMoveOut8];
...
;moved=abs(locv[0, *] - wae[14,*])
;PRINT,'move max ',max(moved)
;PRINT,' move avg ',mean(moved),' +/-',stddev(moved)


t_elapsed = systime(/seconds) - tstart
PRINT, 'Elapesed time (seconds):', t_elapsed
PRINT, 'Elapesed time (minutes):', t_elapsed/60.
PRINT, 'Walleye 1D Movement Ends Here'
