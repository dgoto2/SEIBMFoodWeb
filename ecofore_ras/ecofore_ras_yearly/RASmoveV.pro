FUNCTION RASmoveV, RAS, nRAS, newinput, Grid3D, Grid2D, DOacclim, NewGridID2
;function determines movement in X,Y,Z direction for all RASINBOW SMELT

;----------------------TEST ONLY---------------------------------------------------------------------------
;PRO RASmoveV, RAS, nRAS, newinput, Grid3D, Grid2D, DOacclim, NewGridID2
...
;--Vertical movement---------------------------------------------------------------------------------------------------------------------------------
NewCell = FLTARR(15, nRAS);
NewLoc = FLTARR(nRAS)
Ncell = FLTARR(15, nRAS)
d = FLTARR(nRAS)
...
FOR evvv = 0L, nRAS - 1L DO BEGIN
  FOR ev = 0L, nRAS - 1L DO BEGIN;evv + 1L 
...
LengthV = FLTARR(nRAS*20L)
;depth = indgen(20); 1 = top layer; 20 = bottom layer
j = 19L
FOR ii = 0L, nRAS*20L - 1L DO BEGIN
FOR iii = 0L, nRAS - 1L DO BEGIN
LengthV[ii : j] = RAS[1, iii]
ii = j
j = j + 20L
ENDFOR
ENDFOR
...
;calculate attack probability using chesson's alpha = capture efficiency
  TOT = FLTARR(nRAS*20L); total number of all prey atacked and captured
  t = FLTARR(m, nRAS*20L); total number of each prey atacked and captured
      t[0,*] = (Calpha[0,*] * dens[0,*])
      t[1,*] = (Calpha[1,*] * dens[1,*])
      t[2,*] = (Calpha[2,*] * dens[2,*])
      t[3,*] = (Calpha[3,*] * dens[3,*])
...
preyTOT = FLTARR(nRAS*20L)      
jv = 19L
FOR ivvv = 0L, nRAS*20L - 1L DO BEGIN
...
preyTOT2 = FLTARR(nRAS*20L)
j = 19L
FOR iii = 0L, nRAS*20L - 19L DO BEGIN
FOR iiii = 0L, nRAS*20L - 1L DO BEGIN
...
TOTprop = FLTARR(nRAS*20L)
EnvironV[10, *] = TOT / preyTOT2
TOTprop = TOT / preyTOT2
;PRINT, 'TOTprop =', EnvironV[10, *]
;PRINT, 'TOTprop =', TOTprop
EnvironV10 = EnvironV[10, *] * RANDOMU(seed, nRAS*20L, /DOUBLE)
PRINT, 'EnvironV10 =', EnvironV10

DOf1 = FLTARR(nRAS*20L)
DOf2 = FLTARR(nRAS*20L)
DOf3 = FLTARR(nRAS*20L)
DOf = FLTARR(20L)
Tf1 = FLTARR(nRAS*20L)
Tf2 = FLTARR(nRAS*20L)
Tf3 = FLTARR(nRAS*20L)
...
FOR dovvv = 0L, nRAS - 1L DO BEGIN
  FOR dov = 0L, nRAS - 1L DO BEGIN
...
EnvironVMax = FLTARR(nRAS*20L)
EnvironVMax2 = FLTARR(nRAS*20L)
EnvironVMax3 = FLTARR(nRAS*20L)
jvvv = 19L
; Create array with highest habitat quality-based probability for fish
FOR iv = 0L, nRAS*20L - 1L DO BEGIN
...
DepthLoc = FLTARR(nRAS)
DepthLoc2 = FLTARR(nRAS)
jvvvv = 19L
FOR iv = 0L, nRAS*20L - 1L DO BEGIN
  FOR ivv = 0L, nRAS-1L DO BEGIN 
...
PRINT, ' RAS Vertical Movement Ends Here'
