FUNCTION YEPFishPredArray1DMove, YP, WAE, nGridcell
; Create a fish array for potential predators

PRINT, 'YEPFishPredArray1DMove BEGINS HERE'
...
; Walleye
FOR ID = 0L, nGridcell-1L DO BEGIN
  WAEmultiSI = WHERE((WAE[14, *] EQ ID) AND (YP[14, *] EQ ID), WAEmultiSIcount); 
...
PRINT, 'YEPFishPredArray1DMove ENDS HERE'
RETURN, FishPred
END