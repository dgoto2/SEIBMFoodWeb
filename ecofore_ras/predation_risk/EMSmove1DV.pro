FUNCTION EMSMove1DV, ts, iHour, EMS, nEMS, NewInput, FISHPREY, FishPred, zOldLocWithinCell, Oxydebt
;function determines movement in Z direction for emerald shiner

PRINT, 'Emerald Shiner 1D Movement Begins Here'
tstart = SYSTIME(/seconds)

;*****NEED TO INCORPORATE PREDATOR-BASED HABITAT-QIALITY***********************************
;******NO NEED TO IDENTIFY THE CURRENT CELL
;******Change * to 0 in row subscripting*********


;*****Identify potential cell IDs in all 6 directions*******************************************
LocV = FLTARR(7L, nEMS)
LocV[0, *] = EMS[14, *]; current CELL ID
; VERTICAL LAYERS 0 - 47
; ***Vertical movement is restricted within 3 cells below and above the current layer***

; DOWNWARD MOVEMNT
; current layer -3
Lower3Cells0 = WHERE((EMS[14, *] LE 44L), Lower3Cells0count, complement = NONLower3Cells0, ncomplement = NonLower3Cells0count)
...
; current layer -2
Lower2Cells0 = WHERE((EMS[14, *] LE 45L), Lower2Cells0count, complement = NONLower2Cells0, ncomplement = NonLower2Cells0count)
...
; current layer -1
Lower1Cells0 = WHERE((EMS[14, *] LE 46L), Lower1Cells0count, complement = NONLower1Cells0, ncomplement = NonLower1Cells0count)
...
;UPWARD MOVEMENT
; current layer +1
Upper1Cells0 = WHERE((EMS[14, *] GE 1L), Upper1Cells0count, complement = NONUpper1Cells0, ncomplement = NonUpper1Cells0count)
...
; current layer +2
Upper2Cells0 = WHERE((EMS[14, *] GE 2L), Upper2Cells0count, complement = NONUpper2Cells0, ncomplement = NonUpper2Cells0count)
...
; current layer +3
Upper3Cells0 = WHERE((EMS[14, *] GE 3L), Upper3Cells0count, complement = NONUpper3Cells0, ncomplement = NonUpper3Cells0count)
...
EnvironV = FLTARR(105L, nEMS)
;Envir1D3[4, *] = INDGEN(nVerLay)
;Envir1D3[5, *] = zoopl1; microzooplankton, g/L
;Envir1D3[6, *] = zoopl2 ; mid-size zooplankton; g/L
;Envir1D3[7, *] = zoopl3; large-bodied zooplankton; g/L
...
;****MAY NEED FOR POTENTIAL COMPETION AND PREDATION RISK***************************************
...
; DO***************************************************************************************************************
DOacclim = EMSacclDO(EMS[29, *], EMS[28, *], EMS[20, *], EMS[27, *], EMS[26, *], EMS[19, *], ts, EMS[1, *], EMS[2, *], nEMS, EMS[63, *])
;PRINT, 'DOacclim[7, *]'
...
; parameter vlaues from Arend et al., 2010
CA = 0.254; 0.36; 4.033
CB = -0.276; -0.31; -1.0589
CQ = 2.25; 2.3; 3.8266
CTO = 25.; 26.; 35.0
CTM = 30.; 29.; 41.6
...
; Light*****************************************************************************************************************
litfac = FLTARR(7, nEMS); a multiplication factor to include the effect of light intensity
t1 = FLTARR(7, nEMS)
t2 = FLTARR(7, nEMS)
...
EnvironVDO = FLTARR(7, nEMS)
EnvironVT = FLTARR(7, nEMS)
EnvironVL = FLTARR(7, nEMS)

EnvironVDO[0, *] = DOUBLE(DOf[0, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVDO[1, *] = DOUBLE(DOf[1, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVDO[2, *] = DOUBLE(DOf[2, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVDO[3, *] = DOUBLE(DOf[3, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVDO[4, *] = DOUBLE(DOf[4, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVDO[5, *] = DOUBLE(DOf[5, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVDO[6, *] = DOUBLE(DOf[6, *] * RANDOMU(seed, nEMS, /DOUBLE))
;PRINT, 'Environv[8, *]', EnvironV[8, *]; DO-based habitat index with a random component

EnvironVT[0, *] = DOUBLE(Tf[0, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVT[1, *] = DOUBLE(Tf[1, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVT[2, *] = DOUBLE(Tf[2, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVT[3, *] = DOUBLE(Tf[3, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVT[4, *] = DOUBLE(Tf[4, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVT[5, *] = DOUBLE(Tf[5, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVT[6, *] = DOUBLE(Tf[6, *] * RANDOMU(seed, nEMS, /DOUBLE))
;PRINT, 'Environv[9, *]', EnvironV[9, *]; Temp-based habitat index with a random component

EnvironVL[0, *] = DOUBLE(litfac[0, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVL[1, *] = DOUBLE(litfac[1, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVL[2, *] = DOUBLE(litfac[2, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVL[3, *] = DOUBLE(litfac[3, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVL[4, *] = DOUBLE(litfac[4, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVL[5, *] = DOUBLE(litfac[5, *] * RANDOMU(seed, nEMS, /DOUBLE))
EnvironVL[6, *] = DOUBLE(litfac[6, *] * RANDOMU(seed, nEMS, /DOUBLE))
;PRINT, 'EnvironVDO'
...
PredRisk = FLTARR(14, nEMS)
; walleye predators
IF (LengthLT200Acount GT 0.0) THEN BEGIN
  PredL1A = WHERE(((EMS[1, LengthLT200A] / FishPred[5, LocV[0, LengthLT200A]]) LT 0.6), PredL1Acount, complement = PredL1Ac, ncomplement = PredL1Account)
...
IF (LengthGE200Acount GT 0.0) THEN BEGIN
  PredL1A = WHERE(((EMS[1, LengthGE200A] / FishPred[5, LocV[0, LengthGE200A]]) LT 0.4), PredL1Acount, complement = PredL1Ac, ncomplement = PredL1Account)
...
IF (LengthLT200Bcount GT 0.0) THEN BEGIN
  PredL1B = WHERE(((EMS[1, LengthLT200B] / FishPred[5, LocV[1, LengthLT200B]]) LT 0.6), PredL1Bcount, complement = PredL1Bc, ncomplement = PredL1Bccount)
...
IF (LengthGE200Bcount GT 0.0) THEN BEGIN
  PredL1B = WHERE(((EMS[1, LengthGE200B] / FishPred[5, LocV[1, LengthGE200B]]) LT 0.4), PredL1Bcount, complement = PredL1Bc, ncomplement = PredL1Bccount)
...
IF (LengthLT200Ccount GT 0.0) THEN BEGIN
  PredL1C = WHERE(((EMS[1, LengthLT200C] / FishPred[5, LocV[2, LengthLT200C]]) LT 0.6), PredL1Ccount, complement = PredL1Cc, ncomplement = PredL1Cccount)
...
IF (LengthGE200Ccount GT 0.0) THEN BEGIN
  PredL1C = WHERE(((EMS[1, LengthGE200C] / FishPred[5, LocV[2, LengthGE200C]]) LT 0.4), PredL1Ccount, complement = PredL1Cc, ncomplement = PredL1Cccount)
...
IF (LengthLT200Dcount GT 0.0) THEN BEGIN
  PredL1D = WHERE(((EMS[1, LengthLT200D] / FishPred[5, LocV[3, LengthLT200D]]) LT 0.6), PredL1Dcount, complement = PredL1Dc, ncomplement = PredL1Dccount)
...
IF (LengthGE200Dcount GT 0.0) THEN BEGIN
  PredL1D = WHERE(((EMS[1, LengthGE200D] / FishPred[5, LocV[3, LengthGE200D]]) LT 0.4), PredL1Dcount, complement = PredL1Dc, ncomplement = PredL1Dccount)
...
IF (LengthLT200Ecount GT 0.0) THEN BEGIN
  PredL1E = WHERE(((EMS[1, LengthLT200E] / FishPred[5, LocV[4, LengthLT200E]]) LT 0.6), PredL1Ecount, complement = PredL1Ec, ncomplement = PredL1Eccount)
...
IF (LengthGE200Ecount GT 0.0) THEN BEGIN
  PredL1E = WHERE(((EMS[1, LengthGE200E] / FishPred[5, LocV[4, LengthGE200E]]) LT 0.4), PredL1Ecount, complement = PredL1Ec, ncomplement = PredL1Eccount)
...
IF (LengthLT200Fcount GT 0.0) THEN BEGIN
  PredL1F = WHERE(((EMS[1, LengthLT200F] / FishPred[5, LocV[5, LengthLT200F]]) LT 0.6), PredL1Fcount, complement = PredL1Fc, ncomplement = PredL1Fccount)
...
IF (LengthGE200Fcount GT 0.0) THEN BEGIN
  PredL1F = WHERE(((EMS[1, LengthGE200F] / FishPred[5, LocV[5, LengthGE200F]]) LT 0.4), PredL1Fcount, complement = PredL1Fc, ncomplement = PredL1Fccount)
...
IF (LengthLT200Gcount GT 0.0) THEN BEGIN
  PredL1G = WHERE(((EMS[1, LengthLT200G] / FishPred[5, LocV[6, LengthLT200G]]) LT 0.6), PredL1Gcount, complement = PredL1Gc, ncomplement = PredL1Gccount)
...
IF (LengthGE200Gcount GT 0.0) THEN BEGIN
  PredL1G = WHERE(((EMS[1, LengthGE200G] / FishPred[5, LocV[6, LengthGE200G]]) LT 0.4), PredL1Gcount, complement = PredL1Gc, ncomplement = PredL1Gccount)
...
IF (LengthGE150Acount GT 0.0) THEN BEGIN
  PredL2A = WHERE(((EMS[1, LengthGE150A] / FishPred[0, LocV[0, LengthGE150A]]) LT 0.2), PredL2Acount, complement = PredL2Ac, ncomplement = PredL2Account)
...
IF (LengthGE150Bcount GT 0.0) THEN BEGIN
  PredL2B = WHERE(((EMS[1, LengthGE150B] / FishPred[0, LocV[1, LengthGE150B]]) LT 0.2), PredL2Bcount, complement = PredL2Bc, ncomplement = PredL2Bccount)
...
IF (LengthGE150Ccount GT 0.0) THEN BEGIN
  PredL2C = WHERE(((EMS[1, LengthGE150C] / FishPred[0, LocV[2, LengthGE150C]]) LT 0.2), PredL2Ccount, complement = PredL2Cc, ncomplement = PredL2Cccount)
...
IF (LengthGE150Dcount GT 0.0) THEN BEGIN
  PredL2D = WHERE(((EMS[1, LengthGE150D] / FishPred[0, LocV[3, LengthGE150D]]) LT 0.2), PredL2Dcount, complement = PredL2Dc, ncomplement = PredL2Dccount)
...
IF (LengthGE150Ecount GT 0.0) THEN BEGIN
  PredL2E = WHERE(((EMS[1, LengthGE150E] / FishPred[0, LocV[4, LengthGE150E]]) LT 0.2), PredL2Ecount, complement = PredL2Ec, ncomplement = PredL2Eccount)
...
IF (LengthGE150Fcount GT 0.0) THEN BEGIN
  PredL2F = WHERE(((EMS[1, LengthGE150F] / FishPred[0, LocV[5, LengthGE150F]]) LT 0.2), PredL2Fcount, complement = PredL2Fc, ncomplement = PredL2Fccount)
...
IF (LengthGE150Gcount GT 0.0) THEN BEGIN
  PredL2G = WHERE(((EMS[1, LengthGE150G] / FishPred[0, LocV[6, LengthGE150G]]) LT 0.2), PredL2Gcount, complement = PredL2Gc, ncomplement = PredL2Gccount)
...
TOTpred = FLTARR(7, nEMS); total number of all prey atacked and captured
Predt = FLTARR(2*7, nEMS); total number of each prey atacked and captured
; walleye
...
Calpha = FLTARR(50, nEMS); from Letcher et al. for zooplankton
Calpha[0, *] = 193499 * EMS[1, *]^(-7.64); for microzooplankton - rotifers
Calpha[1, *] = (0.272 * ALOG(EMS[1, *]) - 0.3834) < 1.; for mesozooplankton1 - calanoids
Calpha[2, *] = 0.8 / (1.0 + ((0.4 / 0.09) - 1.0) * EXP(-13.0 * 0.031 * EMS[1, *]))^(1.0 / 0.031); for mesozooplankton2 - cladocerans
...
;Length60 = WHERE(EMS[1, *] GT 60.0, Length60count, complement = Length60c, ncomplement = Length60ccount)
...
;Length80 = WHERE((EMS[1, *] GT 80.0), Length80count, complement = Length80c, ncomplement = Length80ccount)
;PL5A = WHERE(((FISHPREY[1, LocV[0, *]] / EMS[1, *]) LT 0.2), PL5Acount, complement = PL5Ac, ncomplement = PL5Account)
;PL5Aa = WHERE(((FISHPREY[1, LocV[0, *]] / EMS[1, *]) GE 0.2), PL5Aacount, complement = PL5Aac, ncomplement = PL5Aaccount)
...
;
;PL5B = WHERE(((FISHPREY[1, LocV[1, *]] / EMS[1, *]) LT 0.2), PL5Bcount, complement = PL5Bc, ncomplement = PL5Bccount)
;PL5Ba = WHERE(((FISHPREY[1, LocV[1, *]] / EMS[1, *]) GE 0.2), PL5Bacount, complement = PL5Bac, ncomplement = PL5Baccount)
...
;
;PL5C = WHERE(((FISHPREY[1, LocV[2, *]] / EMS[1, *]) LT 0.2), PL5Ccount, complement = PL5Cc, ncomplement = PL5Cccount)
;PL5Ca = WHERE(((FISHPREY[1, LocV[2, *]] / EMS[1, *]) GE 0.2), PL5Cacount, complement = PL5Cac, ncomplement = PL5Caccount)
...
;
;PL5D = WHERE(((FISHPREY[1, LocV[3, *]] / EMS[1, *]) LT 0.2), PL5Dcount, complement = PL5Dc, ncomplement = PL5Dccount)
;PL5Da = WHERE(((FISHPREY[1, LocV[3, *]] / EMS[1, *]) GE 0.2), PL5Dacount, complement = PL5Dac, ncomplement = PL5Daccount)
...
;
;PL5E = WHERE(((FISHPREY[1, LocV[4, *]] / EMS[1, *]) LT 0.2), PL5Ecount, complement = PL5Ec, ncomplement = PL5Eccount)
;PL5Ea = WHERE(((FISHPREY[1, LocV[4, *]] / EMS[1, *]) GE 0.2), PL5Eacount, complement = PL5Eac, ncomplement = PL5Eaccount)
...
;
;PL5F = WHERE(((FISHPREY[1, LocV[5, *]] / EMS[1, *]) LT 0.2), PL5Fcount, complement = PL5Fc, ncomplement = PL5Fccount)
;PL5Fa = WHERE(((FISHPREY[1, LocV[5, *]] / EMS[1, *]) GE 0.2), PL5Facount, complement = PL5Fac, ncomplement = PL5Faccount)
...
;
;PL5G = WHERE(((FISHPREY[1, LocV[6, *]] / EMS[1, *]) LT 0.2), PL5Gcount, complement = PL5Gc, ncomplement = PL5Gccount)
;PL5Ga = WHERE(((FISHPREY[1, LocV[6, *]] / EMS[1, *]) GE 0.2), PL5Gacount, complement = PL5Gac, ncomplement = PL5Gaccount)
...
; Calculate attack probability using chesson's alpha = capture efficiency
TOT = FLTARR(7, nEMS); total number of all prey atacked and captured
t = FLTARR(m*7, nEMS); total number of each prey atacked and captured
; microzooplankton
...
EnvironVprey = FLTARR(7, nEMS)
EnvironVprey[0, *] = (TOT[0, *] / preyTOT2[0, *]) * RANDOMU(seed, nEMS, /DOUBLE)
EnvironVprey[1, *] = (TOT[1, *] / preyTOT2[1, *]) * RANDOMU(seed, nEMS, /DOUBLE)
EnvironVprey[2, *] = (TOT[2, *] / preyTOT2[2, *]) * RANDOMU(seed, nEMS, /DOUBLE)
EnvironVprey[3, *] = (TOT[3, *] / preyTOT2[3, *]) * RANDOMU(seed, nEMS, /DOUBLE)
EnvironVprey[4, *] = (TOT[4, *] / preyTOT2[4, *]) * RANDOMU(seed, nEMS, /DOUBLE)
EnvironVprey[5, *] = (TOT[5, *] / preyTOT2[5, *]) * RANDOMU(seed, nEMS, /DOUBLE)
EnvironVprey[6, *] = (TOT[6, *] / preyTOT2[6, *]) * RANDOMU(seed, nEMS, /DOUBLE)
;PRINT, 'EnvironVprey'
...
Gutfull2 = (100.- (EMS[60, *] < 100.0))/100.; < (1./3.); * RANDOMU(seed, nYP, /DOUBLE)
...
; 2. Predation risk
Gutfull11 = (1. - Gutfull4) * (1 - EnvironVpred[0, *]);  higher the predator-based habitat quality index, lower the weight
...
IF (lcount GT 0.0) THEN S[l] = 3.3354 * ALOG(EMS[1, l]) - 4.8758;(-0.0797 * (YP[1, *][l] * YP[1, *][l]) + 1.9294 * YP[1, *][l] - 8.1761)
 ;SS equation based on data from Houde 1969 in body YP[1, *]s/sec
IF (llcount GT 0.0) THEN S[ll] = (0.28 * EMS[26, ll] + 7.89) / (0.1 * EMS[1, ll])
...
IF zMoveOut0count GT 0.0 THEN EMS[14, zMoveOut0] = LocV[6, zMoveOut0];
IF zMoveOut1count GT 0.0 THEN EMS[14, zMoveOut1] = LocV[6, zMoveOut1];
IF zMoveOut2count GT 0.0 THEN EMS[14, zMoveOut2] = LocV[5, zMoveOut2];
IF zMoveOut3count GT 0.0 THEN EMS[14, zMoveOut3] = LocV[4, zMoveOut3];
IF zMoveOut4count GT 0.0 THEN EMS[14, zMoveOut4] = LocV[0, zMoveOut4];
IF zMoveOut5count GT 0.0 THEN EMS[14, zMoveOut5] = LocV[3, zMoveOut5];
IF zMoveOut6count GT 0.0 THEN EMS[14, zMoveOut6] = LocV[2, zMoveOut6];
IF zMoveOut7count GT 0.0 THEN EMS[14, zMoveOut7] = LocV[1, zMoveOut7];
IF zMoveOut8count GT 0.0 THEN EMS[14, zMoveOut8] = LocV[1, zMoveOut8];
...
PRINT, 'Emerald Shiner 1D Movement Ends Here'
