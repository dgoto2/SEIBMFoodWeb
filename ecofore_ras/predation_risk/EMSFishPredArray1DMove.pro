FUNCTION EMSFishPredArray1DMove, YP, WAE, EMS, nGridcell
; Create a fish array for potential predators

PRINT, 'EMSFishPredArray1DMove BEGINS HERE'
...
FOR ID = 0L, nGridcell-1L DO BEGIN
  YEPmultiSI = WHERE((YP[14, *] EQ ID) AND (EMS[14, *] EQ ID), YEPmultiSIcount); 
...
; Walleye
FOR ID = 0L, nGridcell-1L DO BEGIN
  WAEmultiSI = WHERE((WAE[14, *] EQ ID) AND (EMS[14, *] EQ ID), WAEmultiSIcount); 
...
PRINT, 'EMSFishPredArray1DMove ENDS HERE'
RETURN, FishPred
END