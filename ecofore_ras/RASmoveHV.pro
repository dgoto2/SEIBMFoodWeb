FUNCTION RASMoveHV, ts, iHour, RAS, nRAS, NewInput, TotBenBio, Grid2D, xOldLocWithinCell, yOldLocWithinCell, zOldLocWithinCell, Oxydebt
...
;**********************TEST ONLY*************************************************
;PRO RASMoveHV, RAS, nRAS, NewInput, TotBenBio, Grid2D, xOldLocWithinCell, yOldLocWithinCell, zOldLocWithinCell
;; NEED to change NewInputFiles, YPinitial, YEPacclT, YEPacclDO for testing
;nRAS = 50000L; number of SI YP, MOVEMENT PARAMETER
;
;ihour = 15L
...
;
;NpopRAS = 50000000L; number of WAE individuals
;RAS = RASinitial(NpopRAS, nRAS, TotBenBio, NewInput); MOVEMENT PARAMETER
...
;DOacclR = RAS[29, *]
;DOacclC = RAS[28, *]
;DOacclim = RASacclDO(DOacclR, DOacclC, DOa, TacclR, TacclC, Tamb, ts, Length, Weight, nRAS); MOVEMENT PARAMETER
;
;xOldLocWithinCell = RAS[39, *]; MOVEMENT PARAMETER 
;yOldLocWithinCell = RAS[40, *]; MOVEMENT PARAMETER
;zOldLocWithinCell = RAS[41, *]; MOVEMENT PARAMETER
;***********************************************************************************

PRINT, 'Rainbow Smelt Movement Begins Here'
...
LocH = FLTARR(30, nRAS)
LocHV = FLTARR(9*7L, nRAS)
NewGridXY = FLTARR(10L, nRAS)
...
;IF (ZLocH6count GT 0.0) THEN LocH[6, ZLocH6] = RAS[10, ZLocH6]+1L
LocH[7, *] = RAS[11, *] 
; 5
LocH[8, *] = RAS[10, *] + 1L
...
;IF (ZLocH8count GT 0.0) THEN LocH[8, ZLocH8] = RAS[10, ZLocH8]-1L
LocH[9, *] = RAS[11, *] 
;; 6
;LocH[10, *] = RAS[10, *] - 1L
;LocH[11, *] = RAS[11, *] - 1L
...
FOR ihh = 0L, nRAS - 1L DO BEGIN;*****Time-consuming part*********************************************************************
...
NewGridXY[1, *] = RAS[13, *]+1L; 2-> CURRENT +1 
...
NewGridXY[6, *] = RAS[13, *]-1L; 7 -> CURRENT - 1
...
;NZRAS13 = WHERE((RAS[13, *] GT 0.0), NZRAS13count, complement = ZRAS13, ncomplement = ZRAS13count);
...
;IF (NZRAS13count GT 0.0) THEN NewGridXY[6, NZRAS13] = RAS[13, NZRAS13]-1L; 7 -> CURRENT -1
;IF (ZRAS13count GT 0.0) THEN NewGridXY[6, ZRAS13] = RAS[13, ZRAS13]+1L; 7 -> CURRENT -1
...
LocNH17A = WHERE((NewGridXY[1, *] GE 0.0) AND (Grid2D[0, NewGridXY[1, *]] EQ RAS[10, *]), LocNH17Acount, complement = LocH17A, ncomplement = LocH17Acount) ; LocH[7, *]
...
LocNH17B = WHERE((NewGridXY[6, *] GE 0.0) AND (Grid2D[0, NewGridXY[6, *]] EQ RAS[10, *]), LocNH17Bcount, complement = LocH17B, ncomplement = LocH17Bcount) ; LocH[7, *]
...
IF (NZNewGridXY0count GT 0.0) THEN LocHV[0, NZNewGridXY0] = (NewGridXY[0, NZNewGridXY0])*20. + (RAS[12, NZNewGridXY0]-1.) 
IF (NZNewGridXY1count GT 0.0) THEN LocHV[1, NZNewGridXY1] = (NewGridXY[1, NZNewGridXY1])*20. + (RAS[12, NZNewGridXY1]-1.) 
IF (NZNewGridXY2count GT 0.0) THEN LocHV[2, NZNewGridXY2] = (NewGridXY[2, NZNewGridXY2])*20. + (RAS[12, NZNewGridXY2]-1.) 
IF (NZNewGridXY3count GT 0.0) THEN LocHV[3, NZNewGridXY3] = (NewGridXY[3, NZNewGridXY3])*20. + (RAS[12, NZNewGridXY3]-1.) 
IF (NZNewGridXY4count GT 0.0) THEN LocHV[4, NZNewGridXY4] = (NewGridXY[4, NZNewGridXY4])*20. + (RAS[12, NZNewGridXY4]-1.) 
IF (NZNewGridXY5count GT 0.0) THEN LocHV[5, NZNewGridXY5] = (NewGridXY[5, NZNewGridXY5])*20. + (RAS[12, NZNewGridXY5]-1.) 
IF (NZNewGridXY6count GT 0.0) THEN LocHV[6, NZNewGridXY6] = (NewGridXY[6, NZNewGridXY6])*20. + (RAS[12, NZNewGridXY6]-1.) 
IF (NZNewGridXY7count GT 0.0) THEN LocHV[7, NZNewGridXY7] = (NewGridXY[7, NZNewGridXY7])*20. + (RAS[12, NZNewGridXY7]-1.) 
IF (NZNewGridXY8count GT 0.0) THEN LocHV[8, NZNewGridXY8] = (NewGridXY[8, NZNewGridXY8])*20. + (RAS[12, NZNewGridXY8]-1.) 
...
;DOWNWARD MOVEMNT
; current layer -3
Lower3Cells0 = WHERE((RAS[12, *] LE 17L) AND (NewGridXY[0, *] GE 0.0), Lower3Cells0count)
Lower3Cells1 = WHERE((RAS[12, *] LE 17L) AND (NewGridXY[1, *] GE 0.0), Lower3Cells1count)
Lower3Cells2 = WHERE((RAS[12, *] LE 17L) AND (NewGridXY[2, *] GE 0.0), Lower3Cells2count)
Lower3Cells3 = WHERE((RAS[12, *] LE 17L) AND (NewGridXY[3, *] GE 0.0), Lower3Cells3count)
Lower3Cells4 = WHERE((RAS[12, *] LE 17L) AND (NewGridXY[4, *] GE 0.0), Lower3Cells4count)
Lower3Cells5 = WHERE((RAS[12, *] LE 17L) AND (NewGridXY[5, *] GE 0.0), Lower3Cells5count)
Lower3Cells6 = WHERE((RAS[12, *] LE 17L) AND (NewGridXY[6, *] GE 0.0), Lower3Cells6count)
Lower3Cells7 = WHERE((RAS[12, *] LE 17L) AND (NewGridXY[7, *] GE 0.0), Lower3Cells7count)
Lower3Cells8 = WHERE((RAS[12, *] LE 17L) AND (NewGridXY[8, *] GE 0.0), Lower3Cells8count)
...
NONLower3Cells0 = WHERE((RAS[12, *] GT 17L) AND (NewGridXY[0, *] GE 0.0), NonLower3Cells0count)
NONLower3Cells1 = WHERE((RAS[12, *] GT 17L) AND (NewGridXY[1, *] GE 0.0), NonLower3Cells1count)
NONLower3Cells2 = WHERE((RAS[12, *] GT 17L) AND (NewGridXY[2, *] GE 0.0), NonLower3Cells2count)
NONLower3Cells3 = WHERE((RAS[12, *] GT 17L) AND (NewGridXY[3, *] GE 0.0), NonLower3Cells3count)
NONLower3Cells4 = WHERE((RAS[12, *] GT 17L) AND (NewGridXY[4, *] GE 0.0), NonLower3Cells4count)
NONLower3Cells5 = WHERE((RAS[12, *] GT 17L) AND (NewGridXY[5, *] GE 0.0), NonLower3Cells5count)
NONLower3Cells6 = WHERE((RAS[12, *] GT 17L) AND (NewGridXY[6, *] GE 0.0), NonLower3Cells6count)
NONLower3Cells7 = WHERE((RAS[12, *] GT 17L) AND (NewGridXY[7, *] GE 0.0), NonLower3Cells7count)
NONLower3Cells8 = WHERE((RAS[12, *] GT 17L) AND (NewGridXY[8, *] GE 0.0), NonLower3Cells8count)
...
; current layer -2
Lower2Cells0 = WHERE((RAS[12, *] LE 18L) AND (NewGridXY[0, *] GE 0.0), Lower2Cells0count)
Lower2Cells1 = WHERE((RAS[12, *] LE 18L) AND (NewGridXY[1, *] GE 0.0), Lower2Cells1count)
Lower2Cells2 = WHERE((RAS[12, *] LE 18L) AND (NewGridXY[2, *] GE 0.0), Lower2Cells2count)
Lower2Cells3 = WHERE((RAS[12, *] LE 18L) AND (NewGridXY[3, *] GE 0.0), Lower2Cells3count)
Lower2Cells4 = WHERE((RAS[12, *] LE 18L) AND (NewGridXY[4, *] GE 0.0), Lower2Cells4count)
Lower2Cells5 = WHERE((RAS[12, *] LE 18L) AND (NewGridXY[5, *] GE 0.0), Lower2Cells5count)
Lower2Cells6 = WHERE((RAS[12, *] LE 18L) AND (NewGridXY[6, *] GE 0.0), Lower2Cells6count)
Lower2Cells7 = WHERE((RAS[12, *] LE 18L) AND (NewGridXY[7, *] GE 0.0), Lower2Cells7count)
Lower2Cells8 = WHERE((RAS[12, *] LE 18L) AND (NewGridXY[8, *] GE 0.0), Lower2Cells8count)
...
NONLower2Cells0 = WHERE((RAS[12, *] GT 18L) AND (NewGridXY[0, *] GE 0.0), NonLower2Cells0count)
NONLower2Cells1 = WHERE((RAS[12, *] GT 18L) AND (NewGridXY[1, *] GE 0.0), NonLower2Cells1count)
NONLower2Cells2 = WHERE((RAS[12, *] GT 18L) AND (NewGridXY[2, *] GE 0.0), NonLower2Cells2count)
NONLower2Cells3 = WHERE((RAS[12, *] GT 18L) AND (NewGridXY[3, *] GE 0.0), NonLower2Cells3count)
NONLower2Cells4 = WHERE((RAS[12, *] GT 18L) AND (NewGridXY[4, *] GE 0.0), NonLower2Cells4count)
NONLower2Cells5 = WHERE((RAS[12, *] GT 18L) AND (NewGridXY[5, *] GE 0.0), NonLower2Cells5count)
NONLower2Cells6 = WHERE((RAS[12, *] GT 18L) AND (NewGridXY[6, *] GE 0.0), NonLower2Cells6count)
NONLower2Cells7 = WHERE((RAS[12, *] GT 18L) AND (NewGridXY[7, *] GE 0.0), NonLower2Cells7count)
NONLower2Cells8 = WHERE((RAS[12, *] GT 18L) AND (NewGridXY[8, *] GE 0.0), NonLower2Cells8count)
...
; current layer -1
Lower1Cells0 = WHERE((RAS[12, *] LE 19L) AND (NewGridXY[0, *] GE 0.0), Lower1Cells0count)
Lower1Cells1 = WHERE((RAS[12, *] LE 19L) AND (NewGridXY[1, *] GE 0.0), Lower1Cells1count)
Lower1Cells2 = WHERE((RAS[12, *] LE 19L) AND (NewGridXY[2, *] GE 0.0), Lower1Cells2count)
Lower1Cells3 = WHERE((RAS[12, *] LE 19L) AND (NewGridXY[3, *] GE 0.0), Lower1Cells3count)
Lower1Cells4 = WHERE((RAS[12, *] LE 19L) AND (NewGridXY[4, *] GE 0.0), Lower1Cells4count)
Lower1Cells5 = WHERE((RAS[12, *] LE 19L) AND (NewGridXY[5, *] GE 0.0), Lower1Cells5count)
Lower1Cells6 = WHERE((RAS[12, *] LE 19L) AND (NewGridXY[6, *] GE 0.0), Lower1Cells6count)
Lower1Cells7 = WHERE((RAS[12, *] LE 19L) AND (NewGridXY[7, *] GE 0.0), Lower1Cells7count)
Lower1Cells8 = WHERE((RAS[12, *] LE 19L) AND (NewGridXY[8, *] GE 0.0), Lower1Cells8count)
...
NONLower1Cells0 = WHERE((RAS[12, *] GT 19L) AND (NewGridXY[0, *] GE 0.0), NonLower1Cells0count)
NONLower1Cells1 = WHERE((RAS[12, *] GT 19L) AND (NewGridXY[1, *] GE 0.0), NonLower1Cells1count)
NONLower1Cells2 = WHERE((RAS[12, *] GT 19L) AND (NewGridXY[2, *] GE 0.0), NonLower1Cells2count)
NONLower1Cells3 = WHERE((RAS[12, *] GT 19L) AND (NewGridXY[3, *] GE 0.0), NonLower1Cells3count)
NONLower1Cells4 = WHERE((RAS[12, *] GT 19L) AND (NewGridXY[4, *] GE 0.0), NonLower1Cells4count)
NONLower1Cells5 = WHERE((RAS[12, *] GT 19L) AND (NewGridXY[5, *] GE 0.0), NonLower1Cells5count)
NONLower1Cells6 = WHERE((RAS[12, *] GT 19L) AND (NewGridXY[6, *] GE 0.0), NonLower1Cells6count)
NONLower1Cells7 = WHERE((RAS[12, *] GT 19L) AND (NewGridXY[7, *] GE 0.0), NonLower1Cells7count)
NONLower1Cells8 = WHERE((RAS[12, *] GT 19L) AND (NewGridXY[8, *] GE 0.0), NonLower1Cells8count)
...
; current layer +1
Upper1Cells0 = WHERE((RAS[12, *] GE 2L) AND (NewGridXY[0, *] GE 0.0), Upper1Cells0count)
Upper1Cells1 = WHERE((RAS[12, *] GE 2L) AND (NewGridXY[1, *] GE 0.0), Upper1Cells1count)
Upper1Cells2 = WHERE((RAS[12, *] GE 2L) AND (NewGridXY[2, *] GE 0.0), Upper1Cells2count)
Upper1Cells3 = WHERE((RAS[12, *] GE 2L) AND (NewGridXY[3, *] GE 0.0), Upper1Cells3count)
Upper1Cells4 = WHERE((RAS[12, *] GE 2L) AND (NewGridXY[4, *] GE 0.0), Upper1Cells4count)
Upper1Cells5 = WHERE((RAS[12, *] GE 2L) AND (NewGridXY[5, *] GE 0.0), Upper1Cells5count)
Upper1Cells6 = WHERE((RAS[12, *] GE 2L) AND (NewGridXY[6, *] GE 0.0), Upper1Cells6count)
Upper1Cells7 = WHERE((RAS[12, *] GE 2L) AND (NewGridXY[7, *] GE 0.0), Upper1Cells7count)
Upper1Cells8 = WHERE((RAS[12, *] GE 2L) AND (NewGridXY[8, *] GE 0.0), Upper1Cells8count)
...
NONUpper1Cells0 = WHERE((RAS[12, *] LT 2L) AND (NewGridXY[0, *] GE 0.0), NonUpper1Cells0count)
NONUpper1Cells1 = WHERE((RAS[12, *] LT 2L) AND (NewGridXY[1, *] GE 0.0), NonUpper1Cells1count)
NONUpper1Cells2 = WHERE((RAS[12, *] LT 2L) AND (NewGridXY[2, *] GE 0.0), NonUpper1Cells2count)
NONUpper1Cells3 = WHERE((RAS[12, *] LT 2L) AND (NewGridXY[3, *] GE 0.0), NonUpper1Cells3count)
NONUpper1Cells4 = WHERE((RAS[12, *] LT 2L) AND (NewGridXY[4, *] GE 0.0), NonUpper1Cells4count)
NONUpper1Cells5 = WHERE((RAS[12, *] LT 2L) AND (NewGridXY[5, *] GE 0.0), NonUpper1Cells5count)
NONUpper1Cells6 = WHERE((RAS[12, *] LT 2L) AND (NewGridXY[6, *] GE 0.0), NonUpper1Cells6count)
NONUpper1Cells7 = WHERE((RAS[12, *] LT 2L) AND (NewGridXY[7, *] GE 0.0), NonUpper1Cells7count)
NONUpper1Cells8 = WHERE((RAS[12, *] LT 2L) AND (NewGridXY[8, *] GE 0.0), NonUpper1Cells8count)
...
; current layer +2
Upper2Cells0 = WHERE((RAS[12, *] GE 3L) AND (NewGridXY[0, *] GE 0.0), Upper2Cells0count)
Upper2Cells1 = WHERE((RAS[12, *] GE 3L) AND (NewGridXY[1, *] GE 0.0), Upper2Cells1count)
Upper2Cells2 = WHERE((RAS[12, *] GE 3L) AND (NewGridXY[2, *] GE 0.0), Upper2Cells2count)
Upper2Cells3 = WHERE((RAS[12, *] GE 3L) AND (NewGridXY[3, *] GE 0.0), Upper2Cells3count)
Upper2Cells4 = WHERE((RAS[12, *] GE 3L) AND (NewGridXY[4, *] GE 0.0), Upper2Cells4count)
Upper2Cells5 = WHERE((RAS[12, *] GE 3L) AND (NewGridXY[5, *] GE 0.0), Upper2Cells5count)
Upper2Cells6 = WHERE((RAS[12, *] GE 3L) AND (NewGridXY[6, *] GE 0.0), Upper2Cells6count)
Upper2Cells7 = WHERE((RAS[12, *] GE 3L) AND (NewGridXY[7, *] GE 0.0), Upper2Cells7count)
Upper2Cells8 = WHERE((RAS[12, *] GE 3L) AND (NewGridXY[8, *] GE 0.0), Upper2Cells8count)
...
NONUpper2Cells0 = WHERE((RAS[12, *] LT 3L) AND (NewGridXY[0, *] GE 0.0), NonUpper2Cells0count)
NONUpper2Cells1 = WHERE((RAS[12, *] LT 3L) AND (NewGridXY[1, *] GE 0.0), NonUpper2Cells1count)
NONUpper2Cells2 = WHERE((RAS[12, *] LT 3L) AND (NewGridXY[2, *] GE 0.0), NonUpper2Cells2count)
NONUpper2Cells3 = WHERE((RAS[12, *] LT 3L) AND (NewGridXY[3, *] GE 0.0), NonUpper2Cells3count)
NONUpper2Cells4 = WHERE((RAS[12, *] LT 3L) AND (NewGridXY[4, *] GE 0.0), NonUpper2Cells4count)
NONUpper2Cells5 = WHERE((RAS[12, *] LT 3L) AND (NewGridXY[5, *] GE 0.0), NonUpper2Cells5count)
NONUpper2Cells6 = WHERE((RAS[12, *] LT 3L) AND (NewGridXY[6, *] GE 0.0), NonUpper2Cells6count)
NONUpper2Cells7 = WHERE((RAS[12, *] LT 3L) AND (NewGridXY[7, *] GE 0.0), NonUpper2Cells7count)
NONUpper2Cells8 = WHERE((RAS[12, *] LT 3L) AND (NewGridXY[8, *] GE 0.0), NonUpper2Cells8count)
...
; current layer +3
Upper3Cells0 = WHERE((RAS[12, *] GE 4L) AND (NewGridXY[0, *] GE 0.0), Upper3Cells0count)
Upper3Cells1 = WHERE((RAS[12, *] GE 4L) AND (NewGridXY[1, *] GE 0.0), Upper3Cells1count)
Upper3Cells2 = WHERE((RAS[12, *] GE 4L) AND (NewGridXY[2, *] GE 0.0), Upper3Cells2count)
Upper3Cells3 = WHERE((RAS[12, *] GE 4L) AND (NewGridXY[3, *] GE 0.0), Upper3Cells3count)
Upper3Cells4 = WHERE((RAS[12, *] GE 4L) AND (NewGridXY[4, *] GE 0.0), Upper3Cells4count)
Upper3Cells5 = WHERE((RAS[12, *] GE 4L) AND (NewGridXY[5, *] GE 0.0), Upper3Cells5count)
Upper3Cells6 = WHERE((RAS[12, *] GE 4L) AND (NewGridXY[6, *] GE 0.0), Upper3Cells6count)
Upper3Cells7 = WHERE((RAS[12, *] GE 4L) AND (NewGridXY[7, *] GE 0.0), Upper3Cells7count)
Upper3Cells8 = WHERE((RAS[12, *] GE 4L) AND (NewGridXY[8, *] GE 0.0), Upper3Cells8count)
...
NONUpper3Cells0 = WHERE((RAS[12, *] LT 4L) AND (NewGridXY[0, *] GE 0.0), NonUpper3Cells0count)
NONUpper3Cells1 = WHERE((RAS[12, *] LT 4L) AND (NewGridXY[1, *] GE 0.0), NonUpper3Cells1count)
NONUpper3Cells2 = WHERE((RAS[12, *] LT 4L) AND (NewGridXY[2, *] GE 0.0), NonUpper3Cells2count)
NONUpper3Cells3 = WHERE((RAS[12, *] LT 4L) AND (NewGridXY[3, *] GE 0.0), NonUpper3Cells3count)
NONUpper3Cells4 = WHERE((RAS[12, *] LT 4L) AND (NewGridXY[4, *] GE 0.0), NonUpper3Cells4count)
NONUpper3Cells5 = WHERE((RAS[12, *] LT 4L) AND (NewGridXY[5, *] GE 0.0), NonUpper3Cells5count)
NONUpper3Cells6 = WHERE((RAS[12, *] LT 4L) AND (NewGridXY[6, *] GE 0.0), NonUpper3Cells6count)
NONUpper3Cells7 = WHERE((RAS[12, *] LT 4L) AND (NewGridXY[7, *] GE 0.0), NonUpper3Cells7count)
NONUpper3Cells8 = WHERE((RAS[12, *] LT 4L) AND (NewGridXY[8, *] GE 0.0), NonUpper3Cells8count)
...
DOf1 = FLTARR(9+6, nRAS)
DOf2 = FLTARR(9+6, nRAS)
DOf3 = FLTARR(9+6, nRAS)
DOf = FLTARR(9+6, nRAS)
Tf = FLTARR(9+6, nRAS)
; DO
DOacclim = RASacclDO(RAS[29, *], RAS[28, *], RAS[20, *], RAS[27, *], RAS[26, *], RAS[19, *], ts, RAS[1, *], RAS[2, *], nRAS, RAS[63, *]); MOVEMENT PARAMETER 
...
; Light
litfac = FLTARR(15, nRAS); a multiplication factor to include the effect of light intensity
t1 = FLTARR(15, nRAS)
t2 = FLTARR(15, nRAS)
...
EnvironHVDO = FLTARR(15, nRAS)
EnvironHVT = FLTARR(15, nRAS)
EnvironHVL = FLTARR(15, nRAS)
;EnvironHVDO[0, *] = DOUBLE(DOf[0, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVDO[1, *] = DOUBLE(DOf[1, *] * RANDOMU(seed, nRAS, /DOUBLE))
;EnvironHVDO[2, *] = DOUBLE(DOf[2, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVDO[3, *] = DOUBLE(DOf[3, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVDO[4, *] = DOUBLE(DOf[4, *] * RANDOMU(seed, nRAS, /DOUBLE))
;EnvironHVDO[5, *] = DOUBLE(DOf[5, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVDO[6, *] = DOUBLE(DOf[6, *] * RANDOMU(seed, nRAS, /DOUBLE))
;EnvironHVDO[7, *] = DOUBLE(DOf[7, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVDO[8, *] = DOUBLE(DOf[8, *] * RANDOMU(seed, nRAS, /DOUBLE))

EnvironHVDO[9, *] = DOUBLE(DOf[9, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVDO[10, *] = DOUBLE(DOf[10, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVDO[11, *] = DOUBLE(DOf[11, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVDO[12, *] = DOUBLE(DOf[12, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVDO[13, *] = DOUBLE(DOf[13, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVDO[14, *] = DOUBLE(DOf[14, *] * RANDOMU(seed, nRAS, /DOUBLE))
...
EnvironHVT[1, *] = DOUBLE(Tf[1, *] * RANDOMU(seed, nRAS, /DOUBLE))
;EnvironHVT[2, *] = DOUBLE(Tf[2, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVT[3, *] = DOUBLE(Tf[3, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVT[4, *] = DOUBLE(Tf[4, *] * RANDOMU(seed, nRAS, /DOUBLE))
;EnvironHVT[5, *] = DOUBLE(Tf[5, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVT[6, *] = DOUBLE(Tf[6, *] * RANDOMU(seed, nRAS, /DOUBLE))
;EnvironHVT[7, *] = DOUBLE(Tf[7, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVT[8, *] = DOUBLE(Tf[8, *] * RANDOMU(seed, nRAS, /DOUBLE))

EnvironHVT[9, *] = DOUBLE(Tf[9, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVT[10, *] = DOUBLE(Tf[10, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVT[11, *] = DOUBLE(Tf[11, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVT[12, *] = DOUBLE(Tf[12, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVT[13, *] = DOUBLE(Tf[13, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVT[14, *] = DOUBLE(Tf[14, *] * RANDOMU(seed, nRAS, /DOUBLE))

;EnvironHVT[0, *] = DOUBLE(Tf[0, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVL[1, *] = DOUBLE(litfac[1, *] * RANDOMU(seed, nRAS, /DOUBLE))
;EnvironHVT[2, *] = DOUBLE(Tf[2, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVL[3, *] = DOUBLE(litfac[3, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVL[4, *] = DOUBLE(Tf[4, *] * RANDOMU(seed, nRAS, /DOUBLE))
;EnvironHVL[5, *] = DOUBLE(litfac[5, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVL[6, *] = DOUBLE(litfac[6, *] * RANDOMU(seed, nRAS, /DOUBLE))
;EnvironHVT[7, *] = DOUBLE(Tf[7, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVL[8, *] = DOUBLE(litfac[8, *] * RANDOMU(seed, nRAS, /DOUBLE))

EnvironHVL[9, *] = DOUBLE(litfac[9, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVL[10, *] = DOUBLE(litfac[10, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVL[11, *] = DOUBLE(litfac[11, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVL[12, *] = DOUBLE(litfac[12, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVL[13, *] = DOUBLE(litfac[13, *] * RANDOMU(seed, nRAS, /DOUBLE))
EnvironHVL[14, *] = DOUBLE(litfac[14, *] * RANDOMU(seed, nRAS, /DOUBLE))
...
Calpha = FLTARR(m, nRAS)
Calpha[0, *] = 193499 * RAS[1, *]^(-7.64); for rotifers
Calpha[1, *] = 0.272 * ALOG(RAS[1, *]) - 0.3834; for calanoids
Calpha[2, *] = 0.4 / (1.0 + ((0.4 / 0.09) - 1.0) * EXP(-13.0 * 0.031 * RAS[1, *]))^(1.0 / 0.031) ; for cladocerans
PL3 = WHERE((PL[3] / RAS[1, *]) LE 0.20, pl3count, complement = pl3c, ncomplement = pl3ccount)
IF (pl3count GT 0.0) THEN Calpha[3, PL3] = ABS(0.50 - 1.75 * (PL[3,PL3] / RAS[1, PL3]))
IF (pl3ccount GT 0.0) THEN Calpha[3, PL3c] = 0.00 ;for benthic prey for flounder by Rose et al. 1996 
Length4 = WHERE(RAS[1, *] GT 60.0, L4count, complement = L4c, ncomplement = L4ccount)
...
PL5a = WHERE((PL[5] / RAS[1, *]) LT 0.20, pl5acount, complement = pl5ac, ncomplement = pl5account)
...
TOT = FLTARR(9+6, nRAS); total number of all prey atacked and captured
t = FLTARR(m*(9+6), nRAS); total number of each prey atacked and captured
;t[0,*] = (Calpha[0,*] * dens[0,*])
t[1,*] = (Calpha[0,*] * dens[1,*])
;t[2,*] = (Calpha[0,*] * dens[2,*])
t[3,*] = (Calpha[0,*] * dens[3,*])
...
preyTOT = FLTARR(nRAS)
preyTOT2 = FLTARR(9, nRAS)
; For vertical movement
preyTOT3 = FLTARR(nRAS)
preyTOT4 = FLTARR(7, nRAS)
...
EnvironHVprey = FLTARR(9+7, nRAS)
; For horizontal movement
;EnvironHVprey[0, *] = (TOT[0, *] / preyTOT2[0, *]) * RANDOMU(seed, nYP, /DOUBLE)
EnvironHVprey[1, *] = (TOT[1, *] / preyTOT2[1, *]) * RANDOMU(seed, nRAS, /DOUBLE)
;EnvironHVprey[2, *] = (TOT[2, *] / preyTOT2[2, *]) * RANDOMU(seed, nYP, /DOUBLE)
EnvironHVprey[3, *] = (TOT[3, *] / preyTOT2[3, *]) * RANDOMU(seed, nRAS, /DOUBLE)
EnvironHVprey[4, *] = (TOT[4, *] / preyTOT2[4, *]) * RANDOMU(seed, nRAS, /DOUBLE)
;EnvironHVprey[5, *] = (TOT[5, *] / preyTOT2[5, *]) * RANDOMU(seed, nYP, /DOUBLE)
EnvironHVprey[6, *] = (TOT[6, *] / preyTOT2[6, *]) * RANDOMU(seed, nRAS, /DOUBLE)
;EnvironHVprey[7, *] = (TOT[7, *] / preyTOT2[7, *]) * RANDOMU(seed, nYP, /DOUBLE)
EnvironHVprey[8, *] = (TOT[8, *] / preyTOT2[8, *]) * RANDOMU(seed, nRAS, /DOUBLE)
; For vertical movement
EnvironHVprey[9, *] = (TOT[9, *] / preyTOT4[1, *]) * RANDOMU(seed, nRAS, /DOUBLE)
EnvironHVprey[10, *] = (TOT[10, *] / preyTOT4[2, *]) * RANDOMU(seed, nRAS, /DOUBLE)
EnvironHVprey[11, *] = (TOT[11, *] / preyTOT4[3, *]) * RANDOMU(seed, nRAS, /DOUBLE)
EnvironHVprey[12, *] = (TOT[12, *] / preyTOT4[4, *]) * RANDOMU(seed, nRAS, /DOUBLE)
EnvironHVprey[13, *] = (TOT[13, *] / preyTOT4[5, *]) * RANDOMU(seed, nRAS, /DOUBLE)
EnvironHVprey[14, *] = (TOT[14, *] / preyTOT4[6, *]) * RANDOMU(seed, nRAS, /DOUBLE)
EnvironHVprey[15, *] = (TOT[8, *] / preyTOT4[0, *]) * RANDOMU(seed, nRAS, /DOUBLE)
;PRINT, 'EnvironHVprey =', EnvironHVprey

EnvironHVSum = FLTARR(9+7, nRAS)
...
Gutfull2 = (100.- (RAS[60, *] < 100.0))/100.; < (1./3.); * RANDOMU(seed, nRAS, /DOUBLE)
...
;zMovePos = FLTARR(nRAS)
;zMoveNeg = FLTARR(nRAS)
;zMoveNon = FLTARR(nRAS)
;FOR iv = 0L, nRAS - 1L DO BEGIN
;  ; move upward (11)
...
;PRINT, Length
SS = FLTARR(nRAS)
S = FLTARR(nRAS); NEED A SS FINCTION OFR EMERALD SHINER
l = WHERE(RAS[1, *] LT 20.0, lcount, complement = ll, ncomplement = llcount)
IF (lcount GT 0.0) THEN $
S[l] = 3.0227 * ALOG(RAS[1, l]) - 4.6273; for walleye <20mm, Houde, 1969 
 ;SS equation based on data from Houde 1969 in body lengths/sec
IF (llcount GT 0.0) THEN $
S[ll] = 1000 * (0.263 + 0.72 * RAS[1, ll] + 0.012 * RAS[26, ll]); for walleye >20mm; Peake et al., 2000
...
;PRINT, newinput[12:15, RAS[14, *]]

; Calculate realized swimming speed (mm/s) in xyz-dimensions
MoveSpeed = FLTARR(5, nRAS)
MoveSpeed[0, *] = SS*COS(HorOriAng) * SIN(VerOriAng) * RANDOMU(seed, nRAS, /DOUBLE) + newinput[12, RAS[14, *]]; with a random comopnent
MoveSpeed[1, *] = SS*SIN(HorOriAng) * SIN(VerOriAng) * RANDOMU(seed, nRAS, /DOUBLE) + newinput[13, RAS[14, *]];
IF (Lcount GT 0.0) THEN MoveSpeed[2, L] = SS[L] * COS(VerOriAng[L]) * RANDOMU(seed, Lcount, /DOUBLE)+ newinput[14, RAS[14, L]]; VERTICAL DIRECTION
IF (LLcount GT 0.0) THEN MoveSpeed[2, LL] = SS[LL]/(ts*6.) * COS(VerOriAng[LL]) * RANDOMU(seed, LLcount, /DOUBLE)+ newinput[14, RAS[14, LL]]; VERTICAL DIRECTION
...
MoveSpeed[4, *] = (0.102*(RAS[1, *]/39.10/EXP(0.330)) + 30.3) * 10.0
...
xNewLocWithinCell = FLTARR(nRAS)
yNewLocWithinCell = FLTARR(nRAS)
zNewLocWithinCell = FLTARR(nRAS)
VerSize = FLTARR(nRAS)
VerSize[*] = (Grid2D[3, RAS[13, *]]/20.)*1000.; in mm
...
;IF xyMoveOut1count GT 0.0 THEN BEGIN; XYloc = 1
  IF zMoveOut11count GT 0.0 THEN RAS[14, zMoveOut11] = LocHV[54, zMoveOut11];zMove[zMoveOut1] = 9
  IF zMoveOut12count GT 0.0 THEN RAS[14, zMoveOut12] = LocHV[54, zMoveOut12];zMove[zMoveOut2] = 9
  IF zMoveOut13count GT 0.0 THEN RAS[14, zMoveOut13] = LocHV[45, zMoveOut13];zMove[zMoveOut3] = 10
  IF zMoveOut14count GT 0.0 THEN RAS[14, zMoveOut14] = LocHV[36, zMoveOut14];zMove[zMoveOut4] = 11
  IF zMoveOut15count GT 0.0 THEN RAS[14, zMoveOut15] = LocHV[0, zMoveOut15];zMove[zMoveOut5] = 15
  IF zMoveOut16count GT 0.0 THEN RAS[14, zMoveOut16] = LocHV[27, zMoveOut16];zMove[zMoveOut6] = 12
  IF zMoveOut17count GT 0.0 THEN RAS[14, zMoveOut17] = LocHV[18, zMoveOut17];zMove[zMoveOut7] = 13
  IF zMoveOut18count GT 0.0 THEN RAS[14, zMoveOut18] = LocHV[9, zMoveOut18];zMove[zMoveOut8] = 14
  IF zMoveOut19count GT 0.0 THEN RAS[14, zMoveOut19] = LocHV[9, zMoveOut19];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut1', xyMoveOut1
;IF xyMoveOut2count GT 0.0 THEN BEGIN; XYloc = 2
  IF zMoveOut21count GT 0.0 THEN RAS[14, zMoveOut21] = LocHV[55, zMoveOut21];zMove[zMoveOut1] = 9
  IF zMoveOut22count GT 0.0 THEN RAS[14, zMoveOut22] = LocHV[55, zMoveOut22];zMove[zMoveOut2] = 9
  IF zMoveOut23count GT 0.0 THEN RAS[14, zMoveOut23] = LocHV[46, zMoveOut23];zMove[zMoveOut3] = 10
  IF zMoveOut24count GT 0.0 THEN RAS[14, zMoveOut24] = LocHV[37, zMoveOut24];zMove[zMoveOut4] = 11
  IF zMoveOut25count GT 0.0 THEN RAS[14, zMoveOut25] = LocHV[1, zMoveOut25];zMove[zMoveOut5] = 15
  IF zMoveOut26count GT 0.0 THEN RAS[14, zMoveOut26] = LocHV[28, zMoveOut26];zMove[zMoveOut6] = 12
  IF zMoveOut27count GT 0.0 THEN RAS[14, zMoveOut27] = LocHV[19, zMoveOut27];zMove[zMoveOut7] = 13
  IF zMoveOut28count GT 0.0 THEN RAS[14, zMoveOut28] = LocHV[10, zMoveOut28];zMove[zMoveOut8] = 14
  IF zMoveOut29count GT 0.0 THEN RAS[14, zMoveOut29] = LocHV[10, zMoveOut29];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut2', xyMoveOut2
;IF xyMoveOut3count GT 0.0 THEN BEGIN; XYloc = 3
  IF zMoveOut31count GT 0.0 THEN RAS[14, zMoveOut31] = LocHV[56, zMoveOut31];zMove[zMoveOut1] = 9
  IF zMoveOut32count GT 0.0 THEN RAS[14, zMoveOut32] = LocHV[56, zMoveOut32];zMove[zMoveOut2] = 9
  IF zMoveOut33count GT 0.0 THEN RAS[14, zMoveOut33] = LocHV[47, zMoveOut33];zMove[zMoveOut3] = 10
  IF zMoveOut34count GT 0.0 THEN RAS[14, zMoveOut34] = LocHV[38, zMoveOut34];zMove[zMoveOut4] = 11
  IF zMoveOut35count GT 0.0 THEN RAS[14, zMoveOut35] = LocHV[2, zMoveOut35];zMove[zMoveOut5] = 15
  IF zMoveOut36count GT 0.0 THEN RAS[14, zMoveOut36] = LocHV[29, zMoveOut36];zMove[zMoveOut6] = 12
  IF zMoveOut37count GT 0.0 THEN RAS[14, zMoveOut37] = LocHV[20, zMoveOut37];zMove[zMoveOut7] = 13
  IF zMoveOut38count GT 0.0 THEN RAS[14, zMoveOut38] = LocHV[11, zMoveOut38];zMove[zMoveOut8] = 14
  IF zMoveOut39count GT 0.0 THEN RAS[14, zMoveOut39] = LocHV[11, zMoveOut39];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut3', xyMoveOut3
;IF xyMoveOut4count GT 0.0 THEN BEGIN; XYloc = 4
  IF zMoveOut41count GT 0.0 THEN RAS[14, zMoveOut41] = LocHV[57, zMoveOut41];zMove[zMoveOut1] = 9
  IF zMoveOut42count GT 0.0 THEN RAS[14, zMoveOut42] = LocHV[57, zMoveOut42];zMove[zMoveOut2] = 9
  IF zMoveOut43count GT 0.0 THEN RAS[14, zMoveOut43] = LocHV[48, zMoveOut43];zMove[zMoveOut3] = 10
  IF zMoveOut44count GT 0.0 THEN RAS[14, zMoveOut44] = LocHV[39, zMoveOut44];zMove[zMoveOut4] = 11
  IF zMoveOut45count GT 0.0 THEN RAS[14, zMoveOut45] = LocHV[3, zMoveOut45];zMove[zMoveOut5] = 15
  IF zMoveOut46count GT 0.0 THEN RAS[14, zMoveOut46] = LocHV[30, zMoveOut46];zMove[zMoveOut6] = 12
  IF zMoveOut47count GT 0.0 THEN RAS[14, zMoveOut47] = LocHV[21, zMoveOut47];zMove[zMoveOut7] = 13
  IF zMoveOut48count GT 0.0 THEN RAS[14, zMoveOut48] = LocHV[12, zMoveOut48];zMove[zMoveOut8] = 14
  IF zMoveOut49count GT 0.0 THEN RAS[14, zMoveOut49] = LocHV[12, zMoveOut49];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut4', xyMoveOut4
;IF xyMoveOut5count GT 0.0 THEN BEGIN; XYloc = 5
  IF zMoveOut51count GT 0.0 THEN RAS[14, zMoveOut51] = LocHV[58, zMoveOut51];zMove[zMoveOut1] = 9
  IF zMoveOut52count GT 0.0 THEN RAS[14, zMoveOut52] = LocHV[58, zMoveOut52];zMove[zMoveOut2] = 9
  IF zMoveOut53count GT 0.0 THEN RAS[14, zMoveOut53] = LocHV[49, zMoveOut53];zMove[zMoveOut3] = 10
  IF zMoveOut54count GT 0.0 THEN RAS[14, zMoveOut54] = LocHV[40, zMoveOut54];zMove[zMoveOut4] = 11
  IF zMoveOut55count GT 0.0 THEN RAS[14, zMoveOut55] = LocHV[4, zMoveOut55];zMove[zMoveOut5] = 15
  IF zMoveOut56count GT 0.0 THEN RAS[14, zMoveOut56] = LocHV[31, zMoveOut56];zMove[zMoveOut6] = 12
  IF zMoveOut57count GT 0.0 THEN RAS[14, zMoveOut57] = LocHV[22, zMoveOut57];zMove[zMoveOut7] = 13
  IF zMoveOut58count GT 0.0 THEN RAS[14, zMoveOut58] = LocHV[13, zMoveOut58];zMove[zMoveOut8] = 14
  IF zMoveOut59count GT 0.0 THEN RAS[14, zMoveOut59] = LocHV[13, zMoveOut59];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut5', xyMoveOut5
;IF xyMoveOut6count GT 0.0 THEN BEGIN; XYloc = 6
  IF zMoveOut61count GT 0.0 THEN RAS[14, zMoveOut61] = LocHV[59, zMoveOut61];zMove[zMoveOut1] = 9
  IF zMoveOut62count GT 0.0 THEN RAS[14, zMoveOut62] = LocHV[59, zMoveOut62];zMove[zMoveOut2] = 9
  IF zMoveOut63count GT 0.0 THEN RAS[14, zMoveOut63] = LocHV[50, zMoveOut63];zMove[zMoveOut3] = 10
  IF zMoveOut64count GT 0.0 THEN RAS[14, zMoveOut64] = LocHV[41, zMoveOut64];zMove[zMoveOut4] = 11
  IF zMoveOut65count GT 0.0 THEN RAS[14, zMoveOut65] = LocHV[5, zMoveOut65];zMove[zMoveOut5] = 15
  IF zMoveOut66count GT 0.0 THEN RAS[14, zMoveOut66] = LocHV[32, zMoveOut66];zMove[zMoveOut6] = 12
  IF zMoveOut67count GT 0.0 THEN RAS[14, zMoveOut67] = LocHV[23, zMoveOut67];zMove[zMoveOut7] = 13
  IF zMoveOut68count GT 0.0 THEN RAS[14, zMoveOut68] = LocHV[14, zMoveOut68];zMove[zMoveOut8] = 14
  IF zMoveOut69count GT 0.0 THEN RAS[14, zMoveOut69] = LocHV[14, zMoveOut69];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut6', xyMoveOut6
;IF xyMoveOut7count GT 0.0 THEN BEGIN; XYloc = 7
  IF zMoveOut71count GT 0.0 THEN RAS[14, zMoveOut71] = LocHV[60, zMoveOut71];zMove[zMoveOut1] = 9
  IF zMoveOut72count GT 0.0 THEN RAS[14, zMoveOut72] = LocHV[60, zMoveOut72];zMove[zMoveOut2] = 9
  IF zMoveOut73count GT 0.0 THEN RAS[14, zMoveOut73] = LocHV[51, zMoveOut73];zMove[zMoveOut3] = 10
  IF zMoveOut74count GT 0.0 THEN RAS[14, zMoveOut74] = LocHV[42, zMoveOut74];zMove[zMoveOut4] = 11
  IF zMoveOut75count GT 0.0 THEN RAS[14, zMoveOut75] = LocHV[6, zMoveOut75];zMove[zMoveOut5] = 15
  IF zMoveOut76count GT 0.0 THEN RAS[14, zMoveOut76] = LocHV[33, zMoveOut76];zMove[zMoveOut6] = 12
  IF zMoveOut77count GT 0.0 THEN RAS[14, zMoveOut77] = LocHV[24, zMoveOut77];zMove[zMoveOut7] = 13
  IF zMoveOut78count GT 0.0 THEN RAS[14, zMoveOut78] = LocHV[15, zMoveOut78];zMove[zMoveOut8] = 14
  IF zMoveOut79count GT 0.0 THEN RAS[14, zMoveOut79] = LocHV[15, zMoveOut79];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut7', xyMoveOut7
;IF xyMoveOut8count GT 0.0 THEN BEGIN; XYloc = 8
  IF zMoveOut81count GT 0.0 THEN RAS[14, zMoveOut81] = LocHV[61, zMoveOut81];zMove[zMoveOut1] = 9
  IF zMoveOut82count GT 0.0 THEN RAS[14, zMoveOut82] = LocHV[61, zMoveOut82];zMove[zMoveOut2] = 9
  IF zMoveOut83count GT 0.0 THEN RAS[14, zMoveOut83] = LocHV[52, zMoveOut83];zMove[zMoveOut3] = 10
  IF zMoveOut84count GT 0.0 THEN RAS[14, zMoveOut84] = LocHV[43, zMoveOut84];zMove[zMoveOut4] = 11
  IF zMoveOut85count GT 0.0 THEN RAS[14, zMoveOut85] = LocHV[7, zMoveOut85];zMove[zMoveOut5] = 15
  IF zMoveOut86count GT 0.0 THEN RAS[14, zMoveOut86] = LocHV[34, zMoveOut86];zMove[zMoveOut6] = 12
  IF zMoveOut87count GT 0.0 THEN RAS[14, zMoveOut87] = LocHV[25, zMoveOut87];zMove[zMoveOut7] = 13
  IF zMoveOut88count GT 0.0 THEN RAS[14, zMoveOut88] = LocHV[16, zMoveOut88];zMove[zMoveOut8] = 14
  IF zMoveOut89count GT 0.0 THEN RAS[14, zMoveOut89] = LocHV[16, zMoveOut89];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut8', xyMoveOut8
;IF xyMoveOut9count GT 0.0 THEN BEGIN; XYloc = 9
  IF zMoveOut91count GT 0.0 THEN RAS[14, zMoveOut91] = LocHV[62, zMoveOut91];zMove[zMoveOut1] = 9
  IF zMoveOut92count GT 0.0 THEN RAS[14, zMoveOut92] = LocHV[62, zMoveOut92];zMove[zMoveOut2] = 9
  IF zMoveOut93count GT 0.0 THEN RAS[14, zMoveOut93] = LocHV[53, zMoveOut93];zMove[zMoveOut3] = 10
  IF zMoveOut94count GT 0.0 THEN RAS[14, zMoveOut94] = LocHV[44, zMoveOut94];zMove[zMoveOut4] = 11
  IF zMoveOut95count GT 0.0 THEN RAS[14, zMoveOut95] = LocHV[8, zMoveOut95];zMove[zMoveOut6] = 12
  IF zMoveOut96count GT 0.0 THEN RAS[14, zMoveOut96] = LocHV[35, zMoveOut96];zMove[zMoveOut6] = 12
  IF zMoveOut97count GT 0.0 THEN RAS[14, zMoveOut97] = LocHV[26, zMoveOut97];zMove[zMoveOut7] = 13
  IF zMoveOut98count GT 0.0 THEN RAS[14, zMoveOut98] = LocHV[17, zMoveOut98];zMove[zMoveOut8] = 14
  IF zMoveOut99count GT 0.0 THEN RAS[14, zMoveOut99] = LocHV[17, zMoveOut99];zMove[zMoveOut9] = 14
;ENDIF
...
;PRINT, 'NewInput[2, RAS[14, *]]', transpose(NewInput[2, RAS[14, *]])
;  PRINT, 'YP[10, *]'
;  PRINT, TRANSPOSE(YP[10, *])
;  PRINT, 'YP[11, *]'
;  PRINT, TRANSPOSE(YP[11, *])
