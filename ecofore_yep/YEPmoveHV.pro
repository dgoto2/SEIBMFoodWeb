FUNCTION YEPMoveHV, ts, iHour, YP, nYP, NewInput, TotBenBio, FISHPREY, Grid2D, xOldLocWithinCell, yOldLocWithinCell, $
zOldLocWithinCell, nGridcell, Oxydebt
...
;**********************TEST ONLY*************************************************
;PRO YEPMoveHV, YP, nYP, NewInput, TotBenBio, FISHPREY, Grid2D, xOldLocWithinCell, yOldLocWithinCell, zOldLocWithinCell
;; NEED to change NewInputFiles, YPinitial, YEPacclT, YEPacclDO for testing
;nYP = 50000L; number of SI YP; MOVEMENT PARAMETER 
;nEMS = 500000L;
;nRAS = 500000L;
;nROG = 500000L;
;
...
;
;NpopYP = 50000000L ; number of YEP individuals
;YP = YEPinitial(NpopYP, nYP, TotBenBio, NewInput); MOVEMENT PARAMETER 
;
...
;;DOacclR = YP[29, *]
;;DOacclC = YP[28, *]
;;DOacclim = YEPacclDO(DOacclR, DOacclC, DOa, TacclR, TacclC, Tamb, ts, YP[1, *], Weight, nYP); MOVEMENT PARAMETER 
;
;xOldLocWithinCell = 0.8; YP[39, *]; MOVEMENT PARAMETER 
;yOldLocWithinCell = 0.7; YP[40, *]; MOVEMENT PARAMETER 
;zOldLocWithinCell = YP[41, *]; MOVEMENT PARAMETER 
; ;********************************************************
;; Creat a fish prey array for potential predators, per L
;nGridcell = 77500L
;FISHPREY = FLTARR(21L, nGridcell)
;FISH3DCellIDcount = FLTARR(4, nGridcell)
...
;  IF FISHPREY[1, YP[14, ID]] EQ 0. THEN BEGIN 
;    FISHPREY[1, YP[14, ID]] = YP[1, ID]; LENGTH
;    FISHPREY[2, YP[14, ID]] = YP[2, ID]; WEIGHT
;  ENDIF
;  IF FISHPREY[1, YP[14, ID]] GT 0. THEN BEGIN
;    IF (FISHPREY[1, YP[14, ID]] GT YP[1, ID]) THEN BEGIN
;      FISHPREY[1, YP[14, ID]] = YP[1, ID]
;      FISHPREY[2, YP[14, ID]] = YP[2, ID]
;    ENDIF
;    IF (FISHPREY[1, YP[14, ID]] LT YP[1, ID]) THEN BEGIN
...
;FISHPREY[9, EMS[14, *]] = FISH3DCellIDcount[1, EMS[14, *]]; T the number of superindividuals
...
;FISHPREY[20, *] = TOTAL(FISH3DCellIDcount, 1); TOTAL NUMBER OF SUPERINDIVIDUALS IN A CELL
;
;FISHPREY2 = FISHPREY
;FISHPREY[0:3, *] = FISHPREY[0:3, *]
;FISHPREY[4:7, *] = FISHPREY[5:8, *]
;FISHPREY[8:11, *] = FISHPREY[10:13, *]
;FISHPREY[12:15, *] = FISHPREY[15:18, *]
;;PRINT, FISHPREY
;***********************************************************************************
...
;PRINT, 'Inital location (X, Y, Z, 2D ID, AND 3D ID)'
;PRINT, YP[10:14, *]
...
;IF (ZLocH6count GT 0.0) THEN LocH[6, ZLocH6] = YP[10, ZLocH6]+1L
LocH[7, *] = YP[11, *] 
; 5
LocH[8, *] = YP[10, *] + 1L
...
;IF (ZLocH8count GT 0.0) THEN LocH[8, ZLocH8] = YP[10, ZLocH8]-1L
LocH[9, *] = YP[11, *] 
;PRINT, 'LocH6'
;PRINT, TRANSPOSE(LocH[6:9, 0:200])
...
;PRINT, 'YP[13, *]'
;PRINT, TRANSPOSE(YP[13, 0:100])
;PRINT, 'LOCH21'
;PRINT, TRANSPOSE(LocH[21, 0:100])
;PRINT, 'LOCH22'
;PRINT, TRANSPOSE(LocH[22, 0:100])

; Check if a grid cell exists on both sides in x-dimension
...
NewGridXY[1, *] = YP[13, *]+1L; 2-> CURRENT +1 
...
NewGridXY[6, *] = YP[13, *]-1L; 7 -> CURRENT - 1
NewGridXY[7, *] = NewGridXY[4, *]-1L; 8 -> 5. - 1
;; NewGridXY and YP[13, *] NEED TO BE > 0.0, if not, fish has only one direction ...
;NZNewGridXY3 = WHERE((NewGridXY[3, PLocH21] GT 0.0), NZNewGridXY3count, complement = ZNewGridXY3, ncomplement = ZNewGridXY3count);
;NZYP13 = WHERE((YP[13, *] GT 0.0), NZYP13count, complement = ZYP13, ncomplement = ZYP13count);
;NZNewGridXY4 = WHERE((NewGridXY[4, PLocH22] GT 0.0), NZNewGridXY4count, complement = ZNewGridXY4, ncomplement = ZNewGridXY4count);
;IF (NZNewGridXY3count GT 0.0) THEN NewGridXY[5, NZNewGridXY3] = NewGridXY[3, NZNewGridXY3]-1L; 6 -> 4. -1
;;IF (ZNewGridXY3count GT 0.0) THEN NewGridXY[5, ZNewGridXY3] = NewGridXY[3, ZNewGridXY3]; 6 -> 4. -1
;IF (NZYP13count GT 0.0) THEN NewGridXY[6, NZYP13] = YP[13, NZYP13]-1L; 7 -> CURRENT -1
;;IF (ZYP13count GT 0.0) THEN NewGridXY[6, ZYP13] = YP[13, ZYP13]; 7 -> CURRENT -1
;IF (NZNewGridXY4count GT 0.0) THEN NewGridXY[7, NZNewGridXY4] = NewGridXY[4, NZNewGridXY4]-1L; 8 -> 5. -1
;;IF (ZNewGridXY4count GT 0.0) THEN NewGridXY[7, ZNewGridXY4] = NewGridXY[4, ZNewGridXY4]; 8 -> 5. -1
NewGridXY[8, *] = YP[13, *]; 9 -> CURRENT
;PRINT, 'NewGridXY'
;PRINT, transpose(NewGridXY[*,0:200])
...
LocNH17A = WHERE((NewGridXY[1, *] GE 0.0) AND (Grid2D[0, NewGridXY[1, *]] EQ YP[10, *]), LocNH17Acount, complement = LocH17A, ncomplement = LocH17Acount) ; LocH[7, *]
...
LocNH17B = WHERE((NewGridXY[6, *] GE 0.0) AND (Grid2D[0, NewGridXY[6, *]] EQ YP[10, *]), LocNH17Bcount, complement = LocH17B, ncomplement = LocH17Bcount) ; LocH[7, *]
...
IF (NZNewGridXY0count GT 0.0) THEN LocHV[0, NZNewGridXY0] = (NewGridXY[0, NZNewGridXY0])*20. + (YP[12, NZNewGridXY0]-1.) 
IF (NZNewGridXY1count GT 0.0) THEN LocHV[1, NZNewGridXY1] = (NewGridXY[1, NZNewGridXY1])*20. + (YP[12, NZNewGridXY1]-1.) 
IF (NZNewGridXY2count GT 0.0) THEN LocHV[2, NZNewGridXY2] = (NewGridXY[2, NZNewGridXY2])*20. + (YP[12, NZNewGridXY2]-1.) 
IF (NZNewGridXY3count GT 0.0) THEN LocHV[3, NZNewGridXY3] = (NewGridXY[3, NZNewGridXY3])*20. + (YP[12, NZNewGridXY3]-1.) 
IF (NZNewGridXY4count GT 0.0) THEN LocHV[4, NZNewGridXY4] = (NewGridXY[4, NZNewGridXY4])*20. + (YP[12, NZNewGridXY4]-1.) 
IF (NZNewGridXY5count GT 0.0) THEN LocHV[5, NZNewGridXY5] = (NewGridXY[5, NZNewGridXY5])*20. + (YP[12, NZNewGridXY5]-1.) 
IF (NZNewGridXY6count GT 0.0) THEN LocHV[6, NZNewGridXY6] = (NewGridXY[6, NZNewGridXY6])*20. + (YP[12, NZNewGridXY6]-1.) 
IF (NZNewGridXY7count GT 0.0) THEN LocHV[7, NZNewGridXY7] = (NewGridXY[7, NZNewGridXY7])*20. + (YP[12, NZNewGridXY7]-1.) 
IF (NZNewGridXY8count GT 0.0) THEN LocHV[8, NZNewGridXY8] = (NewGridXY[8, NZNewGridXY8])*20. + (YP[12, NZNewGridXY8]-1.) 
...
; DOWNWARD MOVEMNT
; current layer -3
Lower3Cells0 = WHERE((YP[12, *] LE 17L) AND (NewGridXY[0, *] GE 0.0), Lower3Cells0count)
Lower3Cells1 = WHERE((YP[12, *] LE 17L) AND (NewGridXY[1, *] GE 0.0), Lower3Cells1count)
Lower3Cells2 = WHERE((YP[12, *] LE 17L) AND (NewGridXY[2, *] GE 0.0), Lower3Cells2count)
Lower3Cells3 = WHERE((YP[12, *] LE 17L) AND (NewGridXY[3, *] GE 0.0), Lower3Cells3count)
Lower3Cells4 = WHERE((YP[12, *] LE 17L) AND (NewGridXY[4, *] GE 0.0), Lower3Cells4count)
Lower3Cells5 = WHERE((YP[12, *] LE 17L) AND (NewGridXY[5, *] GE 0.0), Lower3Cells5count)
Lower3Cells6 = WHERE((YP[12, *] LE 17L) AND (NewGridXY[6, *] GE 0.0), Lower3Cells6count)
Lower3Cells7 = WHERE((YP[12, *] LE 17L) AND (NewGridXY[7, *] GE 0.0), Lower3Cells7count)
Lower3Cells8 = WHERE((YP[12, *] LE 17L) AND (NewGridXY[8, *] GE 0.0), Lower3Cells8count)
...
NONLower3Cells0 = WHERE((YP[12, *] GT 17L) AND (NewGridXY[0, *] GE 0.0), NonLower3Cells0count)
NONLower3Cells1 = WHERE((YP[12, *] GT 17L) AND (NewGridXY[1, *] GE 0.0), NonLower3Cells1count)
NONLower3Cells2 = WHERE((YP[12, *] GT 17L) AND (NewGridXY[2, *] GE 0.0), NonLower3Cells2count)
NONLower3Cells3 = WHERE((YP[12, *] GT 17L) AND (NewGridXY[3, *] GE 0.0), NonLower3Cells3count)
NONLower3Cells4 = WHERE((YP[12, *] GT 17L) AND (NewGridXY[4, *] GE 0.0), NonLower3Cells4count)
NONLower3Cells5 = WHERE((YP[12, *] GT 17L) AND (NewGridXY[5, *] GE 0.0), NonLower3Cells5count)
NONLower3Cells6 = WHERE((YP[12, *] GT 17L) AND (NewGridXY[6, *] GE 0.0), NonLower3Cells6count)
NONLower3Cells7 = WHERE((YP[12, *] GT 17L) AND (NewGridXY[7, *] GE 0.0), NonLower3Cells7count)
NONLower3Cells8 = WHERE((YP[12, *] GT 17L) AND (NewGridXY[8, *] GE 0.0), NonLower3Cells8count)
...
; current layer -2
Lower2Cells0 = WHERE((YP[12, *] LE 18L) AND (NewGridXY[0, *] GE 0.0), Lower2Cells0count)
Lower2Cells1 = WHERE((YP[12, *] LE 18L) AND (NewGridXY[1, *] GE 0.0), Lower2Cells1count)
Lower2Cells2 = WHERE((YP[12, *] LE 18L) AND (NewGridXY[2, *] GE 0.0), Lower2Cells2count)
Lower2Cells3 = WHERE((YP[12, *] LE 18L) AND (NewGridXY[3, *] GE 0.0), Lower2Cells3count)
Lower2Cells4 = WHERE((YP[12, *] LE 18L) AND (NewGridXY[4, *] GE 0.0), Lower2Cells4count)
Lower2Cells5 = WHERE((YP[12, *] LE 18L) AND (NewGridXY[5, *] GE 0.0), Lower2Cells5count)
Lower2Cells6 = WHERE((YP[12, *] LE 18L) AND (NewGridXY[6, *] GE 0.0), Lower2Cells6count)
Lower2Cells7 = WHERE((YP[12, *] LE 18L) AND (NewGridXY[7, *] GE 0.0), Lower2Cells7count)
Lower2Cells8 = WHERE((YP[12, *] LE 18L) AND (NewGridXY[8, *] GE 0.0), Lower2Cells8count)
...
NONLower2Cells0 = WHERE((YP[12, *] GT 18L) AND (NewGridXY[0, *] GE 0.0), NonLower2Cells0count)
NONLower2Cells1 = WHERE((YP[12, *] GT 18L) AND (NewGridXY[1, *] GE 0.0), NonLower2Cells1count)
NONLower2Cells2 = WHERE((YP[12, *] GT 18L) AND (NewGridXY[2, *] GE 0.0), NonLower2Cells2count)
NONLower2Cells3 = WHERE((YP[12, *] GT 18L) AND (NewGridXY[3, *] GE 0.0), NonLower2Cells3count)
NONLower2Cells4 = WHERE((YP[12, *] GT 18L) AND (NewGridXY[4, *] GE 0.0), NonLower2Cells4count)
NONLower2Cells5 = WHERE((YP[12, *] GT 18L) AND (NewGridXY[5, *] GE 0.0), NonLower2Cells5count)
NONLower2Cells6 = WHERE((YP[12, *] GT 18L) AND (NewGridXY[6, *] GE 0.0), NonLower2Cells6count)
NONLower2Cells7 = WHERE((YP[12, *] GT 18L) AND (NewGridXY[7, *] GE 0.0), NonLower2Cells7count)
NONLower2Cells8 = WHERE((YP[12, *] GT 18L) AND (NewGridXY[8, *] GE 0.0), NonLower2Cells8count)
...
; current layer -1
Lower1Cells0 = WHERE((YP[12, *] LE 19L) AND (NewGridXY[0, *] GE 0.0), Lower1Cells0count)
Lower1Cells1 = WHERE((YP[12, *] LE 19L) AND (NewGridXY[1, *] GE 0.0), Lower1Cells1count)
Lower1Cells2 = WHERE((YP[12, *] LE 19L) AND (NewGridXY[2, *] GE 0.0), Lower1Cells2count)
Lower1Cells3 = WHERE((YP[12, *] LE 19L) AND (NewGridXY[3, *] GE 0.0), Lower1Cells3count)
Lower1Cells4 = WHERE((YP[12, *] LE 19L) AND (NewGridXY[4, *] GE 0.0), Lower1Cells4count)
Lower1Cells5 = WHERE((YP[12, *] LE 19L) AND (NewGridXY[5, *] GE 0.0), Lower1Cells5count)
Lower1Cells6 = WHERE((YP[12, *] LE 19L) AND (NewGridXY[6, *] GE 0.0), Lower1Cells6count)
Lower1Cells7 = WHERE((YP[12, *] LE 19L) AND (NewGridXY[7, *] GE 0.0), Lower1Cells7count)
Lower1Cells8 = WHERE((YP[12, *] LE 19L) AND (NewGridXY[8, *] GE 0.0), Lower1Cells8count)
...
NONLower1Cells0 = WHERE((YP[12, *] GT 19L) AND (NewGridXY[0, *] GE 0.0), NonLower1Cells0count)
NONLower1Cells1 = WHERE((YP[12, *] GT 19L) AND (NewGridXY[1, *] GE 0.0), NonLower1Cells1count)
NONLower1Cells2 = WHERE((YP[12, *] GT 19L) AND (NewGridXY[2, *] GE 0.0), NonLower1Cells2count)
NONLower1Cells3 = WHERE((YP[12, *] GT 19L) AND (NewGridXY[3, *] GE 0.0), NonLower1Cells3count)
NONLower1Cells4 = WHERE((YP[12, *] GT 19L) AND (NewGridXY[4, *] GE 0.0), NonLower1Cells4count)
NONLower1Cells5 = WHERE((YP[12, *] GT 19L) AND (NewGridXY[5, *] GE 0.0), NonLower1Cells5count)
NONLower1Cells6 = WHERE((YP[12, *] GT 19L) AND (NewGridXY[6, *] GE 0.0), NonLower1Cells6count)
NONLower1Cells7 = WHERE((YP[12, *] GT 19L) AND (NewGridXY[7, *] GE 0.0), NonLower1Cells7count)
NONLower1Cells8 = WHERE((YP[12, *] GT 19L) AND (NewGridXY[8, *] GE 0.0), NonLower1Cells8count)
...
; current layer +1
Upper1Cells0 = WHERE((YP[12, *] GE 2L) AND (NewGridXY[0, *] GE 0.0), Upper1Cells0count)
Upper1Cells1 = WHERE((YP[12, *] GE 2L) AND (NewGridXY[1, *] GE 0.0), Upper1Cells1count)
Upper1Cells2 = WHERE((YP[12, *] GE 2L) AND (NewGridXY[2, *] GE 0.0), Upper1Cells2count)
Upper1Cells3 = WHERE((YP[12, *] GE 2L) AND (NewGridXY[3, *] GE 0.0), Upper1Cells3count)
Upper1Cells4 = WHERE((YP[12, *] GE 2L) AND (NewGridXY[4, *] GE 0.0), Upper1Cells4count)
Upper1Cells5 = WHERE((YP[12, *] GE 2L) AND (NewGridXY[5, *] GE 0.0), Upper1Cells5count)
Upper1Cells6 = WHERE((YP[12, *] GE 2L) AND (NewGridXY[6, *] GE 0.0), Upper1Cells6count)
Upper1Cells7 = WHERE((YP[12, *] GE 2L) AND (NewGridXY[7, *] GE 0.0), Upper1Cells7count)
Upper1Cells8 = WHERE((YP[12, *] GE 2L) AND (NewGridXY[8, *] GE 0.0), Upper1Cells8count)
...
NONUpper1Cells0 = WHERE((YP[12, *] LT 2L) AND (NewGridXY[0, *] GE 0.0), NonUpper1Cells0count)
NONUpper1Cells1 = WHERE((YP[12, *] LT 2L) AND (NewGridXY[1, *] GE 0.0), NonUpper1Cells1count)
NONUpper1Cells2 = WHERE((YP[12, *] LT 2L) AND (NewGridXY[2, *] GE 0.0), NonUpper1Cells2count)
NONUpper1Cells3 = WHERE((YP[12, *] LT 2L) AND (NewGridXY[3, *] GE 0.0), NonUpper1Cells3count)
NONUpper1Cells4 = WHERE((YP[12, *] LT 2L) AND (NewGridXY[4, *] GE 0.0), NonUpper1Cells4count)
NONUpper1Cells5 = WHERE((YP[12, *] LT 2L) AND (NewGridXY[5, *] GE 0.0), NonUpper1Cells5count)
NONUpper1Cells6 = WHERE((YP[12, *] LT 2L) AND (NewGridXY[6, *] GE 0.0), NonUpper1Cells6count)
NONUpper1Cells7 = WHERE((YP[12, *] LT 2L) AND (NewGridXY[7, *] GE 0.0), NonUpper1Cells7count)
NONUpper1Cells8 = WHERE((YP[12, *] LT 2L) AND (NewGridXY[8, *] GE 0.0), NonUpper1Cells8count)
...
; current layer +2
Upper2Cells0 = WHERE((YP[12, *] GE 3L) AND (NewGridXY[0, *] GE 0.0), Upper2Cells0count)
Upper2Cells1 = WHERE((YP[12, *] GE 3L) AND (NewGridXY[1, *] GE 0.0), Upper2Cells1count)
Upper2Cells2 = WHERE((YP[12, *] GE 3L) AND (NewGridXY[2, *] GE 0.0), Upper2Cells2count)
Upper2Cells3 = WHERE((YP[12, *] GE 3L) AND (NewGridXY[3, *] GE 0.0), Upper2Cells3count)
Upper2Cells4 = WHERE((YP[12, *] GE 3L) AND (NewGridXY[4, *] GE 0.0), Upper2Cells4count)
Upper2Cells5 = WHERE((YP[12, *] GE 3L) AND (NewGridXY[5, *] GE 0.0), Upper2Cells5count)
Upper2Cells6 = WHERE((YP[12, *] GE 3L) AND (NewGridXY[6, *] GE 0.0), Upper2Cells6count)
Upper2Cells7 = WHERE((YP[12, *] GE 3L) AND (NewGridXY[7, *] GE 0.0), Upper2Cells7count)
Upper2Cells8 = WHERE((YP[12, *] GE 3L) AND (NewGridXY[8, *] GE 0.0), Upper2Cells8count)
...
NONUpper2Cells0 = WHERE((YP[12, *] LT 3L) AND (NewGridXY[0, *] GE 0.0), NonUpper2Cells0count)
NONUpper2Cells1 = WHERE((YP[12, *] LT 3L) AND (NewGridXY[1, *] GE 0.0), NonUpper2Cells1count)
NONUpper2Cells2 = WHERE((YP[12, *] LT 3L) AND (NewGridXY[2, *] GE 0.0), NonUpper2Cells2count)
NONUpper2Cells3 = WHERE((YP[12, *] LT 3L) AND (NewGridXY[3, *] GE 0.0), NonUpper2Cells3count)
NONUpper2Cells4 = WHERE((YP[12, *] LT 3L) AND (NewGridXY[4, *] GE 0.0), NonUpper2Cells4count)
NONUpper2Cells5 = WHERE((YP[12, *] LT 3L) AND (NewGridXY[5, *] GE 0.0), NonUpper2Cells5count)
NONUpper2Cells6 = WHERE((YP[12, *] LT 3L) AND (NewGridXY[6, *] GE 0.0), NonUpper2Cells6count)
NONUpper2Cells7 = WHERE((YP[12, *] LT 3L) AND (NewGridXY[7, *] GE 0.0), NonUpper2Cells7count)
NONUpper2Cells8 = WHERE((YP[12, *] LT 3L) AND (NewGridXY[8, *] GE 0.0), NonUpper2Cells8count)
...
; current layer +3
Upper3Cells0 = WHERE((YP[12, *] GE 4L) AND (NewGridXY[0, *] GE 0.0), Upper3Cells0count)
Upper3Cells1 = WHERE((YP[12, *] GE 4L) AND (NewGridXY[1, *] GE 0.0), Upper3Cells1count)
Upper3Cells2 = WHERE((YP[12, *] GE 4L) AND (NewGridXY[2, *] GE 0.0), Upper3Cells2count)
Upper3Cells3 = WHERE((YP[12, *] GE 4L) AND (NewGridXY[3, *] GE 0.0), Upper3Cells3count)
Upper3Cells4 = WHERE((YP[12, *] GE 4L) AND (NewGridXY[4, *] GE 0.0), Upper3Cells4count)
Upper3Cells5 = WHERE((YP[12, *] GE 4L) AND (NewGridXY[5, *] GE 0.0), Upper3Cells5count)
Upper3Cells6 = WHERE((YP[12, *] GE 4L) AND (NewGridXY[6, *] GE 0.0), Upper3Cells6count)
Upper3Cells7 = WHERE((YP[12, *] GE 4L) AND (NewGridXY[7, *] GE 0.0), Upper3Cells7count)
Upper3Cells8 = WHERE((YP[12, *] GE 4L) AND (NewGridXY[8, *] GE 0.0), Upper3Cells8count)
...
NONUpper3Cells0 = WHERE((YP[12, *] LT 4L) AND (NewGridXY[0, *] GE 0.0), NonUpper3Cells0count)
NONUpper3Cells1 = WHERE((YP[12, *] LT 4L) AND (NewGridXY[1, *] GE 0.0), NonUpper3Cells1count)
NONUpper3Cells2 = WHERE((YP[12, *] LT 4L) AND (NewGridXY[2, *] GE 0.0), NonUpper3Cells2count)
NONUpper3Cells3 = WHERE((YP[12, *] LT 4L) AND (NewGridXY[3, *] GE 0.0), NonUpper3Cells3count)
NONUpper3Cells4 = WHERE((YP[12, *] LT 4L) AND (NewGridXY[4, *] GE 0.0), NonUpper3Cells4count)
NONUpper3Cells5 = WHERE((YP[12, *] LT 4L) AND (NewGridXY[5, *] GE 0.0), NonUpper3Cells5count)
NONUpper3Cells6 = WHERE((YP[12, *] LT 4L) AND (NewGridXY[6, *] GE 0.0), NonUpper3Cells6count)
NONUpper3Cells7 = WHERE((YP[12, *] LT 4L) AND (NewGridXY[7, *] GE 0.0), NonUpper3Cells7count)
NONUpper3Cells8 = WHERE((YP[12, *] LT 4L) AND (NewGridXY[8, *] GE 0.0), NonUpper3Cells8count)
...
EnvironHV[106, *] = 0.0; invasive species
...
TLL = WHERE((YP[1, *] GE 20.0 ) AND (YP[1, *] LT 80.0 ), TLLcount)
...
TLLL = WHERE((YP[1, *] GE 80.0 ), TLLLcount)
...
; Light
litfac = FLTARR(15, nYP); a multiplication factor to include the effect of light intensity
t1 = FLTARR(15, nYP)
t2 = FLTARR(15, nYP)
...
;EnvironHVT[0, *] = DOUBLE(Tf[0, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVL[1, *] = DOUBLE(litfac[1, *] * RANDOMU(seed, nYP, /DOUBLE))
;EnvironHVT[2, *] = DOUBLE(Tf[2, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVL[3, *] = DOUBLE(litfac[3, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVL[4, *] = DOUBLE(Tf[4, *] * RANDOMU(seed, nYP, /DOUBLE))
;EnvironHVL[5, *] = DOUBLE(litfac[5, *] * RANDOMU(seed, nWAE, /DOUBLE))
EnvironHVL[6, *] = DOUBLE(litfac[6, *] * RANDOMU(seed, nYP, /DOUBLE))
;EnvironHVT[7, *] = DOUBLE(Tf[7, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVL[8, *] = DOUBLE(litfac[8, *] * RANDOMU(seed, nYP, /DOUBLE))

EnvironHVL[9, *] = DOUBLE(litfac[9, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVL[10, *] = DOUBLE(litfac[10, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVL[11, *] = DOUBLE(litfac[11, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVL[12, *] = DOUBLE(litfac[12, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVL[13, *] = DOUBLE(litfac[13, *] * RANDOMU(seed, nYP, /DOUBLE))
EnvironHVL[14, *] = DOUBLE(litfac[14, *] * RANDOMU(seed, nYP, /DOUBLE))
;PRINT, 'EnvironHVDO'
;PRINT, EnvironHVDO
;PRINT, 'EnvironHVT'
;PRINT, EnvironHVT
...
Calpha = FLTARR(50, nYP); from Letcher et al. for zooplankton
Calpha[0, *] = 193499 * YP[1, *]^(-7.64); for microzooplankton - rotifers
Calpha[1, *] = 0.272 * ALOG(YP[1, *]) - 0.3834; for mesozooplankton1 - calanoids
Calpha[2, *] = 0.4 / (1.0 + ((0.4 / 0.09) - 1.0) * EXP(-13.0 * 0.031 * YP[1, *]))^(1.0 / 0.031); for mesozooplankton2 - cladocerans

PL3 = WHERE((PL[3] / YP[1, *]) LE 0.20, pl3count, complement = pl3c, ncomplement = pl3ccount)
IF (pl3count GT 0.0) THEN Calpha[3, PL3] = ABS(0.50 - 1.75 * (PL[3, PL3] / YP[1, PL3]))
...
Length80 = WHERE((YP[1, *] GT 80.0), Length80count, complement = Length80c, ncomplement = Length80ccount)
 
PL5A = WHERE(((FISHPREY[1, LocHV[1, *]] / YP[1, *]) LT 0.2), PL5Acount, complement = PL5Ac, ncomplement = PL5Account)
PL5Aa = WHERE(((FISHPREY[1, LocHV[1, *]] / YP[1, *]) GE 0.2), PL5Aacount, complement = PL5Aac, ncomplement = PL5Aaccount)
...
PL5B = WHERE(((FISHPREY[1, LocHV[3, *]] / YP[1, *]) LT 0.2), PL5Bcount, complement = PL5Bc, ncomplement = PL5Bccount)
PL5Ba = WHERE(((FISHPREY[1, LocHV[3, *]] / YP[1, *]) GE 0.2), PL5Bacount, complement = PL5Bac, ncomplement = PL5Baccount)
...
PL5C = WHERE(((FISHPREY[1, LocHV[4, *]] / YP[1, *]) LT 0.2), PL5Ccount, complement = PL5Cc, ncomplement = PL5Cccount)
PL5Ca = WHERE(((FISHPREY[1, LocHV[4, *]] / YP[1, *]) GE 0.2), PL5Cacount, complement = PL5Cac, ncomplement = PL5Caccount)
...
PL5D = WHERE(((FISHPREY[1, LocHV[6, *]] / YP[1, *]) LT 0.2), PL5Dcount, complement = PL5Dc, ncomplement = PL5Dccount)
PL5Da = WHERE(((FISHPREY[1, LocHV[6, *]] / YP[1, *]) GE 0.2), PL5Dacount, complement = PL5Dac, ncomplement = PL5Daccount)
...
PL5E = WHERE(((FISHPREY[1, LocHV[8, *]] / YP[1, *]) LT 0.2), PL5Ecount, complement = PL5Ec, ncomplement = PL5Eccount)
PL5Ea = WHERE(((FISHPREY[1, LocHV[8, *]] / YP[1, *]) GE 0.2), PL5Eacount, complement = PL5Eac, ncomplement = PL5Eaccount)
...
PL5F = WHERE(((FISHPREY[1, LocHV[17, *]] / YP[1, *]) LT 0.2), PL5Fcount, complement = PL5Fc, ncomplement = PL5Fccount)
PL5Fa = WHERE(((FISHPREY[1, LocHV[17, *]] / YP[1, *]) GE 0.2), PL5Facount, complement = PL5Fac, ncomplement = PL5Faccount)
...
PL5G = WHERE(((FISHPREY[1, LocHV[26, *]] / YP[1, *]) LT 0.2), PL5Gcount, complement = PL5Gc, ncomplement = PL5Gccount)
PL5Ga = WHERE(((FISHPREY[1, LocHV[26, *]] / YP[1, *]) GE 0.2), PL5Gacount, complement = PL5Gac, ncomplement = PL5Gaccount)
...
PL5H = WHERE(((FISHPREY[1, LocHV[35, *]] / YP[1, *]) LT 0.2), PL5Hcount, complement = PL5Hc, ncomplement = PL5Hccount)
PL5Ha = WHERE(((FISHPREY[1, LocHV[35, *]] / YP[1, *]) GE 0.2), PL5Hacount, complement = PL5Hac, ncomplement = PL5Haccount)
...
PL5I = WHERE(((FISHPREY[1, LocHV[44, *]] / YP[1, *]) LT 0.2), PL5Icount, complement = PL5Ic, ncomplement = PL5Iccount)
PL5Ia = WHERE(((FISHPREY[1, LocHV[44, *]] / YP[1, *]) GE 0.2), PL5Iacount, complement = PL5Iac, ncomplement = PL5Iaccount)
...
PL5J = WHERE(((FISHPREY[1, LocHV[53, *]] / YP[1, *]) LT 0.2), PL5Jcount, complement = PL5Jc, ncomplement = PL5Jccount)
PL5Ja = WHERE(((FISHPREY[1, LocHV[53, *]] / YP[1, *]) GE 0.2), PL5Jacount, complement = PL5Jac, ncomplement = PL5Jaccount)
...
PL5K = WHERE(((FISHPREY[1, LocHV[62, *]] / YP[1, *]) LT 0.2), PL5Kcount, complement = PL5Kc, ncomplement = PL5Kccount)
PL5Ka = WHERE(((FISHPREY[1, LocHV[62, *]] / YP[1, *]) GE 0.2), PL5Kacount, complement = PL5Kac, ncomplement = PL5Kaccount)
...
PL6A = WHERE(((FISHPREY[5, LocHV[1, *]] / YP[1, *]) LT 0.2), PL6Acount, complement = PL6Ac, ncomplement = PL6Account)
PL6Aa = WHERE(((FISHPREY[5, LocHV[1, *]] / YP[1, *]) GE 0.2), PL6Aacount, complement = PL6Aac, ncomplement = PL6Aaccount)
...
PL6B = WHERE(((FISHPREY[5, LocHV[3, *]] / YP[1, *]) LT 0.2), PL6Bcount, complement = PL6Bc, ncomplement = PL6Bccount)
PL6Ba = WHERE(((FISHPREY[5, LocHV[3, *]] / YP[1, *]) GE 0.2), PL6Bacount, complement = PL6Bac, ncomplement = PL6Baccount)
...
PL6C = WHERE(((FISHPREY[5, LocHV[4, *]] / YP[1, *]) LT 0.2), PL6Ccount, complement = PL6Cc, ncomplement = PL6Cccount)
PL6Ca = WHERE(((FISHPREY[5, LocHV[4, *]] / YP[1, *]) GE 0.2), PL6Cacount, complement = PL6Cac, ncomplement = PL6Caccount)
...
PL6D = WHERE(((FISHPREY[5, LocHV[6, *]] / YP[1, *]) LT 0.2), PL6Dcount, complement = PL6Dc, ncomplement = PL6Dccount)
PL6Da = WHERE(((FISHPREY[5, LocHV[6, *]] / YP[1, *]) GE 0.2), PL6Dacount, complement = PL6Dac, ncomplement = PL6Daccount)
...
PL6E = WHERE(((FISHPREY[5, LocHV[8, *]] / YP[1, *]) LT 0.2), PL6Ecount, complement = PL6Ec, ncomplement = PL6Eccount)
PL6Ea = WHERE(((FISHPREY[5, LocHV[8, *]] / YP[1, *]) GE 0.2), PL6Eacount, complement = PL6Eac, ncomplement = PL6Eaccount)
...
PL6F = WHERE(((FISHPREY[5, LocHV[17, *]] / YP[1, *]) LT 0.2), PL6Fcount, complement = PL6Fc, ncomplement = PL6Fccount)
PL6Fa = WHERE(((FISHPREY[5, LocHV[17, *]] / YP[1, *]) GE 0.2), PL6Facount, complement = PL6Fac, ncomplement = PL6Faccount)
...
PL6G = WHERE(((FISHPREY[5, LocHV[26, *]] / YP[1, *]) LT 0.2), PL6Gcount, complement = PL6Gc, ncomplement = PL6Gccount)
PL6Ga = WHERE(((FISHPREY[5, LocHV[26, *]] / YP[1, *]) GE 0.2), PL6Gacount, complement = PL6Gac, ncomplement = PL6Gaccount)
...
PL6H = WHERE(((FISHPREY[5, LocHV[35, *]] / YP[1, *]) LT 0.2), PL6Hcount, complement = PL6Hc, ncomplement = PL6Hccount)
PL6Ha = WHERE(((FISHPREY[5, LocHV[35, *]] / YP[1, *]) GE 0.2), PL6Hacount, complement = PL6Hac, ncomplement = PL6Haccount)
...
PL6I = WHERE(((FISHPREY[5, LocHV[44, *]] / YP[1, *]) LT 0.2), PL6Icount, complement = PL6Ic, ncomplement = PL6Iccount)
PL6Ia = WHERE(((FISHPREY[5, LocHV[44, *]] / YP[1, *]) GE 0.2), PL6Iacount, complement = PL6Iac, ncomplement = PL6Iaccount)
...
PL6J = WHERE(((FISHPREY[5, LocHV[53, *]] / YP[1, *]) LT 0.2), PL6Jcount, complement = PL6Jc, ncomplement = PL6Jccount)
PL6Ja = WHERE(((FISHPREY[5, LocHV[53, *]] / YP[1, *]) GE 0.2), PL6Jacount, complement = PL6Jac, ncomplement = PL6Jaccount)
...
PL6K = WHERE(((FISHPREY[5, LocHV[62, *]] / YP[1, *]) LT 0.2), PL6Kcount, complement = PL6Kc, ncomplement = PL6Kccount)
PL6Ka = WHERE(((FISHPREY[5, LocHV[62, *]] / YP[1, *]) GE 0.2), PL6Kacount, complement = PL6Kac, ncomplement = PL6Kaccount)
...
PL7A = WHERE(((FISHPREY[9, LocHV[1, *]] / YP[1, *]) LT 0.2), PL7Acount, complement = PL7Ac, ncomplement = PL7Account)
PL7Aa = WHERE(((FISHPREY[9, LocHV[1, *]] / YP[1, *]) GE 0.2), PL7Aacount, complement = PL7Aac, ncomplement = PL7Aaccount)
...
PL7B = WHERE(((FISHPREY[9, LocHV[3, *]] / YP[1, *]) LT 0.2), PL7Bcount, complement = PL7Bc, ncomplement = PL7Bccount)
PL7Ba = WHERE(((FISHPREY[9, LocHV[3, *]] / YP[1, *]) GE 0.2), PL7Bacount, complement = PL7Bac, ncomplement = PL7Baccount)
...
PL7C = WHERE(((FISHPREY[9, LocHV[4, *]] / YP[1, *]) LT 0.2), PL7Ccount, complement = PL7Cc, ncomplement = PL7Cccount)
PL7Ca = WHERE(((FISHPREY[9, LocHV[4, *]] / YP[1, *]) GE 0.2), PL7Cacount, complement = PL7Cac, ncomplement = PL7Caccount)
...
PL7D = WHERE(((FISHPREY[9, LocHV[6, *]] / YP[1, *]) LT 0.2), PL7Dcount, complement = PL7Dc, ncomplement = PL7Dccount)
PL7Da = WHERE(((FISHPREY[9, LocHV[6, *]] / YP[1, *]) GE 0.2), PL7Dacount, complement = PL7Dac, ncomplement = PL7Daccount)
...
PL7E = WHERE(((FISHPREY[9, LocHV[8, *]] / YP[1, *]) LT 0.2), PL7Ecount, complement = PL7Ec, ncomplement = PL7Eccount)
PL5Ea = WHERE(((FISHPREY[9, LocHV[8, *]] / YP[1, *]) GE 0.2), PL7Eacount, complement = PL7Eac, ncomplement = PL7Eaccount)
...
PL7F = WHERE(((FISHPREY[9, LocHV[17, *]] / YP[1, *]) LT 0.2), PL7Fcount, complement = PL7Fc, ncomplement = PL7Fccount)
PL7Fa = WHERE(((FISHPREY[9, LocHV[17, *]] / YP[1, *]) GE 0.2), PL7Facount, complement = PL7Fac, ncomplement = PL7Faccount)
...
PL7G = WHERE(((FISHPREY[9, LocHV[26, *]] / YP[1, *]) LT 0.2), PL7Gcount, complement = PL7Gc, ncomplement = PL7Gccount)
PL7Ga = WHERE(((FISHPREY[9, LocHV[26, *]] / YP[1, *]) GT 0.2), PL7Gacount, complement = PL7Gac, ncomplement = PL7Gaccount)
...
PL7H = WHERE(((FISHPREY[9, LocHV[35, *]] / YP[1, *]) LT 0.2), PL7Hcount, complement = PL7Hc, ncomplement = PL7Hccount)
PL7Ha = WHERE(((FISHPREY[9, LocHV[35, *]] / YP[1, *]) GE 0.2), PL7Hacount, complement = PL7Hac, ncomplement = PL7Haccount)
...
PL7I = WHERE(((FISHPREY[9, LocHV[44, *]] / YP[1, *]) LT 0.2), PL7Icount, complement = PL7Ic, ncomplement = PL7Iccount)
PL7Ia = WHERE(((FISHPREY[9, LocHV[44, *]] / YP[1, *]) GE 0.2), PL7Iacount, complement = PL7Iac, ncomplement = PL7Iaccount)
...
PL7J = WHERE(((FISHPREY[9, LocHV[53, *]] / YP[1, *]) LT 0.2), PL7Jcount, complement = PL7Jc, ncomplement = PL7Jccount)
PL7Ja = WHERE(((FISHPREY[9, LocHV[53, *]] / YP[1, *]) GE 0.2), PL7Jacount, complement = PL7Jac, ncomplement = PL7Jaccount)
...
PL7K = WHERE(((FISHPREY[9, LocHV[62, *]] / YP[1, *]) LT 0.2), PL7Kcount, complement = PL7Kc, ncomplement = PL7Kccount)
PL7Ka = WHERE(((FISHPREY[9, LocHV[62, *]] / YP[1, *]) GE 0.2), PL7Kacount, complement = PL7Kac, ncomplement = PL7Kaccount)
...
PL8A = WHERE(((FISHPREY[13, LocHV[1, *]] / YP[1, *]) LT 0.2), PL8Acount, complement = PL8Ac, ncomplement = PL8Account)
PL8Aa = WHERE(((FISHPREY[13, LocHV[1, *]] / YP[1, *]) GE 0.2), PL8Aacount, complement = PL8Aac, ncomplement = PL8Aaccount)
...
PL8B = WHERE(((FISHPREY[13, LocHV[3, *]] / YP[1, *]) LT 0.2), PL8Bcount, complement = PL8Bc, ncomplement = PL8Bccount)
PL8Ba = WHERE(((FISHPREY[13, LocHV[3, *]] / YP[1, *]) GE 0.2), PL8Bacount, complement = PL8Bac, ncomplement = PL8Baccount)
...
PL8C = WHERE(((FISHPREY[13, LocHV[4, *]] / YP[1, *]) LT 0.2), PL8Ccount, complement = PL8Cc, ncomplement = PL8Cccount)
PL8Ca = WHERE(((FISHPREY[13, LocHV[4, *]] / YP[1, *]) GE 0.2), PL8Cacount, complement = PL8Cac, ncomplement = PL8Caccount)
...
PL8D = WHERE(((FISHPREY[13, LocHV[6, *]] / YP[1, *]) LT 0.2), PL8Dcount, complement = PL8Dc, ncomplement = PL8Dccount)
PL8Da = WHERE(((FISHPREY[13, LocHV[6, *]] / YP[1, *]) GE 0.2), PL8Dacount, complement = PL8Dac, ncomplement = PL8Daccount)
...
PL8E = WHERE(((FISHPREY[13, LocHV[8, *]] / YP[1, *]) LT 0.2), PL8Ecount, complement = PL8Ec, ncomplement = PL8Eccount)
PL8Ea = WHERE(((FISHPREY[13, LocHV[8, *]] / YP[1, *]) GE 0.2), PL8Eacount, complement = PL8Eac, ncomplement = PL8Eaccount)
...
PL8F = WHERE(((FISHPREY[13, LocHV[17, *]] / YP[1, *]) LT 0.2), PL8Fcount, complement = PL8Fc, ncomplement = PL8Fccount)
PL8Fa = WHERE(((FISHPREY[13, LocHV[17, *]] / YP[1, *]) GE 0.2), PL8Facount, complement = PL8Fac, ncomplement = PL8Faccount)
...
PL8G = WHERE(((FISHPREY[13, LocHV[26, *]] / YP[1, *]) LT 0.2) AND (YP[1, *] LE 80.0), PL8Gcount, complement = PL8Gc, ncomplement = PL8Gccount)
PL8Ga = WHERE(((FISHPREY[13, LocHV[26, *]] / YP[1, *]) GE 0.2) AND (YP[1, *] LE 80.0), PL8Gacount, complement = PL8Gac, ncomplement = PL8Gaccount)
...
PL8H = WHERE(((FISHPREY[13, LocHV[35, *]] / YP[1, *]) LT 0.2), PL8Hcount, complement = PL8Hc, ncomplement = PL8Hccount)
PL8Ha = WHERE(((FISHPREY[13, LocHV[35, *]] / YP[1, *]) GE 0.2), PL8Hacount, complement = PL8Hac, ncomplement = PL8Haccount)
...
PL8I = WHERE(((FISHPREY[13, LocHV[44, *]] / YP[1, *]) LT 0.2), PL8Icount, complement = PL8Ic, ncomplement = PL8Iccount)
PL8Ia = WHERE(((FISHPREY[13, LocHV[44, *]] / YP[1, *]) GE 0.2), PL8Iacount, complement = PL8Iac, ncomplement = PL8Iaccount)
...
PL8J = WHERE(((FISHPREY[13, LocHV[53, *]] / YP[1, *]) LT 0.2), PL8Jcount, complement = PL8Jc, ncomplement = PL8Jccount)
PL8Ja = WHERE(((FISHPREY[13, LocHV[53, *]] / YP[1, *]) GE 0.2), PL8Jacount, complement = PL8Jac, ncomplement = PL8Jaccount)
...
PL8K = WHERE(((FISHPREY[13, LocHV[62, *]] / YP[1, *]) LT 0.2), PL8Kcount, complement = PL8Kc, ncomplement = PL8Kccount)
PL8Ka = WHERE(((FISHPREY[13, LocHV[62, *]] / YP[1, *]) GE 0.2), PL8Kacount, complement = PL8Kac, ncomplement = PL8Kaccount)
...
TOT[1, *] = t[1,*] + t[10,*] + t[19,*] + t[28,*] + t[37,*] + t[46,*] + t[90,*] + t[101,*] + t[112,*]
;TOT[2, *] = t[2,*] + t[11,*] + t[20,*] + t[29,*] + t[38,*] + t[47,*]
TOT[3, *] = t[3,*] + t[12,*] + t[21,*] + t[30,*] + t[39,*] + t[48,*] + t[91,*] + t[102,*] + t[113,*]
TOT[4, *] = t[4,*] + t[13,*] + t[22,*] + t[31,*] + t[40,*] + t[49,*] + t[92,*] + t[103,*] + t[114,*]
;TOT[5, *] = t[5,*] + t[14,*] + t[23,*] + t[32,*] + t[41,*] + t[50,*]
TOT[6, *] = t[6,*] + t[15,*] + t[24,*] + t[33,*] + t[42,*] + t[51,*] + t[93,*] + t[104,*] + t[115,*]
;TOT[7, *] = t[7,*] + t[16,*] + t[25,*] + t[34,*] + t[43,*] + t[52,*]

TOT[8, *] = t[8,*] + t[17,*] + t[26,*] + t[35,*] + t[44,*] + t[53,*] + t[94,*] + t[105,*] + t[116,*]

TOT[9, *] = t[54,*] + t[60,*] + t[66,*] + t[72,*] + t[78,*] + t[84,*] + t[95,*] + t[106,*] + t[117,*]
TOT[10, *] = t[55,*] + t[61,*] + t[67,*] + t[73,*] + t[79,*] + t[85,*] + t[96,*] + t[107,*] + t[118,*]
TOT[11, *] = t[56,*] + t[62,*] + t[68,*] + t[74,*] + t[80,*] + t[86,*] + t[97,*] + t[108,*] + t[119,*]
TOT[12, *] = t[57,*] + t[63,*] + t[69,*] + t[75,*] + t[81,*] + t[87,*] + t[98,*] + t[109,*] + t[120,*]
TOT[13, *] = t[58,*] + t[64,*] + t[70,*] + t[76,*] + t[82,*] + t[88,*] + t[99,*] + t[110,*] + t[121,*]
TOT[14, *] = t[59,*] + t[65,*] + t[71,*] + t[77,*] + t[83,*] + t[89,*] + t[100,*] + t[111,*] + t[122,*]
;PRINT, 'TOT'
;PRINT, TOT
...
IF (Lcount GT 0.0) THEN MoveSpeed[2, L] = SS[L] * COS(VerOriAng[L]) * RANDOMU(seed, Lcount, /DOUBLE)+ newinput[14, YP[14, L]]; VERTICAL DIRECTION
IF (LLcount GT 0.0) THEN MoveSpeed[2, LL] = SS[LL]/(ts*6.) * COS(VerOriAng[LL]) * RANDOMU(seed, LLcount, /DOUBLE)+ newinput[14, YP[14, LL]]; VERTICAL DIRECTION
...
VerSize = FLTARR(nYP)
VerSize[*] = (Grid2D[3, YP[13, *]]/20.)*1000.
...
;xMovePosLoc = WHERE((xMove EQ 5.0) OR (xMove EQ 9.0), xMovePosLoccount, complement = xMovePosLocN, ncomplement = xMovePosLocNcount)
...
;PRINT, 'YP[13, *]'
;PRINT, TRANSPOSE(YP[13, 0:200])
;PRINT, 'YP[10, *]'
;PRINT, TRANSPOSE(YP[10, 0:200])
;PRINT, 'YP[11, *]'
;PRINT, TRANSPOSE(YP[11, 0:200])

; *****Determine new cell locations****************************************************************************************
;PRINT, 'OldGrid 3D ID with vertical movement'
;PRINT, TRANSPOSE(YP[14, 0:200])
...
;IF xyMoveOut1count GT 0.0 THEN BEGIN; XYloc = 1
  IF zMoveOut11count GT 0.0 THEN YP[14, zMoveOut11] = LocHV[54, zMoveOut11];zMove[zMoveOut1] = 14
  IF zMoveOut12count GT 0.0 THEN YP[14, zMoveOut12] = LocHV[54, zMoveOut12];zMove[zMoveOut2] = 14
  IF zMoveOut13count GT 0.0 THEN YP[14, zMoveOut13] = LocHV[45, zMoveOut13];zMove[zMoveOut3] = 13
  IF zMoveOut14count GT 0.0 THEN YP[14, zMoveOut14] = LocHV[36, zMoveOut14];zMove[zMoveOut4] = 12
  IF zMoveOut15count GT 0.0 THEN YP[14, zMoveOut15] = LocHV[0, zMoveOut15];zMove[zMoveOut5] = 15
  IF zMoveOut16count GT 0.0 THEN YP[14, zMoveOut16] = LocHV[27, zMoveOut16];zMove[zMoveOut6] = 11
  IF zMoveOut17count GT 0.0 THEN YP[14, zMoveOut17] = LocHV[18, zMoveOut17];zMove[zMoveOut7] = 10
  IF zMoveOut18count GT 0.0 THEN YP[14, zMoveOut18] = LocHV[9, zMoveOut18];zMove[zMoveOut8] = 9
  IF zMoveOut19count GT 0.0 THEN YP[14, zMoveOut19] = LocHV[9, zMoveOut19];zMove[zMoveOut9] = 9
;ENDIF
;PRINT,'xyMoveOut1', xyMoveOut1
;IF xyMoveOut2count GT 0.0 THEN BEGIN; XYloc = 2
  IF zMoveOut21count GT 0.0 THEN YP[14, zMoveOut21] = LocHV[55, zMoveOut21];zMove[zMoveOut1] = 9
  IF zMoveOut22count GT 0.0 THEN YP[14, zMoveOut22] = LocHV[55, zMoveOut22];zMove[zMoveOut2] = 9
  IF zMoveOut23count GT 0.0 THEN YP[14, zMoveOut23] = LocHV[46, zMoveOut23];zMove[zMoveOut3] = 10
  IF zMoveOut24count GT 0.0 THEN YP[14, zMoveOut24] = LocHV[37, zMoveOut24];zMove[zMoveOut4] = 11
  IF zMoveOut25count GT 0.0 THEN YP[14, zMoveOut25] = LocHV[1, zMoveOut25];zMove[zMoveOut5] = 15
  IF zMoveOut26count GT 0.0 THEN YP[14, zMoveOut26] = LocHV[28, zMoveOut26];zMove[zMoveOut6] = 12
  IF zMoveOut27count GT 0.0 THEN YP[14, zMoveOut27] = LocHV[19, zMoveOut27];zMove[zMoveOut7] = 13
  IF zMoveOut28count GT 0.0 THEN YP[14, zMoveOut28] = LocHV[10, zMoveOut28];zMove[zMoveOut8] = 14
  IF zMoveOut29count GT 0.0 THEN YP[14, zMoveOut29] = LocHV[10, zMoveOut29];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut2', xyMoveOut2
;IF xyMoveOut3count GT 0.0 THEN BEGIN; XYloc = 3
  IF zMoveOut31count GT 0.0 THEN YP[14, zMoveOut31] = LocHV[56, zMoveOut31];zMove[zMoveOut1] = 9
  IF zMoveOut32count GT 0.0 THEN YP[14, zMoveOut32] = LocHV[56, zMoveOut32];zMove[zMoveOut2] = 9
  IF zMoveOut33count GT 0.0 THEN YP[14, zMoveOut33] = LocHV[47, zMoveOut33];zMove[zMoveOut3] = 10
  IF zMoveOut34count GT 0.0 THEN YP[14, zMoveOut34] = LocHV[38, zMoveOut34];zMove[zMoveOut4] = 11
  IF zMoveOut35count GT 0.0 THEN YP[14, zMoveOut35] = LocHV[2, zMoveOut35];zMove[zMoveOut5] = 15
  IF zMoveOut36count GT 0.0 THEN YP[14, zMoveOut36] = LocHV[29, zMoveOut36];zMove[zMoveOut6] = 12
  IF zMoveOut37count GT 0.0 THEN YP[14, zMoveOut37] = LocHV[20, zMoveOut37];zMove[zMoveOut7] = 13
  IF zMoveOut38count GT 0.0 THEN YP[14, zMoveOut38] = LocHV[11, zMoveOut38];zMove[zMoveOut8] = 14
  IF zMoveOut39count GT 0.0 THEN YP[14, zMoveOut39] = LocHV[11, zMoveOut39];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut3', xyMoveOut3
;IF xyMoveOut4count GT 0.0 THEN BEGIN; XYloc = 4
  IF zMoveOut41count GT 0.0 THEN YP[14, zMoveOut41] = LocHV[57, zMoveOut41];zMove[zMoveOut1] = 9
  IF zMoveOut42count GT 0.0 THEN YP[14, zMoveOut42] = LocHV[57, zMoveOut42];zMove[zMoveOut2] = 9
  IF zMoveOut43count GT 0.0 THEN YP[14, zMoveOut43] = LocHV[48, zMoveOut43];zMove[zMoveOut3] = 10
  IF zMoveOut44count GT 0.0 THEN YP[14, zMoveOut44] = LocHV[39, zMoveOut44];zMove[zMoveOut4] = 11
  IF zMoveOut45count GT 0.0 THEN YP[14, zMoveOut45] = LocHV[3, zMoveOut45];zMove[zMoveOut5] = 15
  IF zMoveOut46count GT 0.0 THEN YP[14, zMoveOut46] = LocHV[30, zMoveOut46];zMove[zMoveOut6] = 12
  IF zMoveOut47count GT 0.0 THEN YP[14, zMoveOut47] = LocHV[21, zMoveOut47];zMove[zMoveOut7] = 13
  IF zMoveOut48count GT 0.0 THEN YP[14, zMoveOut48] = LocHV[12, zMoveOut48];zMove[zMoveOut8] = 14
  IF zMoveOut49count GT 0.0 THEN YP[14, zMoveOut49] = LocHV[12, zMoveOut49];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut4', xyMoveOut4
;IF xyMoveOut5count GT 0.0 THEN BEGIN; XYloc = 5
  IF zMoveOut51count GT 0.0 THEN YP[14, zMoveOut51] = LocHV[58, zMoveOut51];zMove[zMoveOut1] = 9
  IF zMoveOut52count GT 0.0 THEN YP[14, zMoveOut52] = LocHV[58, zMoveOut52];zMove[zMoveOut2] = 9
  IF zMoveOut53count GT 0.0 THEN YP[14, zMoveOut53] = LocHV[49, zMoveOut53];zMove[zMoveOut3] = 10
  IF zMoveOut54count GT 0.0 THEN YP[14, zMoveOut54] = LocHV[40, zMoveOut54];zMove[zMoveOut4] = 11
  IF zMoveOut55count GT 0.0 THEN YP[14, zMoveOut55] = LocHV[4, zMoveOut55];zMove[zMoveOut5] = 15
  IF zMoveOut56count GT 0.0 THEN YP[14, zMoveOut56] = LocHV[31, zMoveOut56];zMove[zMoveOut6] = 12
  IF zMoveOut57count GT 0.0 THEN YP[14, zMoveOut57] = LocHV[22, zMoveOut57];zMove[zMoveOut7] = 13
  IF zMoveOut58count GT 0.0 THEN YP[14, zMoveOut58] = LocHV[13, zMoveOut58];zMove[zMoveOut8] = 14
  IF zMoveOut59count GT 0.0 THEN YP[14, zMoveOut59] = LocHV[13, zMoveOut59];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut5', xyMoveOut5
;IF xyMoveOut6count GT 0.0 THEN BEGIN; XYloc = 6
  IF zMoveOut61count GT 0.0 THEN YP[14, zMoveOut61] = LocHV[59, zMoveOut61];zMove[zMoveOut1] = 9
  IF zMoveOut62count GT 0.0 THEN YP[14, zMoveOut62] = LocHV[59, zMoveOut62];zMove[zMoveOut2] = 9
  IF zMoveOut63count GT 0.0 THEN YP[14, zMoveOut63] = LocHV[50, zMoveOut63];zMove[zMoveOut3] = 10
  IF zMoveOut64count GT 0.0 THEN YP[14, zMoveOut64] = LocHV[41, zMoveOut64];zMove[zMoveOut4] = 11
  IF zMoveOut65count GT 0.0 THEN YP[14, zMoveOut65] = LocHV[5, zMoveOut65];zMove[zMoveOut5] = 15
  IF zMoveOut66count GT 0.0 THEN YP[14, zMoveOut66] = LocHV[32, zMoveOut66];zMove[zMoveOut6] = 12
  IF zMoveOut67count GT 0.0 THEN YP[14, zMoveOut67] = LocHV[23, zMoveOut67];zMove[zMoveOut7] = 13
  IF zMoveOut68count GT 0.0 THEN YP[14, zMoveOut68] = LocHV[14, zMoveOut68];zMove[zMoveOut8] = 14
  IF zMoveOut69count GT 0.0 THEN YP[14, zMoveOut69] = LocHV[14, zMoveOut69];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut6', xyMoveOut6
;IF xyMoveOut7count GT 0.0 THEN BEGIN; XYloc = 7
  IF zMoveOut71count GT 0.0 THEN YP[14, zMoveOut71] = LocHV[60, zMoveOut71];zMove[zMoveOut1] = 9
  IF zMoveOut72count GT 0.0 THEN YP[14, zMoveOut72] = LocHV[60, zMoveOut72];zMove[zMoveOut2] = 9
  IF zMoveOut73count GT 0.0 THEN YP[14, zMoveOut73] = LocHV[51, zMoveOut73];zMove[zMoveOut3] = 10
  IF zMoveOut74count GT 0.0 THEN YP[14, zMoveOut74] = LocHV[42, zMoveOut74];zMove[zMoveOut4] = 11
  IF zMoveOut75count GT 0.0 THEN YP[14, zMoveOut75] = LocHV[6, zMoveOut75];zMove[zMoveOut5] = 15
  IF zMoveOut76count GT 0.0 THEN YP[14, zMoveOut76] = LocHV[33, zMoveOut76];zMove[zMoveOut6] = 12
  IF zMoveOut77count GT 0.0 THEN YP[14, zMoveOut77] = LocHV[24, zMoveOut77];zMove[zMoveOut7] = 13
  IF zMoveOut78count GT 0.0 THEN YP[14, zMoveOut78] = LocHV[15, zMoveOut78];zMove[zMoveOut8] = 14
  IF zMoveOut79count GT 0.0 THEN YP[14, zMoveOut79] = LocHV[15, zMoveOut79];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut7', xyMoveOut7
;IF xyMoveOut8count GT 0.0 THEN BEGIN; XYloc = 8
  IF zMoveOut81count GT 0.0 THEN YP[14, zMoveOut81] = LocHV[61, zMoveOut81];zMove[zMoveOut1] = 9
  IF zMoveOut82count GT 0.0 THEN YP[14, zMoveOut82] = LocHV[61, zMoveOut82];zMove[zMoveOut2] = 9
  IF zMoveOut83count GT 0.0 THEN YP[14, zMoveOut83] = LocHV[52, zMoveOut83];zMove[zMoveOut3] = 10
  IF zMoveOut84count GT 0.0 THEN YP[14, zMoveOut84] = LocHV[43, zMoveOut84];zMove[zMoveOut4] = 11
  IF zMoveOut85count GT 0.0 THEN YP[14, zMoveOut85] = LocHV[7, zMoveOut85];zMove[zMoveOut5] = 15
  IF zMoveOut86count GT 0.0 THEN YP[14, zMoveOut86] = LocHV[34, zMoveOut86];zMove[zMoveOut6] = 12
  IF zMoveOut87count GT 0.0 THEN YP[14, zMoveOut87] = LocHV[25, zMoveOut87];zMove[zMoveOut7] = 13
  IF zMoveOut88count GT 0.0 THEN YP[14, zMoveOut88] = LocHV[16, zMoveOut88];zMove[zMoveOut8] = 14
  IF zMoveOut89count GT 0.0 THEN YP[14, zMoveOut89] = LocHV[16, zMoveOut89];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut8', xyMoveOut8
;IF xyMoveOut9count GT 0.0 THEN BEGIN; XYloc = 9
  IF zMoveOut91count GT 0.0 THEN YP[14, zMoveOut91] = LocHV[62, zMoveOut91];zMove[zMoveOut1] = 9
  IF zMoveOut92count GT 0.0 THEN YP[14, zMoveOut92] = LocHV[62, zMoveOut92];zMove[zMoveOut2] = 9
  IF zMoveOut93count GT 0.0 THEN YP[14, zMoveOut93] = LocHV[53, zMoveOut93];zMove[zMoveOut3] = 10
  IF zMoveOut94count GT 0.0 THEN YP[14, zMoveOut94] = LocHV[44, zMoveOut94];zMove[zMoveOut4] = 11
  IF zMoveOut95count GT 0.0 THEN YP[14, zMoveOut95] = LocHV[8, zMoveOut95];zMove[zMoveOut6] = 12
  IF zMoveOut96count GT 0.0 THEN YP[14, zMoveOut96] = LocHV[35, zMoveOut96];zMove[zMoveOut6] = 12
  IF zMoveOut97count GT 0.0 THEN YP[14, zMoveOut97] = LocHV[26, zMoveOut97];zMove[zMoveOut7] = 13
  IF zMoveOut98count GT 0.0 THEN YP[14, zMoveOut98] = LocHV[17, zMoveOut98];zMove[zMoveOut8] = 14
  IF zMoveOut99count GT 0.0 THEN YP[14, zMoveOut99] = LocHV[17, zMoveOut99];zMove[zMoveOut9] = 14
;ENDIF
;PRINT,'xyMoveOut9', xyMoveOut9
;PRINT, 'YP[14, *]'
;PRINT, TRANSPOSE(YP[14, 0:500])
;  PRINT, 'YP[13, *]'
;  PRINT, TRANSPOSE(YP[13, *])
;PRINT, 'NewGrid 3D ID with vertical movement'
;PRINT, TRANSPOSE(YP[14, 0:200])
;PRINT, 'NewInput[0, YP[14, *]]'
;PRINT, transpose(NewInput[0, YP[14, 0:200]])
;PRINT, 'NewInput[1, YP[14, *]]'
;PRINT, transpose(NewInput[1, YP[14, 0:200]])
;PRINT, 'NewInput[2, YP[14, *]]'
;PRINT, transpose(NewInput[2, YP[14, 0:200]])
PRINT, 'NewInput[3, YP[14, *]]'
PRINT, transpose(NewInput[3, YP[14, 0:500]])
;  PRINT, 'YP[10, *]'
...
;PRINT, 'zMoveOut41',zMoveOut41
;PRINT, 'LocHV[57, zMoveOut41]',TRANSPOSE(LocHV[57, zMoveOut41])
;PRINT, 'zMoveOut42',zMoveOut42
;PRINT, 'LocHV[57, zMoveOut42]',TRANSPOSE(LocHV[57, zMoveOut42])
;PRINT, 'zMoveOut43',zMoveOut43
;PRINT, 'LocHV[48, zMoveOut43]',TRANSPOSE(LocHV[48, zMoveOut43])
;PRINT, 'zMoveOut44',zMoveOut44
;PRINT, 'LocHV[39, zMoveOut44]',TRANSPOSE(LocHV[39, zMoveOut44])
;PRINT, 'zMoveOut91',zMoveOut91
;PRINT, 'LocHV[62, zMoveOut91]',TRANSPOSE(LocHV[62, zMoveOut91])
;PRINT, 'zMoveOut92',zMoveOut92
;PRINT, 'LocHV[62, zMoveOut92]',TRANSPOSE(LocHV[62, zMoveOut92])
;PRINT, 'zMoveOut93',zMoveOut93
;PRINT, 'LocHV[53, zMoveOut93]',TRANSPOSE(LocHV[53, zMoveOut93])
;PRINT, 'zMoveOut94',zMoveOut94
;PRINT, 'LocHV[44, zMoveOut94]',TRANSPOSE(LocHV[44, zMoveOut94])
...
;  PRINT, 'LocNH9count', LocNH9count 
;  PRINT, 'LocNH17count', LocNH17count
;  PRINT, 'EnvironHVSumMax', EnvironHVSumMax
;  PRINT, 'MAX(EnvironHVSum[9:11, *], DIMENSION = 1)',MAX(EnvironHVSum[9:11, *], DIMENSION = 1)
;  PRINT, N_ELEMENTS(MAX(EnvironHVSum[9:11, *], DIMENSION = 1))
;PRINT, 'YP[14, *]'
;PRINT, YP[14, *]
;PRINT, 'Grid2D[0, NewGridXY[2, *]] ', TRANSPOSE(Grid2D[0, NewGridXY[2, *]]) 
;PRINT, 'LocHV[*, *]'
;PRINT, (LocHV[*, *])
