FUNCTION YEPresp1D, Tamb, TacclR, Weight, length, ts, DOa, DOacclR, DOcritR, nYP, Oxydebt
; Function to determine respiration

PRINT, 'YEPresp1D Begins Here'
...
IF (TLcount GT 0.0) THEN BEGIN
  ...
  ACT[TL] = 1.; from WBM, but this doesn't allow the fish to grow due to high resp costs 
ENDIF
...
IF (TLLcount GT 0.0) THEN BEGIN 
  ...
  ACT[TLL] = 1.6; multiplication factor
ENDIF
...
IF (TLLLcount GT 0.0) THEN BEGIN 
  ...
  ACT[TLLL] = 1.6; multiplication factor
ENDIF
