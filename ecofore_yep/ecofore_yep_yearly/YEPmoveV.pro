FUNCTION YEPmoveV, YP, nYP, newinput, Grid3D, Grid2D, DOacclim, NewGridID2
;function determines movement in X,Y,Z direction for all yellow perch

;----------------------TEST ONLY---------------------------------------------------------------------------
;PRO YEPmoveV, YP, nYP, newinput, Grid3D, Grid2D, DOacclim, NewGridID2
;; NEED to change NewInputFiles, YPinitial, YEPacclT, YEPacclDO for testing
;nYP = 50L; number of SI YP
;ihour = 2L
;YP = YPinitial()
...
EnvironV = FLTARR(16, nYP*20L)
;NewGridID2 = FLTARR(nYP)
NewGridID3D = FLTARR(20L)
;;NewGridIDindex = FLTARR(nYP*20L)
NewGridID3D2 = FLTARR(nYP*20L)
...
FOR evvv = 0L, nYP - 1L DO BEGIN
  FOR ev = 0L, nYP - 1L DO BEGIN;evv + 1L
    ;NewGridID3D[evv] = WHERE((evv + 1L EQ newinput[2, *] AND NewGridID[ev] EQ newinput[3, 0L : *]), NewLoccount2);    
    NewGridID3D2[evvv : jv] = WHERE(NewGridID2[ev] EQ NewInput[3, 0L : *], NewLoccount2); matching new grid cell IDs with input file IDs
   
    ;NewGridID3D2[evvv : jv] = NewGridID3D[*]
  ...
  ;PRINT, 'NewGridID3D2 =', NewGridID3D2 
ENDFOR
PRINT, 'NewGridID3D2 =', NewGridID3D2; new 3D grid cell IDs

; Identify a new environment using new 3D grid cell IDs
...
LengthV = FLTARR(nYP*20L)
;depth = indgen(20); 1 = top layer; 20 = bottom layer
j = 19L
FOR ii = 0L, nYP*20L - 1L DO BEGIN
FOR iii = 0L, nYP - 1L DO BEGIN
LengthV[ii : j] = YP[1, iii]
ii = j
j = j + 20L
ENDFOR
ENDFOR
...
;calculate attack probability using chesson's alpha = capture efficiency
  TOT = FLTARR(nYP*20L); total number of all prey atacked and captured
  t = FLTARR(m, nYP*20L); total number of each prey atacked and captured
      t[0,*] = (Calpha[0,*] * dens[0,*])
      t[1,*] = (Calpha[1,*] * dens[1,*])
      t[2,*] = (Calpha[2,*] * dens[2,*])
      t[3,*] = (Calpha[3,*] * dens[3,*])
...
preyTOT = FLTARR(nYP*20L)      
jv = 19L
FOR ivvv = 0L, nYP*20L - 1L DO BEGIN
...
preyTOT2 = FLTARR(nYP*20L)
j = 19L
FOR iii = 0L, nYP*20L - 19L DO BEGIN
FOR iiii = 0L, nYP*20L - 1L DO BEGIN
...
TOTprop = FLTARR(nYP*20L)
EnvironV[10, *] = TOT / preyTOT2
TOTprop = TOT / preyTOT2
;PRINT, 'TOTprop =', EnvironV[10, *]
;PRINT, 'TOTprop =', TOTprop
EnvironV10 = EnvironV[10, *] * RANDOMU(seed, nYP*20L, /DOUBLE)
;EnvironV10 = EnvironV[10, *] * RANDOMU(seed, nYP*20L)
PRINT, 'EnvironV10 =', EnvironV10
...
FOR dovvv = 0L, nYP - 1L DO BEGIN
  FOR dov = 0L, nYP - 1L DO BEGIN
...
EnvironVMax = FLTARR(nYP*20L)
EnvironVMax2 = FLTARR(nYP*20L)
EnvironVMax3 = FLTARR(nYP*20L)
jvvv = 19L
; Create array with highest habitat quality-based probability for fish
FOR iv = 0L, nYP*20L - 1L DO BEGIN
...
PRINT, 'Vertical Movement Ends Here'
;PRINT, 'YP Chiro =', YP[18, 0:99]
