FUNCTION EMSForage, iHour, Length, Temp, Light, EMSpbio, EMScmx, PreStom, CAftDig0, CAftDig1, CAftDig2, CAftDig3, CAftDig4, CAftDig5, $
StCap, TotCday, TotC0, TotC1, TotC2, TotC3, TotC4, TotC5, ts, nEMS, DOa, DOacclC, DOcritC, EMS;, Grid2D
; This function calculates total consumption (g) by EMERALD SHINER


PRINT, 'Emerald Shiner Begins Here'
tstart = SYSTIME(/seconds)

tss = ts*60.; time step in secound
m = 6; the number of prey types 
NewStom = FLTARR(nEMS); the amount of food in stomach
TotC = FLTARR(m, nEMS); daily cumulative consumption of each prey item 
TotCts = FLTARR(nEMS)
DigestedFood = FLTARR(m, nEMS); the total amount of digested prey used for growth
PotStAftDig = FLTARR(nEMS); potential stomach content after digestion 
C = FLTARR(m, nEMS); the amount of each prey item consumed per time step
CAftDig = FLTARR(m ,nEMS); the amount of each prey item undigested 
;CAftDig[0,*] = CAftDig0[*]
;CAftDig[1,*] = CAftDig1[*]
;CAftDig[2,*] = CAftDig2[*]
;CAftDig[3,*] = CAftDig3[*]
...
; Prey length (mm), with stochasticity within the predetermined ranges
PL = FLTARR(m+4, nEMS)
...
PL[5, *] = EMSpbio[6, *]; length for yellow perch in mm 
PL[6, *] = EMSpbio[10, *]; length for emerald shiner in mm 
PL[7, *] = EMSpbio[14, *]; length for rainbow smelt in mm 
PL[8, *] = EMSpbio[18, *]; length for round goby in mm
PL[9, *] = EMSpbio[22, *]; length for walleye in mm 
;  PRINT, 'Prey length (PL, mm)'
;  PRINT, PL


; Prey individual weight (g, wet)
PW = FLTARR(m+4, nEMS); weight of each prey type
...
dens = FLTARR(m+4, nEMS)
dens[0,*] = EMSpbio[0,*] / Pw[0,*]; for microzooplankton 
dens[1,*] = EMSpbio[1,*] / Pw[1,*]; for small mesozooplankton
dens[2,*] = EMSpbio[2,*] / Pw[2,*]; for large mesozooplankton
pbio3 = WHERE(EMSpbio[3,*] GT 0.0, pbio3count, complement = pbio3c, ncomplement = pbio3ccount)
IF pbio3count GT 0.0 THEN dens[3, pbio3] = EMSpbio[3, pbio3] / Pw[3,pbio3] ELSE dens[3, pbio3c] = 0.0; for chironmoids
pbio4 = WHERE(EMSpbio[4,*] GT 0.0, pbio4count, complement = pbio4c, ncomplement = pbio4ccount)
IF pbio4count GT 0.0 THEN dens[4, pbio4] = EMSpbio[4, pbio4] / Pw[4,pbio4] ELSE dens[4, pbio4c] = 0.0; for invasive species 

dens[5, *] = EMSpbio[5, *]; for yellow perch
dens[6, *] = EMSpbio[9, *]; for emerald shiner
dens[7, *] = EMSpbio[13, *]; for rainbow smlet
dens[8, *] = EMSpbio[17, *]; round goby 
dens[9, *] = EMSpbio[21, *]; for walleye
...
tds = WHERE((TotCday LT EMScmx) AND (PreStom LT Stcap), tdscount, complement = d, ncomplement = dcount)
PRINT, 'Number of fish with less than full stomach Cmax =', tdscount
...
;***NEED TO CORRECT FUNCTIONS AND INCLUDE SS FOR EMERALD SHINER***
  SS = FLTARR(nEMS)
  S = FLTARR(nEMS)
...
; Calculate predator size-based prey selectivity (Chesson's alpha) for each prey tEMSe
  Calpha = FLTARR(m, nEMS); from Letcher et al. for zooplankton
  ...
  Calpha[2, *] = 0.8 / (1.0 + ((0.4 / 0.09) - 1.0) * EXP(-13.0 * 0.031 * length))^(1.0 / 0.031) ; for mesozooplankton2 - cladocerans
...
;  PRINT, Calpha
  ...
  litfac = FLTARR(nEMS); a multiplication factor to include the effect of light intensity on RD
...
;  PRINT, TRANSPOSE(alphaDig)
  ...
  RD = FLTARR(m+4, nEMS); RD = 0.5*PL/tan(2.0*!PI*(1.0/360.0)*alphaDig/2.0)
...
;  PRINT, ER 
  ...
  ERP = FLTARR(5, nEMS); ENCOUNTER RATE FOR COMPETITORS
  ; Forage fish only
  ERP[0, *] = CEIL(VS[5, *] * EMSpbio[25, *]); yellow perch
  ERP[1, *] = CEIL(VS[6, *] * EMSpbio[26, *]); emerald shiner
  ERP[2, *] = CEIL(VS[7, *] * EMSpbio[27, *]); rainbow smelt
  ERP[3, *] = CEIL(VS[8, *] * EMSpbio[28, *]); round goby
  ERP[4, *] = CEIL(VS[9, *] * EMSpbio[29, *]); walleye
  
  ERPintra = FLTARR(nEMS)
  ERPinter = FLTARR(nEMS)
  ERPintra[*] = ERP[1, *]
  ERPinter[*] = ERP[0, *] + ERP[2, *] + ERP[3, *] + ERP[4, *]
...
;*****HandLing time should not exceed time step*******-->CHECK HOW HANDLING TIME IS TREATED IN OTHER MODELS*********** 
...
; Calculate attack probability using chesson's alpha = capture efficiency
  TOT = FLTARR(nEMS); probability of all prey atacked and captured
  t = FLTARR(m, nEMS);  probability of each prey atacked and captured
...
;  PRINT, TOT
  ...
  Q = FLTARR(m,nEMS); Probability of attack and capture
...
; Calculate the number of each prey consumed 
  E = FLTARR(m, nEMS); encounter rate in individuals
  EPintra = FLTARR(nEMS); intraspecific encounter rate
  EPinter = FLTARR(nEMS); interspecific encounter rate
  NumP = FLTARR(m, nEMS); potential total number of prey consumed 
  FOR ind = 0L, nEMS - 1L DO BEGIN ; for each superindividual
...
;    IF ER[8, ind] GT 0. THEN E[8, ind] = RANDOMU(seed, POISSON = [ER[8, ind]], /double)
    ...
    ; Stochastic estimates for the number and prey type based on E from binomial distriution
...
;  PRINT, NumP
  ...
  SD = FLTARR(m, nEMS); if handling time (HT) > ts, then no conumption during each time step 
  SD[0,*] =  E[0,*] * Q[0,*] * HT[0,*]; 
  SD[1,*] =  E[1,*] * Q[1,*] * HT[1,*]; 
  SD[2,*] =  E[2,*] * Q[2,*] * HT[2,*]; 
...
; calculate the amount of each prey type consumed 
 ; IF handling time is greater than ts, no consumption
  cons = FLTARR(m, nEMS)
...
;  PRINT, (TotCts)
  ...
  densPintra = FLTARR(nEMS)
  densPinter = FLTARR(nEMS)
  densPintra[*] = EMSpbio[31, *]
  densPinter[*] = EMSpbio[30, *] + EMSpbio[32, *] + EMSpbio[33, *] + EMSpbio[34, *]
...
; PRINT, densPinter[0:100]
  ...
  Cratio = FLTARR(m, nEMS); PROPORTIONS OF PREY-SPECIFIC CONSUMPTION
...
; Check if potential stomach weight is less than stomach capacity
  nstom = FLTARR(nEMS)
  Premove = FLTARR(nEMS)
  TotCAftDig = FLTARR(nEMS)
  RemCAftDig = FLTARR(nEMS)
  RemCAftDig0 = FLTARR(nEMS)
  RemCAftDig1 = FLTARR(nEMS)
  RemCAftDig2 = FLTARR(nEMS)
  RemCAftDig3 = FLTARR(nEMS)
  RemCAftDig4 = FLTARR(nEMS)
  RemCAftDig5 = FLTARR(nEMS)
...
;  PRINT, TRANSPOSE(stcap)
  ...
  IF (Pcount GT 0.0) THEN BEGIN
    ...
    Premove[P] = DOUBLE(stcap[P] / TotPotSt[P]); proportion of consumed food to keep
;    PRINT, 'Premove'
;    PRINT, (premove)
    ...
    RemCAftDig0[P] = RemCAftDig[P] * Cratio[0, P]; prey-specific removal
...
;***Hypoxia effect is incorporated in the digestion process (physiological) -> slows digestion -> lowers foraging***
  ; Gut evacuation rate, R, based on Eurasian perch from Durbin et al. 1983
  EMSGutEvacR = 0.0182 * EXP(0.14 * Temp) / 60.0 /0.15 * fDOfc2; in g/min acclimated temperature-dependent
  ;PRINT, 'EMSGutEvacR'
  ;PRINT, TRANSPOSE(EMSGutEvacR)
...
;  PRINT, PotSt[*, tds]
  ...
  ;newstomAFTdig = prestom[*] * exp(-EMSGutEvacR * ts) + ((newstom - prestom[*]) / EMSGutEvacR) * (1 - exp(-EMSGutEvacR * ts))
  CAftDig[0, tds] = (CAftDig0[tds] * EXP(-EMSGutEvacR[tds] * ts) + (C[0, tds]) * EXP(-EMSGutEvacR[tds] * ts));
  CAftDig[1, tds] = (CAftDig1[tds] * EXP(-EMSGutEvacR[tds] * ts) + (C[1, tds]) * EXP(-EMSGutEvacR[tds] * ts)); 
  CAftDig[2, tds] = (CAftDig2[tds] * EXP(-EMSGutEvacR[tds] * ts) + (C[2, tds]) * EXP(-EMSGutEvacR[tds] * ts)); 
  CAftDig[3, tds] = (CAftDig3[tds] * EXP(-EMSGutEvacR[tds] * ts) + (C[3, tds]) * EXP(-EMSGutEvacR[tds] * ts));
  CAftDig[4, tds] = (CAftDig4[tds] * EXP(-EMSGutEvacR[tds] * ts) + (C[4, tds]) * EXP(-EMSGutEvacR[tds] * ts)); 
  CAftDig[5, tds] = (CAftDig5[tds] * EXP(-EMSGutEvacR[tds] * ts) + (C[5, tds]) * EXP(-EMSGutEvacR[tds] * ts));
;  CAftDig[6, tds] = (CAftDig6[tds] * EXP(-EMSGutEvacR[tds] * ts) + (C[6, tds]) * EXP(-EMSGutEvacR[tds] * ts));
;  CAftDig[7, tds] = (CAftDig7[tds] * EXP(-EMSGutEvacR[tds] * ts) + (C[7, tds]) * EXP(-EMSGutEvacR[tds] * ts));
;  CAftDig[8, tds] = (CAftDig8[tds] * EXP(-EMSGutEvacR[tds] * ts) + (C[8, tds]) * EXP(-EMSGutEvacR[tds] * ts));


  ; determine the amount of food digested per time step
  ; DigestedFood = Nstom - NstomAFTdig
...
;  DigestedFood[8, tds] = DOUBLE(potst[8, tds] - CAftDig[8, tds])
  ...
  ; Determine total undigested stomach content in g after time step
  ...
  NewStom[tds] = (PotStAftDig[tds]); tds = TotCday < EMScmx
...
IF (dcount GT 0.0) THEN BEGIN; dcount = totcday > EMScmx 

  ; Digestion and evacuation for each prey item
  EMSGutEvacR = 0.0182 * EXP(0.14 * Temp) / 60.0 /0.15 * fDOfc2  ;in g/min temperature-dependent
  
  CAftDig[0, d] = DOUBLE(CAftDig0[d] * EXP(-EMSGutEvacR[d] * ts));
  CAftDig[1, d] = DOUBLE(CAftDig1[d] * EXP(-EMSGutEvacR[d] * ts)); 
  CAftDig[2, d] = DOUBLE(CAftDig2[d] * EXP(-EMSGutEvacR[d] * ts)); 
  CAftDig[3, d] = DOUBLE(CAftDig3[d] * EXP(-EMSGutEvacR[d] * ts)); 
  CAftDig[4, d] = DOUBLE(CAftDig4[d] * EXP(-EMSGutEvacR[d] * ts));
  CAftDig[5, d] = DOUBLE(CAftDig5[d] * EXP(-EMSGutEvacR[d] * ts)); 
;  CAftDig[6, d] = DOUBLE(CAftDig[6, d] * EXP(-EMSGutEvacR[d] * ts))
;  CAftDig[7, d] = DOUBLE(CAftDig[7, d] * EXP(-EMSGutEvacR[d] * ts))
;  CAftDig[8, d] = DOUBLE(CAftDig[8, d] * EXP(-EMSGutEvacR[d] * ts))
...
;VerSize = (Grid2D[3, EMS[13, *]]/20L)*1000L; individual cell depth in mm, there are always 20 vertical cells.
;;CellDepth = (VerSize[*]/1000)
;;PRINT, 'Cell depth (m)'
;;PRINT, CellDepth
;ZooplBio1 = (EMS[15,*])*4000000000D*(VerSize[*]/1000.); g per cell of 2km x 2km x depth from g/L 
;ZooplBio2 = (EMS[16,*])*4000000000D*(VerSize[*]/1000.); g per cell of 2km x 2km x depth from g/L 
;ZooplBio3 = (EMS[17,*])*4000000000D*(VerSize[*]/1000.); g per cell of 2km x 2km x depth from g/L 
;ZooplBioAft1 = ZooplBio1 - C[0,*]*EMS[0,*]; Czoopl1 = g per individual
;ZooplBioAft2 = ZooplBio2 - C[1,*]*EMS[0,*];
;ZooplBioAft3 = ZooplBio3 - C[2,*]*EMS[0,*];
;  PRINT, 'ZooplBio2'
;  PRINT, TRANSPOSE(ZooplBio2)
;  PRINT, 'Czoopl2*EMS[0,*]'
;  PRINT, TRANSPOSE(C[2,*]*EMS[0,*])
;  PRINT, 'ZooplBioAft2'
;  PRINT, TRANSPOSE(ZooplBioAft2)


consumption = FLTARR(50, nEMS)
...
; total amount of prey consumed after time step need to be updated every 24h
consumption[29,*] = TotC[0,*]; total amount of daily consumption of microzooplankton
consumption[30,*] = TotC[1,*]; total amount of daily consumption of small mssozooplankton
consumption[31,*] = TotC[2,*]; total amount of daily consumption of large mesozooplankton
...
; the NUMBER of each prey consumed per time step
...
PRINT, 'Elapesed time (seconds):', t_elapsed/60.
PRINT, 'Emerald Shiner Foraging Ends Here'
