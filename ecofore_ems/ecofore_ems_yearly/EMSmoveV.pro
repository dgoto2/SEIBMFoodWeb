FUNCTION EMSmoveV, EMS, nEMS, newinput, Grid3D, Grid2D, DOacclim, NewGridID2
;function determines movement in X,Y,Z direction for all EMERALD SHINER

;----------------------TEST ONLY---------------------------------------------------------------------------
;PRO EMSmoveV, EMS, nEMS, newinput, Grid3D, Grid2D, DOacclim, NewGridID2
;; NEED to change NewInputFiles, EMSinitial, EMSacclT, EMSacclDO for testing
;nEMS = 50L; number of SI EMS
;ihour = 2L
;EMS = EMSinitial()
...
PRINT, 'EMSVertical Movement Begins Here'
...
;--Vertical movement---------------------------------------------------------------------------------------------------------------------------------
NewCell = FLTARR(15, nEMS);
NewLoc = FLTARR(nEMS)
Ncell = FLTARR(15, nEMS)
d = FLTARR(nEMS)
...
EnvironV = FLTARR(16, nEMS*20L)
;NewGridID2 = FLTARR(nWAE)
NewGridID3D = FLTARR(20L)
;;NewGridIDindex = FLTARR(nWAE*20L)
NewGridID3D2 = FLTARR(nEMS*20L)
...
FOR evvv = 0L, nEMS - 1L DO BEGIN
  FOR ev = 0L, nEMS - 1L DO BEGIN;evv + 1L 
...
LengthV = FLTARR(nEMS*20L)
;depth = indgen(20); 1 = top layer; 20 = bottom layer
j = 19L
FOR ii = 0L, nEMS*20L - 1L DO BEGIN
FOR iii = 0L, nEMS - 1L DO BEGIN
LengthV[ii : j] = EMS[1, iii]
ii = j
j = j + 20L
ENDFOR
ENDFOR
...
;calculate attack probability using chesson's alpha = capture efficiency
  TOT = FLTARR(nEMS*20L); total number of all prey atacked and captured
  t = FLTARR(m, nEMS*20L); total number of each prey atacked and captured
      t[0,*] = (Calpha[0,*] * dens[0,*])
      t[1,*] = (Calpha[1,*] * dens[1,*])
      t[2,*] = (Calpha[2,*] * dens[2,*])
      t[3,*] = (Calpha[3,*] * dens[3,*])
...
preyTOT = FLTARR(nEMS*20L)      
jv = 19L
FOR ivvv = 0L, nEMS*20L - 1L DO BEGIN
...
preyTOT2 = FLTARR(nEMS*20L)
j = 19L
FOR iii = 0L, nEMS*20L - 19L DO BEGIN
FOR iiii = 0L, nEMS*20L - 1L DO BEGIN
...
FOR dovvv = 0L, nEMS - 1L DO BEGIN
  FOR dov = 0L, nEMS - 1L DO BEGIN
...
EnvironVMax = FLTARR(nEMS*20L)
EnvironVMax2 = FLTARR(nEMS*20L)
EnvironVMax3 = FLTARR(nEMS*20L)
jvvv = 19L
; Create array with highest habitat quality-based probability for fish
FOR iv = 0L, nEMS*20L - 1L DO BEGIN
...
DepthLoc = FLTARR(nEMS)
DepthLoc2 = FLTARR(nEMS)
jvvvv = 19L
FOR iv = 0L, nEMS*20L - 1L DO BEGIN
  FOR ivv = 0L, nEMS-1L DO BEGIN 
...
PRINT, 'EMSVertical Movement Ends Here'
